...
uses
  ...
  cxGridCustomView, cxGrid, ExtCtrls, IBase,  Kernel, uUtvDoc,
  cxSplitter, RxMemDS, frxClass, frxDBSet,
  cxEditRepositoryItems, uMainPerem, frxDesgn,uMainFanc;
...
type
  ...
  TWorkDocForm = class(TForm)
    ...
    lblR_Label: TLabel;
    lblS_Label: TLabel;
    lblSch1_Label: TLabel;
    lbl5: TLabel;
    lbl6: TLabel;
    lbl7: TLabel;
    lbl8: TLabel;
    lbl9: TLabel;
    lblSch2_Label: TLabel;
    lbl10: TLabel;
    lblKekv_Label: TLabel;
    pnlProvPanel: TPanel;
    ...
    Summa_Doc_val: TcxGridColumn;
    Summa_Prov_Val: TcxGridColumn;
    lbl11: TLabel;
    cxKurs: TcxCurrencyEdit;
    lbl12: TLabel;
    cxValute: TcxButtonEdit;
    DS_VAL: TpFIBDataSet;
    Summa_Prov_Val1: TcxGridColumn;
    ...
    procedure tmrFlashTimerTimer(Sender: TObject);
    ...
    procedure pnlMainPanelResize(Sender: TObject);
    ...
    procedure cxValutePropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure cxValuteExit(Sender: TObject);
    procedure cxKursPropertiesEditValueChanged(Sender: TObject);
    procedure Summa_Doc_valGetDisplayText(Sender: TcxCustomGridTableItem;
      ARecord: TcxCustomGridRecord; var AText: String);
    procedure Summa_Prov_ValGetDisplayText(Sender: TcxCustomGridTableItem;
      ARecord: TcxCustomGridRecord; var AText: String);
...
var
  WorkDocForm  : TWorkDocForm;  // For add
  WorkDocForm2 : TWorkDocForm; //  For edt

implementation

uses  LoadDogManedger, DogLoaderUnit, GlobalSpr, Math, DateUtils, uSumDocVal, uSpVal;
//Input_Sum,
{$R *.dfm}

procedure TWorkDocForm.FormShow(Sender: TObject);
var
 i, j     : integer;
 Flag     : boolean;
 Sum, Sum_Val      : currency;
 idd      : integer;
 kr       : string;
 do_group : string;
...
// TagTypes.GetFormParams(Self);
 no_action:=0;
 if kurs=0 then begin kurs:=1; cxKurs.EditValue:=1; end;
 if id_valute=0 then cxValute.text:='';;
 ...
 if State = 'View' then begin
  ...
  Summa_Doc_val.Options.Focusing := False;
 ...
 while not DataSet.Eof do begin
  ...
  with ProvArray[Length(ProvArray) - 1] do begin
   ...
   _Pr_Summa_V := DataSet['SUMMA']/kurs;
   _State_Prov := '';
 ...
 with TableView.DataController do begin
  for i := 0 to RecordCount - 1 do begin
   ...
   for j := 0 to Length(ProvArray) - 1 do
    if (Values[i, 7] = ProvArray[j]._IDD) and not (is_Native and ProvArray[j]._is_Sec) then
    begin
      Sum := Sum + ProvArray[j]._Prov_Summa;
      Sum_Val := Sum_Val + ProvArray[j]._Pr_Summa_V;
    end;
   Values[i, 5] := Sum;
   Values[i, 9] := Sum_Val;
  end;
...
procedure TWorkDocForm.FormClose(Sender: TObject;
...
procedure TWorkDocForm.CancelButtonClick(Sender: TObject);
begin
no_action:=1; 
...
procedure TWorkDocForm.FormCreate(Sender: TObject);
begin
 ...
 Summa_Doc_val.DataBinding.ValueTypeClass        := TcxCurrencyValueType;
 ...
 Summa_Prov_Val.DataBinding.ValueTypeClass       := TcxCurrencyValueType;
 ...
 Summa_Prov_Val1.DataBinding.ValueTypeClass       := TcxCurrencyValueType;
 ...
 lblSch1_Label.Caption := '';
 lblSch2_Label.Caption := '';
 lblS_Label.Caption    := '';
 lblR_Label.Caption    := '';
 lblSt_Label.Caption   := '';
 lblKekv_Label.Caption := '';
end;

procedure TWorkDocForm.AddDogButtonClick(Sender: TObject);
var
 S        : TSumDocValForm;
 Sum      : currency;
 Res      : TDogResult;
...
begin
 ...
 S := TSumDocValForm.Create(Self);
// S.ShowExtras := True;
// S.Use_fSum   := True;
kurs:=StrToFloat(cxKurs.Text);
 if ((id_valute<>0) and (kurs<>0)) then
 begin
   s.cxRB_VAL.Checked:=True;
   s.lblVal.Caption:='Валюта: '+cxValute.Text;
   s.lblKurs.Caption:='Курс: '+FloatToStr(kurs);
   s.lblVal.Visible:=True;
   s.lblKurs.Visible:=True;
 end
 else
 begin
   s.cxRB_GRN.Checked:=True;
   s.cxRB_VAL.Enabled:=False;
   s.lblVal.Visible:=False;
   s.lblKurs.Visible:=False;
 end;
 ...
 if ((id_valute<>0) and (kurs<>0)) then
 begin
  if s.cxRB_VAL.Checked=True then
  Sum := s.cxSumma.Value*kurs
  else Sum := S.cxSumma.Value;
 end
 else Sum := S.cxSumma.Value;
 ...
 input.id_Group_add := GROUP_DOG_SEL_COMMON;
 input.Summa        := Sum;
 ...
 num_dog:=StoredProc.ParamByName('NUM_DOG').AsString;
...
// ShowMessage(IntToStr());
  ...
  Values[RecordCount - 1, 4] := num_dog;//Res.regest_num;
  Values[RecordCount - 1, 5] := Sum;
  Values[RecordCount - 1, 6] := Res.name_shablon;
  Values[RecordCount - 1, 7] := IDD;
  Values[RecordCount - 1, 8] := Res.id_customer;
  Values[RecordCount - 1, 9] := Sum/kurs;

end;

//for i:=0 to  TableView.DataController.ItemCount-1 do ShowMessage(VarToStr( TableView.DataController.Values[0,i]));
...
procedure TWorkDocForm.SetProv(idd, id_Prov : integer);
var
 i, j : integer;
 Flag : boolean;
begin
 ...
 for i := 0 to Length(ProvArray) - 1 do begin
  ...
  if Flag then with ProvTableView.DataController do begin
   ...
   Values[RecordCount - 1, 21] := ProvArray[i]._Pr_Summa_V;
  end
   ...
   Values[RecordCount - 1, 19] := ProvArray[i]._id_Kekv;
   Values[RecordCount - 1, 20] := ProvArray[i]._Name_Kekv;
   Values[RecordCount - 1, 21] := ProvArray[i]._Prov_Summa/kurs;
  end;
...
procedure TWorkDocForm.TableViewFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TWorkDocForm.Reg_Num_ColumnPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
 k       : integer;
 ResProc : TResProc;
...
procedure TWorkDocForm.ProvTableViewFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
destructor TWorkDocForm.Destroy;
begin
  if State = 'Add' then WorkDocForm := nil
  else if State = 'Edit' then WorkDocForm2 := nil;
  inherited;
end;

procedure TWorkDocForm.Del_Prov_ButtonClick(Sender: TObject);
var
 Rec     : integer;
 k       : integer;
 i, n    : integer;
...
procedure TWorkDocForm.Add_Prov_ButtonClick(Sender: TObject);
var
 Rec     : integer;
 id_Prov : integer;
 i       : integer;
...
procedure TWorkDocForm.AddSecondButtonClick(Sender: TObject);
var
 Rec     : integer;
 id_Prov : integer;
 i       : integer;
...
procedure TWorkDocForm.SetLabels;
var
 k : integer;
begin
 lblSch1_Label.Caption := '';
 ...
 if ProvGrid.IsFocused or not SecGrid.Focused then with ProvTableView.DataController do begin
  k := FocusedRecordIndex;
  if k >= 0 then begin
   if not VarIsNull(Values[k, 14]) then lblS_Label.Caption    := Values[k, 10];
   if not VarIsNull(Values[k, 15]) then lblR_Label.Caption    := Values[k, 11];
   if not VarIsNull(Values[k, 16]) then lblSt_Label.Caption   := Values[k, 12];
   if not VarIsNull(Values[k, 20]) then lblKekv_Label.Caption := Values[k, 13];
   if not VarIsNull(Values[k, 5])  then lblSch1_Label.Caption := Values[k, 3];
   if not VarIsNull(Values[k, 6])  then lblSch2_Label.Caption := Values[k, 4];

   if not VarIsNull(Values[k, 14]) then lblS_Label.Caption    := lblS_Label.Caption    + ' "' + Values[k, 14] + '"';
   if not VarIsNull(Values[k, 15]) then lblR_Label.Caption    := lblR_Label.Caption    + ' "' + Values[k, 15] + '"';
   if not VarIsNull(Values[k, 16]) then lblSt_Label.Caption   := lblSt_Label.Caption   + ' "' + Values[k, 16] + '"';
   if not VarIsNull(Values[k, 20]) then lblKekv_Label.Caption := lblKekv_Label.Caption + ' "' + Values[k, 20] + '"';
   if not VarIsNull(Values[k, 5])  then lblSch1_Label.Caption := lblSch1_Label.Caption + ' "' + Values[k, 5]  + '"';
   if not VarIsNull(Values[k, 6])  then lblSch2_Label.Caption := lblSch2_Label.Caption + ' "' + Values[k, 6]  + '"';
  end;
 end
 else if SecGrid.IsFocused then with SecTableView.DataController do begin
  k := FocusedRecordIndex;
 ...
 lblSch1_Label.Hint := lblSch1_Label.Caption;
 lblSch2_Label.Hint := lblSch2_Label.Caption;
 lblS_Label.Hint    := lblS_Label.Caption;
 lblR_Label.Hint    := lblR_Label.Caption;
 lblSt_Label.Hint   := lblSt_Label.Caption;
 lblKekv_Label.Hint := lblKekv_Label.Caption;
end;

procedure TWorkDocForm.SchTableViewDblClick(Sender: TObject);
var
 k : integer;
begin
...
procedure TWorkDocForm.SchTableViewKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TWorkDocForm.ProvTableViewCustomDrawCell(
...
procedure TWorkDocForm.ApplyButtonClick(Sender: TObject);
var
 Flag : boolean;
 i, j : integer;
 k    : integer;
...
procedure TWorkDocForm.DelDogButtonClick(Sender: TObject);
var
 k    : integer;
 i, j : integer;
 b    : boolean;
...
function TWorkDocForm.SaveDoc : boolean;
var
 Kredit      : word;
 i, j        : integer;
 Sum         : currency;
...
procedure TWorkDocForm.CollapseInfoMemo;
begin
...
procedure TWorkDocForm.ExpandInfoMemo;
begin
 if InfoMemo.Height < 56 then InfoMemo.Height := InfoMemo.Height + 1
 else Timer.Enabled := False;
end;

procedure TWorkDocForm.TimerTimer(Sender: TObject);
begin
 if Expand then ExpandInfoMemo
 else CollapseInfoMemo;
end;

procedure TWorkDocForm.tmrFlashTimerTimer(Sender: TObject);
begin
 ...
 if (FlashTime < 0) or not (FlashDate or FlashMemo or FlashNum) then begin
  ...
  tmrFlashTimer.Enabled := False;
 end;
end;

procedure TWorkDocForm.FlashInfoMemo;
begin
 InfoMemo.Text := '';
 Expand        := False;
 Timer.Enabled := True;
...
function TWorkDocForm.CheckBudget(Num : integer): boolean;
begin
 Result := False;
...
procedure TWorkDocForm.Kod_S_ColumnPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
 Rec : integer;
 k   : integer;
...
procedure TWorkDocForm.EditProvButtonClick(Sender: TObject);
var
 Rec     : integer;
 k       : integer;
 i       : integer;
...
procedure TWorkDocForm.kod_Kekv_ColumnPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
 Rec  : integer;
 k, i : integer;
...
procedure TWorkDocForm.CloneButtonClick(Sender: TObject);
var
 k        : integer;
 S        : TSumDocValForm;
 Sum      : currency;
...
procedure TWorkDocForm.Prov_Summa_ColumnPropertiesEditValueChanged(
  Sender: TObject);
var
 Rec : integer;
 k   : integer;
...
procedure TWorkDocForm.TableViewKeyDown(Sender: TObject;
...
procedure TWorkDocForm.ProvTableViewKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var
 k : integer;
begin
...
procedure TWorkDocForm.act1Execute(Sender: TObject);
begin
 ApplyButtonClick(Sender);
end;

procedure TWorkDocForm.NumDocKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TWorkDocForm.DateDocKeyPress(Sender: TObject;
  var Key: Char);
begin
 if Key = #13 then begin
  Key := #0;
...
procedure TWorkDocForm.NoteMemoKeyPress(Sender: TObject;
  var Key: Char);
begin
 if Key = #13 then begin
  Key := #0;
...
procedure TWorkDocForm.ProvGridEnter(Sender: TObject);
begin
 SetLabels;
end;

procedure TWorkDocForm.EditDogButtonClick(Sender: TObject);
var
 k : integer;
 Res      : TDogResult;
 input    : TDogInput;
...
procedure TWorkDocForm.pnlMainPanelResize(Sender: TObject);
begin
 pnlDogPanel.Width := (pnlMainPanel.Width - 15) div 2;

 pnlProvPanel.Left  := pnlDogPanel.Width + 10;
 pnlProvPanel.Width := (pnlMainPanel.Width - 15) div 2;

 pnlInfoPanel.Left  := pnlProvPanel.Left;
 pnlInfoPanel.Width := pnlProvPanel.Width;

 bvl1.Width    := pnlDogPanel.Width;
 NoteMemo.Width  := bvl1.Width - 147 - 5;

 lblSch1_Label.Width := pnlInfoPanel.Width - 73;
 lblSch2_Label.Width := pnlInfoPanel.Width - 73;
 lblS_Label.Width    := pnlInfoPanel.Width - 73;
 lblR_Label.Width    := pnlInfoPanel.Width - 73;
 lblSt_Label.Width   := pnlInfoPanel.Width - 73;
 lblKekv_Label.Width := pnlInfoPanel.Width - 73;

 SecGrid.Height := pnlProvPanel.Height div 2;
end;

procedure TWorkDocForm.PrintButtonClick(Sender: TObject);
var
 i, j : integer;
begin
...
procedure TWorkDocForm.ReportGetValue(const VarName: String;
  var Value: Variant);
var
 i : integer;
 s : currency;
...
procedure TWorkDocForm.Num_Sch1_ColumnGetProperties(
...
procedure TWorkDocForm.Num_Sch2_ColumnGetProperties(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TWorkDocForm.cxEditRepository1PopupItem1PropertiesPopup(
  Sender: TObject);
var
 k   : integer;
 Key : string;
...
procedure TWorkDocForm.cxEditRepository1PopupItem1PropertiesCloseUp(
  Sender: TObject);
var
 Rec : integer;
 k, t : integer;
...
procedure TWorkDocForm.cxEditRepository1ButtonItem1PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
...
procedure TWorkDocForm.cxValutePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
  var f:TSpVAlForm;
begin
 ...
 begin
  cxValute.Text:=VarToStr(DS_VAL.FieldValues['CODE_VALUTE']);
...
procedure TWorkDocForm.cxValuteExit(Sender: TObject);
begin
 ds_val.Close;
 DS_VAL.SelectSQL.Clear;
 DS_VAL.SelectSQL.Add(' Select first(1) a.date_kurs, b.code_valute, a.kurs, a.id_valute, b.special_sign ');
 ...
 DS_VAL.Prepare;
 DS_VAL.ParamByName('DATE_KURS').AsDate:=DateDoc.EditValue;
 DS_VAL.ParamByName('CODE_VALUTE').AsString:=AnsiUpperCase(cxValute.Text);
 ds_val.closeopen(false);
...
procedure TWorkDocForm.cxKursPropertiesEditValueChanged(Sender: TObject);
var i:Integer;
begin
{kurs:=cxKurs.EditValue;
with TableView.DataController do
for i := 0 to RecordCount - 1 do
begin
  ShowMessage(VarToStr(Values[i,9])+ ' '+VarToStr(Values[i,5]));
  Values[i,9]:=values[i,5]/kurs;
end; }
//SetProv(-1,-1);

end;

procedure TWorkDocForm.Summa_Doc_valGetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
  var i:Integer;
begin
 with TableView.DataController do begin
   for i := 0 to RecordCount - 1 do
   if Values[i, 9]<>null then
   AText:=FloatToStrF(Values[i, 9], ffNumber, 18, 2)+' '+symbol;//Values[i, 7] > idd then idd := Values[i, 7];
   end
 //  AText:=FloatToStrF(Summa_Doc_val.EditValue, ffNumber, 18, 2)+' '+symbol;
end;

procedure TWorkDocForm.Summa_Prov_ValGetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
begin
  if VarToStr(Summa_Prov_Val.EditValue) = '' then atext:='0'+DecimalSeparator+'00 '+symbol
  else AText:=FloatToStrF(Summa_Prov_Val.EditValue, ffNumber, 18, 2)+' '+symbol;
end;

procedure TWorkDocForm.cxKursExit(Sender: TObject);
var i:Integer;
begin
 kurs:=StrToFloat(cxKurs.Text);
