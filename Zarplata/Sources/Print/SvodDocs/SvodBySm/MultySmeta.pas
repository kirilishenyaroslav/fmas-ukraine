...
uses
  ...
  ActnList, Placemnt, dxStatusBar, findsmeta, cxCheckBox, RxMemDS, cxCalc,
...
type
  ...
  public
    ...
    constructor Create(AOwner : TComponent; DBHANDLE : TISC_DB_HANDLE;ActualDate:TDateTime;id_session:Integer=0); overload;
  end;


  function GetSmetsMulty(AOwner : TComponent; DBHANDLE : TISC_DB_HANDLE;ActualDate:TDateTime;id_session:integer):TDataSet;stdcall;
...
function GetSmetsMulty(AOwner : TComponent; DBHANDLE : TISC_DB_HANDLE;ActualDate:TDateTime;id_session:integer):TDataSet;
var T:TFpub_spr_smet;
    Res:TDataSet;
begin
     T:=TFpub_spr_smet.Create(AOwner,DBHANDLE,ActualDate,id_session);
     T.ShowModal;
     Res:=T.ResultDataSet;
     T.Free;
     T:=nil;
...
constructor TFpub_spr_smet.Create(AOwner: TComponent; DBHANDLE: TISC_DB_HANDLE; ActualDate: TDateTime; id_session:Integer);
begin
      ...
      DateTimeToString(ActualDate_str,'dd.mm.yyyy',self.ActualDate);
                       ...
                       begin
                             ...
                             SmetaDataSet.SQLs.SelectSQL.Text:= '   SELECT DISTINCT S.ID_SMETA, S.SMETA_TITLE, S.SMETA_KOD, SC.sch_number, SC.sch_title '+
                                                                '   FROM PUB_SPR_SMETA S inner join Z_SVOD_ACCOUNT A on A.ID_SMETA=S.ID_SMETA'+
                                                                '      inner join pub_sp_main_sch sc on sc.id_sch = A.kod_toper    '+
                                                                '   WHERE       A.id_session='+inttostr(id_session)+
                                                                '   ORDER BY SC.sch_number, S.SMETA_KOD                           ';
                             SmetaDataSet.Open;
                             ...
                             RxMemoryData1.FieldDefs.Add('sch_number',ftString,20,True);
                             RxMemoryData1.FieldDefs.Add('sch_title',ftString,100,True);
