...
type
  TSvodByGroupPr_DM = class(TDataModule)
    DB: TpFIBDatabase;
...
procedure TSvodByGroupPr_DM.PrintSpr(AParameter:TSvodParam;ResDateSet:TDataSet);
...
begin
 ...
 {DSetFoundationData.SQLs.SelectSQL.Text :=     'SELECT * FROM Z_SVODBYGROUPSM_SMETA1(:ID_TYPE_FINANCE'+','''+
                                                                                    DateToStr(DateForm)+''','+
 ...
 PathReport:=IniFile.ReadString(SectionOfIniFile,'NameReport1',NameReport);
 //else
...
procedure TSvodByGroupPr_DM.ReportGetValue(const VarName: String; var Value: Variant);
begin
if UpperCase(VarName)='PKOD_SETUP' then
  Value:=KodSetupToPeriod(PKod_Setup,4);
end;

procedure TSvodByGroupPr_DM.DataModuleDestroy(Sender: TObject);
begin
