...
const PathReport               = 'Reports\Zarplata\SprYearDecl.fr3';
...
begin
 ...
 CanEdit := True;
 DSet.SQLs.SelectSQL.Text := 'SELECT * FROM Z_SPR_DOH_DECLARATION('+IntToStr(AParameter.ID_man)+','
                                                       +IntToStr(AParameter.Kod_setup1)+','
                                                       +IntToStr(AParameter.Kod_setup2)+')';
 DSetFoundationData.SQLs.SelectSQL.Text := 'SELECT * FROM Z_FOUNDATION_DATA_SPR_YEAR_DECL('+IntToStr(AParameter.ID_man)+','+IntToStr(AParameter.Kod_setup2)+')';
 ...
 while not DSet.Eof do
  begin
    SummaDoh:=SummaDoh+DSet['SUMMA_DOH_ALL'];
