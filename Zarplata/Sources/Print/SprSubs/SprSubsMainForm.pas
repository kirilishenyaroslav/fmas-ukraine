...
uses
  ...
  cxDropDownEdit, cxControls, cxGroupBox, IBase, SprSubsDataModul,
...
type
  TFSprSubs = class(TForm)
    ...
    pnl1: TPanel;
    cxLabel2: TcxLabel;
    RadioBtnFact: TcxRadioButton;
    RadioBtnMonth: TcxRadioButton;
...
{RadioBtnWorker:=TcxRadioButton.Create(cxRadioGroup1);
RadioBtnWorker.ParentBackground
RadioBtnStud:=TcxRadioButton.Create(cxRadioGroup1);
RadioBtnMan:=TcxRadioButton.Create(cxRadioGroup1);

RadioBtnFact:=TcxRadioButton.Create(cxRadioGroup2);
RadioBtnMonth:=TcxRadioButton.Create(cxRadioGroup2); }
...
begin
  ...
  PParameter.DB_Handle:=PDBHandle;
  PParameter.Owner:=self;
  Man:=LoadSimplePackage(self,PDBHandle,'Zarplata\SpSotr.bpl','View_SpSotr');;
end;
...
if RadioBtnFact.Checked then PFilterData.P1:=1 //za factom
else if RadioBtnMonth.Checked then PFilterData.P1:=2; // po mesychno
