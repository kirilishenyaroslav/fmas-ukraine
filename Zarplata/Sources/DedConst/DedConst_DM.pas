...
uses
  ...
  IBase, RxMemDS, FIBQuery, pFIBQuery, pFIBStoredProc, frxClass, frxDBSet, Forms,
  frxDesgn, zProc, Dates;
...
begin
  wf:=ShowWaitForm(Application.MainForm);
   ...
   DSet1.SQLs.SelectSQL.Text:='SELECT * FROM Z_DEDCONST_LISTPEOPLE('+IntToStr(pKodSetupBeg)+','+IntToStr(pKodSetupEnd)+') order by tn';
   DSet2.SQLs.SelectSQL.Text:='SELECT * FROM Z_GET_ASUP_RMOVING(?ID_MAN)';
   DSet3.SQLs.SelectSQL.Text:='SELECT * FROM Z_DEDCONST_SELECT(?RMOVING,'+IntToStr(pKodSetupBeg)+','+IntToStr(pKodSetupEnd)+') order by DATE_BEG';
   DSet1.Open;
   DSet2.Open;
   DSet3.Open;
  CloseWaitForm(wf);
...
const NameReport = '\Reports\Zarplata\DedConst.fr3';
var wf:TForm;
begin
 wf:=ShowWaitForm(Self.Owner as TForm);
 if DSet4.Active then DSet4.Close;
 DSet4.SQLs.SelectSQL.Text:='SELECT * FROM Z_DEDCONST_SELECT_FOR_REPORT('+IntToStr(pKodSetupBeg)+','+IntToStr(pKodSetupEnd)+') order by kod_vidopl, tn';
 DSet4.Open;
