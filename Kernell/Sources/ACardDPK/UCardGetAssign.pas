...
procedure TfrmGetAssignMain.ConfigureGrid;
...
begin
     ...
     GetKekvInfo.SelectSQL.Text:=' SELECT * '+
                                 '   FROM ACARDS_GET_KEKV3( '+ IntToStr(id_pkv)+','+IntToStr(id_tfin)+
     ...
     then begin
               ...
               begin
                    ...
                    ArrColumns[i-1].Tag:=GetKekvInfo.FieldByName('ID_KEKV').AsInteger;
...
constructor TfrmGetAssignMain.Create(AOwner: TComponent; DBHandle: TISC_DB_HANDLE; Id_user: Int64;id_param_set:Integer);
...
procedure TfrmGetAssignMain.FormClose(Sender: TObject;
...
function TfrmGetAssignMain.getFactQuerySql: String;
var  Res:String;
...
begin
     ...
     then begin
               ...
               begin
                    Res:=Res+ ' (SELECT KASSA_SUMMA FROM ACARD_GET_ASSIGN_FACT_VAL_ITEM('+IntToStr(id_pkv)+','+IntToStr(id_tfin)+','+IntToStr(fid_param_set)+
     ...
     Res:=Res+'  FROM ACARD_GET_ASSIGN_FACT_VALUES_EX ('+IntToStr(id_pkv)+','+IntTostr(id_tfin)+','+IntToStr(fid_param_set)+
...
function TfrmGetAssignMain.getPlanQuerySql: String;
var  Res:String;
     GetKekvInfo:TpFibDataSet;
     i:Integer;
begin
...
procedure TfrmGetAssignMain.fillTmpTable;
...
procedure TfrmGetAssignMain.getReportData;
begin
     Screen.Cursor:=crHourGlass;
...
procedure TfrmGetAssignMain.clearTmpTable;
var fillSP:TpFibStoredProc;
begin
     fillSP:=TpFibStoredProc.Create(self);
     fillSP.Database:=WorkDatabase;
...
procedure TfrmGetAssignMain.ToolButton3Click(Sender: TObject);
...
procedure TfrmGetAssignMain.ToolButton5Click(Sender: TObject);
begin
     Close;
end;

procedure TfrmGetAssignMain.ToolButton2Click(Sender: TObject);
     ...
     then begin
              ...
              then begin
                         ...
                         then begin
                                   ...
                                   then begin
                                             id_razdel:=-1;
                                             id_kekv  :=FactView.Controller.FocusedColumn.Tag;
                                   end
                                   else begin
                                             id_razdel:=FactView.Controller.FocusedColumn.Tag;
                                             id_kekv  :=-1
                                   end;
...
procedure TfrmGetAssignMain.ToolButton6Click(Sender: TObject);

      var T:TAcardPrintReportForm;
begin
     T:=TAcardPrintReportForm.Create(self,5);
     T.ShowModal;
     T.Free;
end;
