...
type
  ...
  procedure GetKrDbZ(AOwner:TComponent;DBHandle:TISC_DB_HANDLE;Id_User:Integer);stdcall;
  exports GetKrDbZ;

implementation


uses UKrDbConfigureForm, cxRadioGroup, cxCalendar, UGetSchFilter;
{$R *.dfm}
...
begin
     ...
     SQLText:='SELECT SUM(DB_SUMMA) AS DB_SUMMA, SUM(KR_SUMMA) AS KR_SUMMA '+SeleSch + SeleSmGr + SeleSm + SeleRz + SeleSt + SeleKV + SelePKV+
              '  FROM MBOOK_GET_KRDB_Z('+''''+DateToStr(DateBeg)+''''+','+IntToStr(Integer(ShowSchFilter))+','+IntToStr(self.KEY_SESSION)+')'+
...
begin
     ...
     then begin
                 ...
                 ShowSchFilter:=T.ShowSchFilter.Checked;

                 if ShowSchFilter
                 then begin
                           F:=TfrmGetSchFilter.Create(self,KEY_SESSION);
                           if F.ShowModal=mrYes
                           then ShowSchFilter:=true
                           else ShowSchFilter:=false;
                           F.Free;
                 end;
...
begin
     ...
     T.ShowSchFilter.Checked:=ShowSchFilter;
...
begin
     ...
     begin
          ...
          and (OstView.Columns[i].DataBinding.FieldName<>'DB_SUMMA')
          and (OstView.Columns[i].DataBinding.FieldName<>'KR_SUMMA')
          then begin
     ...
     begin
          ...
          then begin
                     ...
                     if (OstView.Columns[i].DataBinding.FieldName='DB_SUMMA')
                     or (OstView.Columns[i].DataBinding.FieldName='KR_SUMMA')
                     then begin
