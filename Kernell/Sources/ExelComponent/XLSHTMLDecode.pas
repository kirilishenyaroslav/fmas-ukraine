...
uses Classes, SysUtils, XLSUtils2;

function UTF8ToWideString(S: string): WideString;
function DecodeHTMLText(Text: WideString): WideString;

implementation

type THTMLChar = record
     Name: string;
     Value: WideChar;
     end;

// Taken from http://www.w3.org/TR/REC-html40/sgml/entities.html.
const HTMLChars: array[0..252] of THTMLChar = (
   ...
   // General Punctuation
    ...
    (Name: 'apos';     Value: #39),   // single quote
...
begin
  ...
  while i <= Length(S) do begin
    ...
    if x < $80 then begin
      ...
      if (x and $E0) = $C0 then
        m := $1F
      else if (x and $F0) = $E0 then
        m := $0F
      else if (x and $F8) = $F0 then
        m := $07
      else if (x and $FC) = $F8 then
        m := $03
      else if (x and $FE) = $FC then
        m := $01
      else
        m := 0;
      V := x and m;
  ...
  while i <= Length(Text) do begin
    if Text[i] = '&' then begin
      ...
      while (i <= Length(Text)) and (Text[i] <> ';') do
        Inc(i);
      S := Copy(Text,j + 1,i - j - 1);
      j := HTMLCharList.IndexOf(S);
      if j >= 0 then
        Result := Result + WideChar(HTMLCharList.Objects[j]);
      Inc(i);
    end
    else begin
...
initialization
  HTMLCharList := TStringList.Create;
  for ii := 0 to High(HTMLChars) do
    HTMLCharList.AddObject(HTMLChars[ii].Name,Pointer(HTMLChars[ii].Value));
  HTMLCharList.Sorted := True;
