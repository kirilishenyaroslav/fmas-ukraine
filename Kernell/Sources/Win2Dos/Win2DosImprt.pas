...
uses
  ...
  cxDropDownEdit, cxCalendar,pFibStoredProc, FIBDatabase, pFIBDatabase,
  IniFiles, uPwdDecrypt;
...
begin
     ...
     then begin
                ...
                CheckSP.Transaction:=WorkTransaction;
                CheckSP.StoredProcName:='WIN_TO_DOS_CHECK_DOC';
                CheckSP.Prepare;

                //Инициализация забора проводок
                ProvFromWinDS:=TpfibDataSet.Create(self);
                ProvFromWinDS.Database:=WorkDatabase;
                ProvFromWinDS.Transaction:=WorkTransaction;

                //Забор документов за период
                DocFromWinDS:=TpfibDataSet.Create(self);
                DocFromWinDS.Database:=WorkDatabase;
                DocFromWinDS.Transaction:=WorkTransaction;
                ...
                begin
                       SetWindowText(handle,PAnsiChar(IntToStr(i)+' из '+IntToStr(DocFromWinDS.RecordCount)));
                       Docs.StepIt;
                       Application.ProcessMessages;
...
procedure TfrmDosImport.FormShow(Sender: TObject);
var
   IniFile : TIniFile;
   APP_PATH:string;
begin
    //Чтение информации из файла инициализации
    IniFile:=nil;
    APP_PATH := ExtractFileDir(Application.ExeName) + '\';
    if not FileExists(APP_PATH + INI_FILENAME)
    then begin
            MessageBox(Application.Handle,'Не найден файл инициилизации!','Ошибка!',MB_OK or MB_ICONWARNING);
            SendMessage(self.Handle,WM_CLOSE,0,0);
    end
    else begin
              try
                  ...
                  DB_PASSWORD     := uPwdDecrypt.PwdDeCrypt(IniFile.ReadString('CONNECTION', 'Password_Crypted', DB_PASSWORD));
                  DBFPATH         := IniFile.ReadString('W2D', 'DBFPath', ExtractFileDir(Application.ExeName)+'\');
                  cxDateEdit1.Date:= StrToDate(IniFile.ReadString('W2D', 'StartDate', '01.09.2005'));
                  EdDBFOutputPath.Text:=DBFPATH;
                  CORRECT_CONFIG:=true;
              finally
                  IniFile.Free;
              end;
    ...
    WorkDatabase.DBName :=DB_SERVER+':'+DB_PATH;
    WorkDatabase.ConnectParams.UserName := DB_USER;
    WorkDatabase.ConnectParams.Password := DB_PASSWORD;
    try
        WorkDatabase.Open;
        ...
        on E:exception do
            begin
                MessageBox(Application.Handle,'Ошибка подключения к БД!','Ошибка!',MB_OK or MB_ICONWARNING);
                SendMessage(self.Handle,WM_CLOSE,0,0);
