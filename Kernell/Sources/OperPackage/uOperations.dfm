object frmSchOperations: TfrmSchOperations
  Left = 602
  Top = 157
  Width = 665
  Height = 500
  Caption = #1056#1086#1073#1086#1090#1072' '#1079' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1103#1084#1080' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1110#1103#1084
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object CoolBar1: TCoolBar
    Left = 0
    Top = 0
    Width = 649
    Height = 47
    AutoSize = True
    Bands = <
      item
        Control = ToolBar1
        ImageIndex = -1
        MinHeight = 41
        Width = 423
      end
      item
        Break = False
        Control = ToolBar3
        ImageIndex = -1
        MinHeight = 43
        Width = 220
      end>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    object ToolBar1: TToolBar
      Left = 9
      Top = 1
      Width = 410
      Height = 41
      AutoSize = True
      ButtonHeight = 39
      ButtonWidth = 59
      Caption = 'ToolBar1'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Images = SmallImages
      ParentFont = False
      ShowCaptions = True
      TabOrder = 0
      object InsertButton: TToolButton
        Left = 0
        Top = 0
        Hint = #1053#1086#1074#1072' '#1086#1087#1077#1088#1072#1094#1110#1103
        Caption = #1044#1086#1076#1072#1090#1080
        ImageIndex = 1
        ParentShowHint = False
        ShowHint = True
        OnClick = InsertButtonClick
      end
      object DeleteButton: TToolButton
        Left = 59
        Top = 0
        Hint = #1042#1080#1076#1072#1083#1080#1090#1080' '#1086#1087#1077#1088#1072#1094#1110#1102
        Caption = #1042#1080#1076#1072#1083'.'
        DropdownMenu = DelPopupMenu
        ImageIndex = 2
        ParentShowHint = False
        ShowHint = True
        Style = tbsDropDown
        OnClick = DeleteButtonClick
      end
      object UpdateButton: TToolButton
        Left = 133
        Top = 0
        Hint = #1056#1077#1076#1072#1075#1091#1074#1072#1090#1080' '#1086#1087#1077#1088#1072#1094#1110#1102
        Caption = #1047#1084#1110#1085#1080#1090#1080
        ImageIndex = 5
        ParentShowHint = False
        ShowHint = True
        OnClick = UpdateButtonClick
      end
      object ToolButton1: TToolButton
        Left = 192
        Top = 0
        Width = 24
        Caption = 'ToolButton1'
        ImageIndex = 6
        Style = tbsSeparator
      end
      object cbMonth: TComboBox
        Left = 216
        Top = 8
        Width = 107
        Height = 22
        Style = csOwnerDrawFixed
        Color = clInfoBk
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ItemHeight = 16
        ParentFont = False
        TabOrder = 0
        OnChange = cbMonthChange
      end
      object cbYear: TComboBox
        Left = 323
        Top = 8
        Width = 74
        Height = 22
        Style = csOwnerDrawFixed
        Color = clInfoBk
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ItemHeight = 16
        ParentFont = False
        TabOrder = 1
        OnChange = cbMonthChange
      end
    end
    object ToolBar3: TToolBar
      Left = 434
      Top = 0
      Width = 207
      Height = 43
      ButtonHeight = 39
      ButtonWidth = 64
      Caption = 'ToolBar1'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Images = SmallImages
      ParentFont = False
      ShowCaptions = True
      TabOrder = 1
      object PropButton: TToolButton
        Left = 0
        Top = 0
        Hint = #1042#1083#1072#1089#1090#1080#1074#1086#1089#1090#1110' '#1086#1087#1077#1088#1072#1094#1110#1111
        Caption = #1042#1083#1072#1089#1090'.'
        ImageIndex = 14
        ParentShowHint = False
        ShowHint = True
        OnClick = PropButtonClick
      end
      object RefreshButton: TToolButton
        Left = 64
        Top = 0
        Hint = #1054#1085#1086#1074#1080#1090#1080' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1102' '#1079' '#1089#1077#1088#1074#1077#1088#1072
        Caption = #1054#1085#1086#1074#1080#1090#1080
        ImageIndex = 10
        ParentShowHint = False
        ShowHint = True
        OnClick = RefreshButtonClick
      end
      object ChooseButton: TToolButton
        Left = 128
        Top = 0
        Hint = #1042#1080#1093#1110#1076
        Caption = #1042#1080#1093#1110#1076
        ImageIndex = 6
        ParentShowHint = False
        ShowHint = True
        OnClick = ChooseButtonClick
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 47
    Width = 649
    Height = 34
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 1
    object Label1: TLabel
      Left = 0
      Top = 0
      Width = 649
      Height = 34
      Align = alClient
      Alignment = taCenter
      Caption = 'Label1'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 114
    Width = 649
    Height = 328
    ActivePage = TabSheet1
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    Style = tsFlatButtons
    TabOrder = 2
    object TabSheet1: TTabSheet
      Caption = #1043#1086#1083#1086#1074#1085#1080#1081' '#1088#1072#1093#1091#1085#1086#1082' '#1091' '#1076#1077#1073#1077#1090#1110
      object DBTreeList: TcxDBTreeList
        Left = 0
        Top = 0
        Width = 641
        Height = 294
        Styles.Background = cxStyle1
        Styles.Content = cxStyle1
        Styles.BandBackground = cxStyle1
        Styles.BandContent = cxStyle1
        Styles.BandHeader = cxStyle2
        Styles.ColumnHeader = cxStyle2
        Align = alClient
        Bands = <
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1056#1072#1093#1091#1085#1082#1080' '#1086#1087#1077#1088#1072#1094#1110#1111
            FixedKind = tlbfLeft
            Width = 206
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1050#1086#1085#1092#1110#1075#1091#1088#1072#1094#1110#1103' '#1086#1087#1077#1088#1072#1094#1110#1111
            Visible = False
            Width = 366
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1050#1086#1088#1077#1089#1087#1086#1085#1076#1091#1102#1095#1110#1081' '#1088#1072#1093#1091#1085#1086#1082
            Visible = False
            Width = 208
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1052#1077#1084#1086#1088#1110#1072#1083#1100#1085#1080#1081' '#1086#1073#1083#1110#1082
            Width = 379
          end>
        BufferedPaint = False
        DataController.DataSource = DbDataSource
        DataController.ParentField = 'linkto'
        DataController.KeyField = 'id_oper'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        OptionsCustomizing.RowSizing = True
        OptionsData.CancelOnExit = False
        OptionsData.Editing = False
        OptionsData.Deleting = False
        OptionsData.CheckHasChildren = False
        OptionsSelection.CellSelect = False
        OptionsView.Bands = True
        OptionsView.ColumnAutoWidth = True
        OptionsView.GridLines = tlglBoth
        OptionsView.ShowRoot = False
        OptionsView.TreeLineStyle = tllsSolid
        ParentFont = False
        RootValue = -1
        TabOrder = 0
        object cxDBTreeList1cxDBTreeListColumn1: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1077#1073#1077#1090
          DataBinding.FieldName = 'ROOT_TEXT'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 0
          Width = 115
        end
        object cxDBTreeList1cxDBTreeListColumn2: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1050#1088#1077#1076#1080#1090
          DataBinding.FieldName = 'ROOT_TEXT_EX'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 0
          Width = 109
        end
        object cxDBTreeList1cxDBTreeListColumn3: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1050#1086#1076' '#1088#1077#1075#1110#1089#1090#1088#1072
          DataBinding.FieldName = 'KR_REG_UCH_KOD'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 2
          Width = 85
        end
        object DBTreeListcxDBTreeListColumn1: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1053#1072#1081#1084#1077#1085#1091#1074#1072#1085#1085#1103' '#1088#1077#1075#1110#1089#1090#1088#1072
          DataBinding.FieldName = 'KR_REG_UCH_TITLE'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 2
          Width = 123
        end
        object DBTreeListcxDBTreeListColumn2: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1054#1073#1086#1088#1086#1090#1080' '#1088#1072#1093#1091#1102#1090#1100#1089#1103' '#1076#1086' '#1084#1077#1084#1086#1088#1110#1072#1083#1100#1085#1086#1075#1086' '#1086#1088#1076#1077#1088#1072
          DataBinding.FieldName = 'MAIN_MO_KOD'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          SortOrder = soAscending
          Width = 413
        end
        object DBTreeListcxDBTreeListColumn3: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taLeftJustify
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1052'.'#1086#1088#1076#1077#1088' '#1087#1086' '#1076#1077#1073#1077#1090#1091
          DataBinding.FieldName = 'DB_ID_MO_KOD'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          Visible = False
        end
        object DBTreeListcxDBTreeListColumn4: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1052'.'#1086#1088#1076#1077#1088' '#1087#1086' '#1082#1088#1077#1076#1080#1090#1091
          DataBinding.FieldName = 'KR_ID_MO_KOD'
          Position.ColIndex = 2
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          Visible = False
          Width = 105
        end
        object DBTreeListcxDBTreeListColumn5: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1086#1082#1091#1084#1077#1085#1090' '#1074' '#1076#1077#1073#1077#1090#1110
          DataBinding.FieldName = 'DB_CR_DOC'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 90
        end
        object DBTreeListcxDBTreeListColumn6: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1055#1088#1086#1074#1086#1076#1082#1072' '#1074' '#1076#1077#1073#1077#1090#1110
          DataBinding.FieldName = 'DB_CR_PROV'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 95
        end
        object DBTreeListcxDBTreeListColumn7: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1086#1082#1091#1084#1077#1085#1090' '#1074' '#1082#1088#1077#1076#1080#1090#1110
          DataBinding.FieldName = 'KR_CR_DOC'
          Position.ColIndex = 2
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 91
        end
        object DBTreeListcxDBTreeListColumn8: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1055#1088#1086#1074#1086#1076#1082#1072' '#1074' '#1082#1088#1077#1076#1080#1090#1110
          DataBinding.FieldName = 'KR_CR_PROV'
          Position.ColIndex = 3
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 90
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = #1043#1086#1083#1086#1074#1085#1080#1081' '#1088#1072#1093#1091#1085#1086#1082' '#1091' '#1082#1088#1077#1076#1080#1090#1110
      ImageIndex = 1
      object KrTreeList: TcxDBTreeList
        Left = 0
        Top = 0
        Width = 649
        Height = 305
        Styles.Background = cxStyle1
        Styles.Content = cxStyle1
        Styles.BandBackground = cxStyle1
        Styles.BandContent = cxStyle1
        Styles.BandHeader = cxStyle2
        Styles.ColumnHeader = cxStyle2
        Align = alClient
        Bands = <
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1056#1072#1093#1091#1085#1082#1080' '#1086#1087#1077#1088#1072#1094#1110#1111
            FixedKind = tlbfLeft
            Width = 207
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1050#1086#1085#1092#1110#1075#1091#1088#1072#1094#1110#1103' '#1086#1087#1077#1088#1072#1094#1110#1111
            Visible = False
            Width = 314
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1050#1086#1088#1077#1089#1087#1086#1085#1076#1091#1102#1095#1110#1081' '#1088#1072#1093#1091#1085#1086#1082
            Visible = False
            Width = 208
          end
          item
            Caption.AlignHorz = taCenter
            Caption.AlignVert = vaCenter
            Caption.MultiLine = True
            Caption.Text = #1052#1077#1084#1086#1088#1110#1072#1083#1100#1085#1080#1081' '#1086#1073#1083#1110#1082
            Width = 364
          end>
        BufferedPaint = False
        DataController.DataSource = KrDataSource
        DataController.ParentField = 'linkto'
        DataController.KeyField = 'id_oper'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        OptionsData.CancelOnExit = False
        OptionsData.Editing = False
        OptionsData.Deleting = False
        OptionsData.CheckHasChildren = False
        OptionsSelection.CellSelect = False
        OptionsView.Bands = True
        OptionsView.ColumnAutoWidth = True
        OptionsView.GridLines = tlglBoth
        OptionsView.ShowRoot = False
        OptionsView.TreeLineStyle = tllsSolid
        ParentFont = False
        RootValue = -1
        TabOrder = 0
        object cxDBTreeListColumn1: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1050#1088#1077#1076#1080#1090
          DataBinding.FieldName = 'ROOT_TEXT'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 0
          SortOrder = soAscending
          Width = 120
        end
        object cxDBTreeListColumn2: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1077#1073#1077#1090
          DataBinding.FieldName = 'ROOT_TEXT_EX'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 0
          SortOrder = soAscending
          Width = 114
        end
        object cxDBTreeListColumn3: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1050#1086#1076' '#1088#1077#1075#1110#1089#1090#1088#1072
          DataBinding.FieldName = 'DB_REG_UCH_KOD'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 2
          Width = 85
        end
        object cxDBTreeListColumn4: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1053#1072#1081#1084#1077#1085#1091#1074#1072#1085#1085#1103' '#1088#1077#1075#1110#1089#1090#1088#1072
          DataBinding.FieldName = 'DB_REG_UCH_TITLE'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 2
          Width = 123
        end
        object cxDBTreeListColumn5: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1054#1073#1086#1088#1086#1090#1080' '#1088#1072#1093#1091#1102#1090#1100#1089#1103' '#1076#1086' '#1084#1077#1084#1086#1088#1110#1072#1083#1100#1085#1086#1075#1086' '#1086#1088#1076#1077#1088#1072
          DataBinding.FieldName = 'MAIN_MO_KOD'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          Width = 411
        end
        object cxDBTreeListColumn6: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1052'.'#1086#1088#1076#1077#1088' '#1074' '#1076#1077#1073#1077#1090#1110
          DataBinding.FieldName = 'DB_ID_MO_KOD'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          Visible = False
          Width = 84
        end
        object cxDBTreeListColumn7: TcxDBTreeListColumn
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Properties.Alignment.Vert = taVCenter
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1052'.'#1086#1088#1076#1077#1088' '#1074' '#1082#1088#1077#1076#1080#1090#1110
          DataBinding.FieldName = 'KR_ID_MO_KOD'
          Position.ColIndex = 2
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 3
          Visible = False
          Width = 105
        end
        object cxDBTreeListColumn8: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1086#1082#1091#1084#1077#1085#1090' '#1074' '#1076#1077#1073#1077#1090#1110
          DataBinding.FieldName = 'DB_CR_DOC'
          Position.ColIndex = 0
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 90
        end
        object cxDBTreeListColumn9: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1055#1088#1086#1074#1086#1076#1082#1072' '#1074' '#1076#1077#1073#1077#1090#1110
          DataBinding.FieldName = 'DB_CR_PROV'
          Position.ColIndex = 1
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 95
        end
        object cxDBTreeListColumn10: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1044#1086#1082#1091#1084#1077#1085#1090' '#1074' '#1082#1088#1077#1076#1080#1090#1110
          DataBinding.FieldName = 'KR_CR_DOC'
          Position.ColIndex = 2
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 91
        end
        object cxDBTreeListColumn11: TcxDBTreeListColumn
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.DisplayUnchecked = 'False'
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
          Caption.AlignHorz = taCenter
          Caption.AlignVert = vaCenter
          Caption.MultiLine = True
          Caption.Text = #1055#1088#1086#1074#1086#1076#1082#1072' '#1074' '#1082#1088#1077#1076#1080#1090#1110
          DataBinding.FieldName = 'KR_CR_PROV'
          Position.ColIndex = 3
          Position.LineCount = 2
          Position.RowIndex = 0
          Position.BandIndex = 1
          Width = 90
        end
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 81
    Width = 649
    Height = 33
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 3
    object cxCheckBox1: TcxCheckBox
      Left = 0
      Top = 1
      Width = 185
      Height = 24
      ParentFont = False
      Properties.DisplayUnchecked = 'False'
      Properties.OnChange = cxCheckBox1PropertiesChange
      Properties.Caption = #1053#1072#1089#1090#1088#1086#1081#1082#1080' '#1086#1087#1077#1088#1072#1094#1110#1111
      TabOrder = 0
    end
  end
  object dxStatusBar1: TdxStatusBar
    Left = 0
    Top = 442
    Width = 649
    Height = 20
    Panels = <
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'Ins  -  '#1044#1086#1076#1072#1090#1080
        Width = 100
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'Del  -  '#1042#1080#1076#1072#1083#1080#1090#1080
        Width = 100
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'F2  -  '#1047#1084#1110#1085#1080#1090#1080
        Width = 100
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'F5  -  '#1054#1085#1086#1074#1080#1090#1080
        Width = 100
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'Alt+Enter  -  '#1056#1086#1079#1075#1086#1088#1085#1091#1090#1080
        Width = 150
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Text = 'Esc  -  '#1042#1080#1093#1110#1076
        Width = 100
      end>
    PaintStyle = stpsOffice11
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
  end
  object WriteTransaction: TpFIBTransaction
    DefaultDatabase = WorkDatabase
    TimeoutAction = TARollback
    Left = 40
    Top = 336
  end
  object ReadTransaction: TpFIBTransaction
    DefaultDatabase = WorkDatabase
    TimeoutAction = TARollback
    Left = 72
    Top = 336
  end
  object OperPopup: TPopupMenu
    Images = SmallImages
    Left = 8
    Top = 240
    object N1: TMenuItem
      Caption = #1053#1086#1074#1072#1103' '#1086#1087#1077#1088#1072#1094#1080#1103
      ImageIndex = 1
      ShortCut = 45
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #1059#1076#1072#1083#1080#1090#1100' '#1086#1087#1077#1088#1072#1094#1080#1102
      ImageIndex = 2
      ShortCut = 46
      OnClick = N2Click
    end
    object N3: TMenuItem
      Caption = #1056#1077#1076#1072#1082#1090#1080#1088#1086#1074#1072#1090#1100' '#1086#1087#1077#1088#1072#1094#1080#1102
      ImageIndex = 5
      ShortCut = 113
      OnClick = N3Click
    end
    object N9: TMenuItem
      Caption = '-'
    end
    object N4: TMenuItem
      Caption = #1057#1074#1086#1081#1089#1090#1074#1072' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1080#1080
      ImageIndex = 14
      ShortCut = 16464
      OnClick = N4Click
    end
    object N10: TMenuItem
      Caption = '-'
    end
    object N5: TMenuItem
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100' '#1080#1085#1092#1086#1088#1084#1072#1094#1080#1102' '#1089' '#1089#1077#1088#1074#1077#1088#1072
      ImageIndex = 10
      ShortCut = 116
      OnClick = N5Click
    end
    object N6: TMenuItem
      Caption = #1055#1088#1086#1089#1084#1086#1090#1088' '#1080#1085#1092'. '#1087#1086' '#1086#1087#1077#1088#1072#1094#1080#1080
      ImageIndex = 13
      ShortCut = 16470
    end
    object N11: TMenuItem
      Caption = '-'
    end
    object N7: TMenuItem
      Caption = #1055#1086#1084#1086#1097#1100
      ImageIndex = 8
      ShortCut = 112
    end
    object N8: TMenuItem
      Caption = #1042#1099#1093#1086#1076
      ImageIndex = 6
      ShortCut = 32883
      OnClick = N8Click
    end
  end
  object SmallImages: TImageList
    Left = 8
    Top = 48
    Bitmap = {
      
      000000000000}
  end
  object WorkDatabase: TpFIBDatabase
    DBParams.Strings = (
      'lc_ctype=WIN1251')
    DefaultTransaction = ReadTransaction
    SQLDialect = 3
    Timeout = 0
    WaitForRestoreConnect = 0
    Left = 9
    Top = 334
  end
  object ActionList1: TActionList
    Left = 41
    Top = 48
    object Action1: TAction
      Caption = 'Action1'
      ShortCut = 32781
      OnExecute = Action1Execute
    end
    object Ins: TAction
      Caption = 'Ins'
      ShortCut = 45
      OnExecute = InsExecute
    end
    object Del: TAction
      Caption = 'Del'
      ShortCut = 46
      OnExecute = DelExecute
    end
    object Change: TAction
      Caption = 'Change'
      ShortCut = 113
      OnExecute = ChangeExecute
    end
    object Update: TAction
      Caption = 'Update'
      ShortCut = 116
      OnExecute = UpdateExecute
    end
    object Esc: TAction
      Caption = 'Esc'
      ShortCut = 27
      OnExecute = EscExecute
    end
  end
  object FormStorage1: TFormStorage
    Options = [fpState]
    UseRegistry = True
    StoredValues = <>
    Left = 8
    Top = 272
  end
  object cxStyleRepository1: TcxStyleRepository
    Left = 8
    Top = 296
    object cxStyle1: TcxStyle
      AssignedValues = [svColor]
      Color = clInfoBk
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor]
      Color = 4707838
    end
  end
  object DbDataSet: TpFIBDataSet
    Database = WorkDatabase
    Transaction = ReadTransaction
    Left = 9
    Top = 145
    poSQLINT64ToBCD = True
  end
  object KrDataSet: TpFIBDataSet
    Database = WorkDatabase
    Transaction = ReadTransaction
    Left = 40
    Top = 145
    poSQLINT64ToBCD = True
  end
  object DbDataSource: TDataSource
    DataSet = DbDataSet
    Left = 18
    Top = 177
  end
  object KrDataSource: TDataSource
    DataSet = KrDataSet
    Left = 58
    Top = 175
  end
  object DelPopupMenu: TPopupMenu
    Images = SmallImages
    Left = 116
    Top = 48
    object N12: TMenuItem
      Caption = #1042#1080#1076#1072#1083#1080#1090#1080
      ImageIndex = 2
      OnClick = N12Click
    end
    object N13: TMenuItem
      Caption = #1042#1080#1076#1072#1083#1080#1090#1080' '#1074#1089#1110
      ImageIndex = 2
      OnClick = N13Click
    end
  end
end
