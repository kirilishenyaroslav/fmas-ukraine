...
uses DB, uCommonDB, StdCtrls, uMovingTable, TableCentral, Controls, SysUtils,
    DateUtils, ProcessUnit, uAsupDataCheckMain;
...
begin
    ...
    begin
        ...
        begin
            Result := 'різні кількість годин у день ' + IntToStr(i);
            Exit;
        end;
        ...
        begin
            Result := 'різні кількість годин за графіком у день ' + IntToStr(i);
            Exit;
        end;

        if table1[i].Hours.NHours <> table2[i].Hours.NHours then
        begin
            Result := 'різні кількість нічних годин у день ' + IntToStr(i);
            Exit;
        end;

        if table1[i].Hours.PHours <> table2[i].Hours.PHours then
        begin
            Result := 'різні кількість святкових годин у день ' + IntToStr(i);
            Exit;
        end;

        if table1[i].Hours.SHours <> table2[i].Hours.SHours then
        begin
            Result := 'різні кількість понаднормових годин у день ' + IntToStr(i);
            Exit;
        end;

        if table1[i].Hours.VHours <> table2[i].Hours.VHours then
        begin
            Result := 'різні кількість годин роботи у вихідний день ' + IntToStr(i);
            Exit;
        end;
...
begin
    ...
    people := Curr_DB.QueryData('SELECT * FROM Get_Cur_Moving_Period(:Date_Beg,:Date_End)',
            'Date_Beg, Date_End');
