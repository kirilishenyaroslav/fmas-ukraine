object PeopleViewForm: TPeopleViewForm
  Left = 141
  Top = 133
  Width = 599
  Height = 494
  Caption = #1044#1086#1074#1110#1076#1085#1080#1082': '#1060#1110#1079#1080#1095#1085#1110' '#1086#1089#1086#1073#1080
  Color = clBtnFace
  Constraints.MinWidth = 498
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label3: TLabel
    Left = 304
    Top = 0
    Width = 5
    Height = 18
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Verdana'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PeopleGrid: TDBGrid
    Left = 0
    Top = 55
    Width = 591
    Height = 370
    Align = alClient
    Color = clInfoBk
    Constraints.MinHeight = 100
    DataSource = PeopleSource
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = [fsBold]
    OnCellClick = PeopleGridCellClick
    OnDblClick = PeopleGridDblClick
    OnKeyDown = PeopleGridKeyDown
    OnKeyPress = PeopleGridKeyPress
    Columns = <
      item
        Expanded = False
        FieldName = 'FIO'
        Width = 573
        Visible = True
      end>
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 591
    Height = 2
    AutoSize = True
    ButtonHeight = 57
    Caption = 'ToolBar1'
    TabOrder = 1
    object ToolButton1: TToolButton
      Left = 0
      Top = 2
      Width = 93
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 2
    Width = 591
    Height = 53
    Align = alTop
    TabOrder = 2
    object CancelButton: TSpeedButton
      Left = 529
      Top = 0
      Width = 60
      Height = 50
      Caption = #1047#1072#1082#1088#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = CancelButtonClick
    end
    object OkButton: TSpeedButton
      Left = 460
      Top = 0
      Width = 60
      Height = 50
      Caption = #1043#1072#1088#1072#1079#1076
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = OkButtonClick
    end
    object FindButton: TSpeedButton
      Left = 195
      Top = 0
      Width = 60
      Height = 50
      Caption = #1047#1085#1072#1081#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        A4A0A0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = FindButtonClick
    end
    object DetailsButton: TSpeedButton
      Left = 130
      Top = 0
      Width = 60
      Height = 50
      Caption = #1044#1086#1082#1083#1072#1076#1085#1086
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = DetailsButtonClick
    end
    object ModifyButton: TSpeedButton
      Left = 65
      Top = 0
      Width = 60
      Height = 50
      Caption = #1047#1084#1110#1085#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFF868686777777FFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = ModifyButtonClick
    end
    object AddButton: TSpeedButton
      Left = 0
      Top = 0
      Width = 60
      Height = 50
      Caption = #1044#1086#1076#1072#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = AddButtonClick
    end
    object DieButton: TSpeedButton
      Left = 344
      Top = 0
      Width = 57
      Height = 49
      Hint = #1042#1080#1083#1091#1095#1080#1090#1080'  '#1092#1110#1079'. '#1086#1089#1086#1073#1091
      Flat = True
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      OnClick = DieClick
    end
    object RefreshButton: TSpeedButton
      Left = 261
      Top = 0
      Width = 60
      Height = 50
      Caption = #1054#1085#1086#1074#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        B2B2B2FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = RefreshButtonClick
    end
  end
  object FindPanel: TPanel
    Left = 0
    Top = 425
    Width = 591
    Height = 35
    Align = alBottom
    TabOrder = 3
    object SearchLabel: TLabel
      Left = 16
      Top = 8
      Width = 48
      Height = 13
      Caption = #1064#1091#1082#1072#1090#1080':'
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object SearchEdit: TEdit
      Left = 80
      Top = 4
      Width = 161
      Height = 21
      Hint = '195.184.198.137'
      TabOrder = 0
      OnChange = SearchEditChange
    end
  end
  object ResultQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Id_Man, Familia, Imya, Otchestvo,Id_Sex ,'
      'Familia || '#39' '#39' || Imya || '#39' '#39' || Otchestvo AS FIO, Id_Server'
      'FROM People'
      'ORDER BY Familia, Imya'
      '')
    Left = 24
    Top = 384
    object ResultQueryID_MAN: TIntegerField
      FieldName = 'ID_MAN'
      Required = True
    end
    object ResultQueryFAMILIA: TIBStringField
      DisplayLabel = #1055#1088#1110#1079#1074#1080#1097#1077
      FieldName = 'FAMILIA'
      Required = True
    end
    object ResultQueryIMYA: TIBStringField
      DisplayLabel = #1030#1084#39#1103
      FieldName = 'IMYA'
      Required = True
    end
    object ResultQueryOTCHESTVO: TIBStringField
      DisplayLabel = #1055#1086' '#1073#1072#1090#1100#1082#1086#1074#1110
      FieldName = 'OTCHESTVO'
      Required = True
    end
    object ResultQueryFIO: TIBStringField
      DisplayLabel = #1055#1088#1110#1079#1074#1080#1097#1077', '#1110#1084#39#1103' '#1090#1072' '#1087#1086' '#1073#1072#1090#1100#1082#1086#1074#1110
      FieldName = 'FIO'
      Size = 62
    end
    object ResultQueryID_SEX: TIntegerField
      FieldName = 'ID_SEX'
      Required = True
    end
    object ResultQueryID_SERVER: TIntegerField
      FieldName = 'ID_SERVER'
      Origin = 'PEOPLE.ID_SERVER'
    end
  end
  object PeopleSource: TDataSource
    DataSet = ResultQuery
    Left = 64
    Top = 384
  end
  object DeleteManQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'EXECUTE PROCEDURE Util_Man_Del(:Id_Man)')
    Left = 424
    Top = 26
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_Man'
        ParamType = ptUnknown
      end>
  end
  object SysOptQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT ID_SERVER FROM SYS_OPTIONS')
    Left = 136
    Top = 384
    object SysOptQueryID_SERVER: TIntegerField
      FieldName = 'ID_SERVER'
      Origin = 'SYS_OPTIONS.ID_SERVER'
    end
  end
end
