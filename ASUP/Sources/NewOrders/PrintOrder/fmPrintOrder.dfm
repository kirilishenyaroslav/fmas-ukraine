object PrintOrderForm: TPrintOrderForm
  Left = 341
  Top = 156
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = #1044#1088#1091#1082' '#1085#1072#1082#1072#1079#1091
  ClientHeight = 70
  ClientWidth = 464
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object OkButton: TBitBtn
    Left = 200
    Top = 36
    Width = 129
    Height = 27
    Action = OkAction
    Caption = #1044#1088#1091#1082' '#1085#1072#1082#1072#1079#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 0
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 344
    Top = 36
    Width = 113
    Height = 27
    Action = CancelAction
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object BitBtn1: TBitBtn
    Left = 8
    Top = 36
    Width = 177
    Height = 27
    Action = VipisPrintAction
    Caption = #1044#1088#1091#1082' '#1074#1080#1090#1103#1075#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 2
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object FontSize: TqFIntControl
    Left = 8
    Top = 8
    Width = 200
    Height = 21
    DisplayName = #1056#1086#1079#1084#1110#1088' '#1096#1088#1080#1092#1090#1091
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
    MaxLength = 10
    NegativeAllowed = False
    ZeroAllowed = False
  end
  object LocalDatabase: TpFIBDatabase
    DBName = 'server1212:fmas'
    DBParams.Strings = (
      'user_name=SYSDBA'
      'password=masterkey'
      'sql_role_name=')
    DefaultTransaction = LocalReadTransaction
    SQLDialect = 3
    Timeout = 0
    DesignDBOptions = []
    WaitForRestoreConnect = 0
    Left = 363
    Top = 24
  end
  object LocalReadTransaction: TpFIBTransaction
    DefaultDatabase = LocalDatabase
    TimeoutAction = TARollback
    Left = 395
    Top = 24
  end
  object LocalWriteTransaction: TpFIBTransaction
    DefaultDatabase = LocalDatabase
    TimeoutAction = TARollback
    Left = 427
    Top = 24
  end
  object ReportSummary: TpFIBDataSet
    Database = LocalDatabase
    Transaction = LocalReadTransaction
    SelectSQL.Strings = (
      'select *'
      'from ASUP_ORDER_PRINT_SUMMARY(:ID_ORDER_GROUP)')
    Left = 392
    Top = 52
    poSQLINT64ToBCD = True
  end
  object ReportHeader: TpFIBDataSet
    Database = LocalDatabase
    Transaction = LocalReadTransaction
    SelectSQL.Strings = (
      'SELECT * FROM ASUP_ORDER_PRINT_HEADER(:Id_Order_Group)')
    Left = 360
    Top = 52
    poSQLINT64ToBCD = True
    object ReportHeaderFIRM_NAME: TFIBStringField
      FieldName = 'FIRM_NAME'
      Size = 255
      EmptyStrToNull = True
    end
    object ReportHeaderORDER_DATE: TFIBDateField
      FieldName = 'ORDER_DATE'
      DisplayFormat = 'dd.mm.yyyy'
    end
    object ReportHeaderCITY: TFIBStringField
      FieldName = 'CITY'
      Size = 255
      EmptyStrToNull = True
    end
    object ReportHeaderNUM_ORDER: TFIBStringField
      FieldName = 'NUM_ORDER'
      Size = 255
      EmptyStrToNull = True
    end
    object ReportHeaderPRINT_NAME: TFIBStringField
      FieldName = 'PRINT_NAME'
      Size = 255
      EmptyStrToNull = True
    end
    object ReportHeaderISPOLNITEL: TFIBStringField
      FieldName = 'ISPOLNITEL'
      Size = 255
      EmptyStrToNull = True
    end
    object ReportHeaderNUM_AND_DATE: TFIBStringField
      FieldName = 'NUM_AND_DATE'
      Size = 512
      EmptyStrToNull = True
    end
    object ReportHeaderDATE_ORDER_CANC: TFIBStringField
      FieldName = 'DATE_ORDER_CANC'
      Size = 255
      EmptyStrToNull = True
    end
  end
  object KeyList: TActionList
    Left = 332
    Top = 31
    object OkAction: TAction
      Caption = #1043#1072#1088#1072#1079#1076
      ShortCut = 13
      SecondaryShortCuts.Strings = (
        'F10')
      OnExecute = OkActionExecute
    end
    object CancelAction: TAction
      Caption = #1042#1110#1076#1084#1110#1085#1072
      ShortCut = 27
      OnExecute = CancelActionExecute
    end
    object VipisPrintAction: TAction
      Caption = #1044#1088#1091#1082' '#1074#1080#1087#1080#1089#1082#1080
      OnExecute = VipisPrintActionExecute
    end
  end
  object Report: TfrxReport
    Version = '3.15'
    DotMatrixReport = False
    EngineOptions.DoublePass = True
    EngineOptions.MaxMemSize = 10000000
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = #1047#1072' '#1079#1072#1084#1086#1074#1095#1091#1074#1072#1085#1085#1103#1084
    ReportOptions.CreateDate = 38539.730101805600000000
    ReportOptions.LastChange = 38757.736513715280000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'uses '#39'Reports\GoodPageBreak.pas'#39';'
      ''
      'procedure ReportTitle1OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  pfInit;'
      'end;'
      ''
      'procedure MasterData1OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  pfMasterCheck(Sender);'
      'end;'
      ''
      'procedure MasterData1OnAfterCalcHeight(Sender: TfrxComponent);'
      'begin'
      '  pfSaveMasterDataHeight(Sender);'
      'end;'
      ''
      'procedure MasterData2OnAfterCalcHeight(Sender: TfrxComponent);'
      'begin'
      '  pfSaveSummaryHeight(Sender);'
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 16
    Top = 32
    Datasets = <
      item
        DataSet = FooterDataset
        DataSetName = 'FooterDataset'
      end
      item
        DataSet = HeaderDataset
        DataSetName = 'HeaderDataset'
      end
      item
        DataSet = SummuryDataset
        DataSetName = 'SummaryDataset'
      end>
    Variables = <>
    Style = <>
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 30.000000000000000000
      RightMargin = 15.000000000000000000
      TopMargin = 20.000000000000000000
      BottomMargin = 20.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Times New Roman'
        Font.Style = []
        Height = 144.010513330000000000
        ParentFont = False
        Top = 18.897650000000000000
        Width = 623.622450000000000000
        OnBeforePrint = 'ReportTitle1OnBeforePrint'
        Stretched = True
        object Memo2: TfrxMemoView
          Left = 277.811225000000000000
          Top = 49.769016670000000000
          Width = 94.488250000000000000
          Height = 22.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Times New Roman'
          Font.Style = [fsBold]
          Memo.Strings = (
            #1053' '#1040' '#1050' '#1040' '#1047)
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Left = 3.333333330000000000
          Top = 76.435683330000000000
          Width = 209.866666670000000000
          Height = 22.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Times New Roman'
          Font.Style = [fsBold]
          Memo.Strings = (
            '"___" _________ 200__'#1088'.')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 436.666666660000000000
          Top = 76.561679790000000000
          Width = 166.512306670000000000
          Height = 22.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Times New Roman'
          Font.Style = [fsBold]
          Memo.Strings = (
            #8470'________________')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 218.677180000000000000
          Top = 76.561679790000000000
          Width = 207.630426670000000000
          Height = 22.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Times New Roman'
          Font.Style = []
          HAlign = haCenter
          Memo.Strings = (
            '[HeaderDataset."CITY"]')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 5.333333330000000000
          Top = 119.769016670000000000
          Width = 227.154916670000000000
          Height = 20.897650000000000000
          StretchMode = smActualHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Times New Roman'
          Font.Style = []
          Memo.Strings = (
            '[HeaderDataset."PRINT_NAME"]')
          ParentFont = False
        end
        object Memo1: TfrxMemoView
          Left = 0.666666660000000000
          Top = 1.769016659999998000
          Width = 622.446196670000000000
          Height = 46.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Times New Roman'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.Strings = (
            '[HeaderDataset."FIRM_NAME"]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Times New Roman'
        Font.Style = []
        Height = 23.905380000000000000
        ParentFont = False
        Top = 272.126160000000000000
        Width = 623.622450000000000000
        OnAfterCalcHeight = 'MasterData1OnAfterCalcHeight'
        OnBeforePrint = 'MasterData1OnBeforePrint'
        DataSet = SummuryDataset
        DataSetName = 'SummaryDataset'
        RowCount = 0
        Stretched = True
        object Memo7: TfrxMemoView
          Left = 2.000000000000000000
          Top = 3.007729999999981000
          Width = 623.112863330000000000
          Height = 18.897650000000000000
          StretchMode = smActualHeight
          AllowHTMLTags = True
          DataSet = SummuryDataset
          DataSetName = 'SummaryDataset'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          HAlign = haBlock
          LineSpacing = 8.000000000000000000
          Memo.Strings = (
            '[SummaryDataset."COLUMN1"]')
          ParagraphGap = 1.250000000000000000
          ParentFont = False
        end
      end
      object Header1: TfrxHeader
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Times New Roman'
        Font.Style = []
        Height = 28.010513330000000000
        ParentFont = False
        Top = 222.992270000000000000
        Width = 623.622450000000000000
      end
      object MasterData2: TfrxMasterData
        Height = 22.677180000000000000
        Top = 317.480520000000000000
        Width = 623.622450000000000000
        OnAfterCalcHeight = 'MasterData2OnAfterCalcHeight'
        DataSet = FooterDataset
        DataSetName = 'FooterDataset'
        RowCount = 0
        Stretched = True
        object Memo8: TfrxMemoView
          Left = 1.889763780000000000
          Top = 2.755719999999996000
          Width = 325.952755910000000000
          Height = 18.897650000000000000
          StretchMode = smMaxHeight
          DataField = 'COLUMN1'
          DataSet = FooterDataset
          DataSetName = 'FooterDataset'
          Memo.Strings = (
            '[FooterDataset."COLUMN1"]')
          WordBreak = True
        end
        object Memo9: TfrxMemoView
          Left = 330.779530000000000000
          Top = 2.755719999999996000
          Width = 292.834635910000000000
          Height = 18.897650000000000000
          StretchMode = smMaxHeight
          DataField = 'COLUMN2'
          DataSet = FooterDataset
          DataSetName = 'FooterDataset'
          Memo.Strings = (
            '[FooterDataset."COLUMN2"]')
          WordBreak = True
        end
      end
    end
  end
  object ReportFooter: TpFIBDataSet
    Database = LocalDatabase
    Transaction = LocalReadTransaction
    SelectSQL.Strings = (
      'select * '
      'from ASUP_ORDER_PRINT_FOOTER(:id_order_group)')
    Left = 424
    Top = 52
    poSQLINT64ToBCD = True
    object ReportFooterCOLUMN1: TFIBStringField
      FieldName = 'COLUMN1'
      Size = 4096
      EmptyStrToNull = True
    end
    object ReportFooterCOLUMN2: TFIBStringField
      FieldName = 'COLUMN2'
      Size = 4096
      EmptyStrToNull = True
    end
  end
  object SummuryDataset: TfrxDBDataset
    UserName = 'SummaryDataset'
    CloseDataSource = False
    DataSet = ReportSummary
    Left = 48
    Top = 16
  end
  object HeaderDataset: TfrxDBDataset
    UserName = 'HeaderDataset'
    CloseDataSource = False
    DataSet = ReportHeader
    Left = 80
    Top = 32
  end
  object FooterDataset: TfrxDBDataset
    UserName = 'FooterDataset'
    CloseDataSource = False
    DataSet = ReportFooter
    Left = 48
    Top = 46
  end
  object RTFExport: TfrxRTFExport
    ExportNotPrintable = True
    UseFileCache = True
    OpenAfterExport = True
    ShowProgress = True
    Wysiwyg = True
    Creator = 'FastReport http://www.fast-report.com'
    Left = 112
    Top = 48
  end
  object frxXLSExport1: TfrxXLSExport
    UseFileCache = True
    ShowProgress = True
    AsText = False
    Background = True
    FastExport = True
    PageBreaks = True
    Left = 104
    Top = 24
  end
  object frxHTMLExport1: TfrxHTMLExport
    UseFileCache = True
    FixedWidth = True
    Background = False
    Left = 144
    Top = 32
  end
end
