...
uses
  ...
  uSimpleCheck, qFTools, uDismissSovmestOrderForm, uShtatUtils,
  dmDismissSovmestOrder;
...
begin
    ...
    DM.LocalDatabase.Handle := TISC_DB_HANDLE(hnd);
        ...
        // Form prepare
    Form := TfmMassDismissOrder.Create(Application.MainForm);
...
//    DM.RxMD_Dismissed.CopyStructure(Dm.SelectDismissedInfo);

    Printed:=False;

    Dm.SelectDismissedInfo.Close;
    Dm.SelectDismissedInfo.ParamByName('ID_ORDER').AsInteger := DM.IDOrder;
    Dm.SelectDismissedInfo.Open;

    Form.fmSearch.Prepare(DM.RxMD_Dismissed);
    Form.ShowModal;
