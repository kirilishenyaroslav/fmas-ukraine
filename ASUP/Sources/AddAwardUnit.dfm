object AddAwardForm: TAddAwardForm
  Left = 263
  Top = 180
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1044#1086#1076#1072#1090#1080' '#1079#1072#1086#1093#1086#1095#1077#1085#1085#1103
  ClientHeight = 124
  ClientWidth = 417
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDefault
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object WarPostLabel: TLabel
    Left = 16
    Top = 16
    Width = 74
    Height = 13
    Caption = #1047#1072#1086#1093#1086#1095#1077#1085#1085#1103':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object WarSostavLabel: TLabel
    Left = 16
    Top = 56
    Width = 99
    Height = 13
    Caption = #1058#1080#1087' '#1079#1072#1086#1093#1086#1095#1077#1085#1085#1103':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PrivFieldControl: TFieldControl
    Left = 392
    Top = 16
    Width = 18
    Height = 18
    Control = PrivEdit
    ReadFromField = PrivInfoQueryNAME_AWARD
    Title = '"'#1047#1072#1086#1093#1086#1095#1077#1085#1085#1103' "'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object PrivTypeFieldControl: TFieldControl
    Left = 392
    Top = 56
    Width = 17
    Height = 18
    Control = PrivTypeBox
    ReadFromField = PrivInfoQueryID_AWARD_TYPE
    Title = '"'#1058#1080#1087' '#1079#1072#1086#1093#1086#1095#1077#1085#1085#1103'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object OkButton: TBitBtn
    Left = 118
    Top = 88
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 2
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 254
    Top = 88
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 3
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object PrivTypeBox: TSpComboBox
    Left = 144
    Top = 56
    Width = 241
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    ItemCount = 5
    SpOpenItem = '* '#1044#1086#1074#1110#1076#1085#1080#1082' ...'
    AutoDrop = True
    SpParams.Table = 'Sp_Award_Type'
    SpParams.IdField = 'Id_Award_Type'
    SpParams.SpFields = 'Name_Award_Type'
    SpParams.Title = #1058#1080#1087#1080' '#1079#1072#1086#1093#1086#1095#1077#1085#1100
    SpParams.ComboField = 'Name_Award_Type'
    SpParams.ColumnNames = #1058#1080#1087#1080' '#1079#1072#1086#1093#1086#1095#1077#1085#1100
    SpParams.InsertProcName = 'Sp_Award_Type_Insert'
    SpParams.UpdateProcName = 'Sp_Award_type_Update'
    SpParams.DeleteProcName = 'Sp_Award_type_delete'
    SpParams.DontHideFirstColumn = False
    SpParams.SpMode = [spfAdd, spfModify, spfDelete, spfFind, spfSelect, spfExt]
  end
  object PrivEdit: TEdit
    Left = 144
    Top = 16
    Width = 241
    Height = 21
    MaxLength = 50
    TabOrder = 0
  end
  object PrivInfoQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Name_Award, Id_Award_Type'
      'FROM Sp_Award'
      'WHERE Id_Award = :Id_Award')
    Left = 16
    Top = 80
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_Award'
        ParamType = ptUnknown
      end>
    object PrivInfoQueryNAME_AWARD: TIBStringField
      FieldName = 'NAME_AWARD'
      Size = 50
    end
    object PrivInfoQueryID_AWARD_TYPE: TIntegerField
      FieldName = 'ID_AWARD_TYPE'
      Required = True
    end
  end
end
