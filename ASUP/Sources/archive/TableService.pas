...
begin
    ...
    begin
            // подготовить табель
        movTable.Prepare(PeopleQuery['Id_Man_Moving'], Month_Beg, Month_End,
            PeopleQuery['Kol_Stavok']);

            // сформировать только график
        movTable.FormTable(True);

            // обновить график
        res := movTable.UpdateGraphic;
...
begin
    ...
    begin
        try
                // подготовить табеля
            movTable.Prepare(PeopleQuery['Id_Man_Moving'],
                Month_Beg, Month_End, PeopleQuery['Kol_Stavok']);
            movTable2.Prepare(PeopleQuery['Id_Man_Moving'],
                Month_Beg, Month_End, PeopleQuery['Kol_Stavok']);
