object DepartmentsDetailsForm: TDepartmentsDetailsForm
  Left = 257
  Top = 117
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsDialog
  Caption = #1055#1110#1076#1088#1086#1079#1076#1110#1083
  ClientHeight = 378
  ClientWidth = 421
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object NameFullLabel: TLabel
    Left = 16
    Top = 64
    Width = 81
    Height = 13
    Caption = #1055#1086#1074#1085#1072' '#1085#1072#1079#1074#1072':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object NameShortLabel: TLabel
    Left = 16
    Top = 104
    Width = 107
    Height = 13
    Caption = #1057#1082#1086#1088#1086#1095#1077#1085#1072' '#1085#1072#1079#1074#1072':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object TypeLabel: TLabel
    Left = 136
    Top = 24
    Width = 92
    Height = 13
    Caption = #1058#1080#1087' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object FC_NameFull: TFieldControl
    Left = 400
    Top = 80
    Width = 18
    Height = 18
    Control = NameFullEdit
    ReadFromField = DetailsQueryNAME_FULL
    Title = '"'#1055#1086#1074#1085#1072' '#1085#1072#1079#1074#1072'"'
    MaxLength = 255
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object FC_NameShort: TFieldControl
    Left = 400
    Top = 120
    Width = 18
    Height = 18
    Control = NameShortEdit
    ReadFromField = DetailsQueryNAME_SHORT
    Required = False
    NotNull = True
    Title = '"'#1057#1082#1086#1088#1086#1095#1077#1085#1072' '#1085#1072#1079#1074#1072'"'
    MaxLength = 50
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object FC_DepType: TFieldControl
    Left = 400
    Top = 40
    Width = 18
    Height = 18
    Control = DepTypeBox
    ReadFromField = DetailsQueryID_DEP_TYPE
    Title = '"'#1058#1080#1087' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object ParentLabel: TLabel
    Left = 16
    Top = 8
    Width = 69
    Height = 13
    Caption = 'ParentLabel'
  end
  object DateBegLabel: TLabel
    Left = 16
    Top = 152
    Width = 148
    Height = 13
    Caption = #1055#1086#1095#1072#1090#1086#1082' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object DateEndLabel: TLabel
    Left = 264
    Top = 152
    Width = 138
    Height = 13
    Caption = #1050#1110#1085#1077#1094#1100' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object FC_DateBeg: TFieldControl
    Left = 152
    Top = 168
    Width = 18
    Height = 18
    Control = DateBegPicker
    ReadFromField = DetailsQueryDATE_BEG
    Title = '"'#1055#1086#1095#1072#1090#1086#1082' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
    Default = 10959d
  end
  object FC_DateEnd: TFieldControl
    Left = 400
    Top = 168
    Width = 18
    Height = 18
    Control = DateEndPicker
    ReadFromField = DetailsQueryDATE_END
    Required = False
    NotNull = True
    Title = '"'#1050#1110#1085#1077#1094#1100' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object PropertiesLabel: TLabel
    Left = 16
    Top = 208
    Width = 73
    Height = 13
    Caption = #1042#1083#1072#1089#1090#1080#1074#1086#1089#1090#1110
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object CodeLabel: TLabel
    Left = 16
    Top = 24
    Width = 92
    Height = 13
    Caption = #1050#1086#1076' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object FC_Code: TFieldControl
    Left = 112
    Top = 40
    Width = 18
    Height = 18
    Control = CodeEdit
    ReadFromField = DetailsQueryDEPARTMENT_CODE
    Title = '"'#1050#1086#1076' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091'"'
    MaxLength = 10
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object AddButton: TSpeedButton
    Left = 359
    Top = 224
    Width = 41
    Height = 113
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    OnClick = AddButtonClick
  end
  object CancelButton: TBitBtn
    Left = 303
    Top = 347
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 8
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object NameFullEdit: TCheckEdit
    Left = 16
    Top = 80
    Width = 385
    Height = 21
    MaxLength = 255
    TabOrder = 2
    InputSet = isAll
  end
  object NameShortEdit: TCheckEdit
    Left = 16
    Top = 120
    Width = 385
    Height = 21
    MaxLength = 50
    TabOrder = 3
    OnEnter = NameShortEditEnter
    InputSet = isAll
  end
  object DepTypeBox: TSpComboBox
    Left = 136
    Top = 40
    Width = 265
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnEnter = DepTypeBoxEnter
    ItemCount = 5
    SpOpenItem = '* '#1044#1086#1074#1110#1076#1085#1080#1082' ...'
    AutoDrop = True
    SpParams.Table = 'Sp_Dep_Type'
    SpParams.IdField = 'Id_Dep_Type'
    SpParams.SpFields = 'Name_Dep_Type'
    SpParams.Title = #1058#1080#1087#1080' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1110#1074
    SpParams.ComboField = 'Name_Dep_Type'
    SpParams.ColumnNames = #1058#1080#1087' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091
    SpParams.InsertProcName = 'Sp_Dep_Type_Insert'
    SpParams.UpdateProcName = 'Sp_Dep_Type_Update'
    SpParams.DeleteProcName = 'Sp_Dep_Type_Delete'
    SpParams.DontHideFirstColumn = False
    SpParams.SpMode = [spfAdd, spfModify, spfDelete, spfFind, spfSelect]
  end
  object DateBegPicker: TDateTimePicker
    Left = 16
    Top = 168
    Width = 137
    Height = 21
    Date = 37700.756995590300000000
    Time = 37700.756995590300000000
    ShowCheckbox = True
    Checked = False
    TabOrder = 4
  end
  object DateEndPicker: TDateTimePicker
    Left = 264
    Top = 168
    Width = 137
    Height = 21
    Date = 37700.757137557900000000
    Time = 37700.757137557900000000
    ShowCheckbox = True
    Checked = False
    TabOrder = 5
  end
  object PropListBox: TCheckListBox
    Left = 16
    Top = 224
    Width = 337
    Height = 113
    Flat = False
    ItemHeight = 13
    Sorted = True
    TabOrder = 6
    OnClick = PropListBoxClick
  end
  object OkButton: TBitBtn
    Left = 192
    Top = 347
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 7
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CodeEdit: TCheckEdit
    Left = 16
    Top = 40
    Width = 89
    Height = 21
    MaxLength = 10
    TabOrder = 0
    Text = '---'
    InputSet = isAll
  end
  object DetailsQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Id_Dep_Type,  Department_Code, Name_Full, '
      'Name_Short, Date_Beg, Date_End'
      'FROM Sp_Department'
      'WHERE'
      'Use_End > '#39'NOW'#39
      'AND Id_Department = :Id_Department')
    Left = 32
    Top = 336
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Id_Department'
        ParamType = ptUnknown
      end>
    object DetailsQueryID_DEP_TYPE: TIntegerField
      FieldName = 'ID_DEP_TYPE'
      Required = True
    end
    object DetailsQueryDEPARTMENT_CODE: TIBStringField
      FieldName = 'DEPARTMENT_CODE'
      Size = 10
    end
    object DetailsQueryNAME_FULL: TIBStringField
      FieldName = 'NAME_FULL'
      Required = True
      Size = 255
    end
    object DetailsQueryNAME_SHORT: TIBStringField
      FieldName = 'NAME_SHORT'
      Required = True
      Size = 50
    end
    object DetailsQueryDATE_BEG: TDateField
      FieldName = 'DATE_BEG'
      Required = True
    end
    object DetailsQueryDATE_END: TDateField
      FieldName = 'DATE_END'
      Required = True
    end
  end
  object AllPropQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Id_Property, Name_Property'
      'FROM Sp_Dep_Prop'
      'ORDER BY Name_Property')
    Left = 272
    Top = 192
    object AllPropQueryID_PROPERTY: TIntegerField
      FieldName = 'ID_PROPERTY'
      Required = True
    end
    object AllPropQueryNAME_PROPERTY: TIBStringField
      FieldName = 'NAME_PROPERTY'
      Required = True
      Size = 50
    end
  end
  object PropQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT P.Id_Property, Name_Property'
      'FROM Dep_Properties DP, Sp_Dep_Prop P'
      'WHERE Id_Department = :Id_Department'
      'AND P.Id_Property = DP.Id_Property')
    Left = 176
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Id_Department'
        ParamType = ptUnknown
      end>
    object PropQueryID_PROPERTY: TIntegerField
      FieldName = 'ID_PROPERTY'
      Required = True
    end
    object PropQueryNAME_PROPERTY: TIBStringField
      FieldName = 'NAME_PROPERTY'
      Required = True
      Size = 50
    end
  end
  object ChangeProperty: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    Left = 224
    Top = 192
  end
  object GetIdDepartmentQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT IdDepartment FROM Get_Id_Department')
    Left = 320
    Top = 8
  end
  object BufferQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT First_PBKey, Last_PBKey FROM ASUP_LTD_ONE_DEPARTMENT_EXPO' +
        'RT(:Id_Department)')
    Left = 80
    Top = 336
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_Department'
        ParamType = ptUnknown
      end>
  end
end
