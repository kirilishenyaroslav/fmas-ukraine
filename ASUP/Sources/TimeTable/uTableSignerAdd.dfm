object fmTableSignerAdd: TfmTableSignerAdd
  Left = 383
  Top = 244
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  ClientHeight = 245
  ClientWidth = 458
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object FormControl: TqFFormControl
    Left = 24
    Top = 216
    Width = 60
    Height = 21
    ControlsParent = Panel
    ControlsOwner = Owner
    InsertSQL.Strings = (
      
        'SELECT * FROM Dt_Table_Signers_Insert(:Name_Post, :FIO, :Kind, :' +
        'Id)')
    UpdateSQL.Strings = (
      'EXECUTE PROCEDURE DT_TABLE_SIGNERS_UPDATE :where,'
      ':Name_Post, :FIO, :Kind, :Id')
    SelectSQL.Strings = (
      'SELECT * FROM Dt_Table_Signers_Info(:where)')
    AddCaption = #1044#1086#1076#1072#1090#1080' '#1087#1110#1076#1087#1080#1089
    ModifyCaption = #1047#1084#1110#1085#1080#1090#1080' '#1087#1110#1076#1087#1080#1089
    InfoCaption = #1055#1110#1076#1087#1080#1089' '#1090#1072#1073#1077#1083#1103': '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1103
  end
  object OkButton: TBitBtn
    Left = 110
    Top = 212
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 270
    Top = 212
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 2
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object Panel: TPanel
    Left = 16
    Top = 8
    Width = 425
    Height = 193
    TabOrder = 0
    object FIO: TqFCharControl
      Left = 16
      Top = 24
      Width = 390
      Height = 21
      FieldName = 'FIO'
      DisplayName = #1055#1030#1041
      Interval = 120
      Value = ''
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      MaxLength = 100
    end
    object Name_Post: TqFCharControl
      Left = 16
      Top = 64
      Width = 390
      Height = 21
      FieldName = 'Name_Post'
      DisplayName = #1055#1086#1089#1072#1076#1072
      Interval = 120
      Value = ''
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      MaxLength = 100
    end
    object Kind: TqFEnumControl
      Left = 16
      Top = 104
      Width = 390
      Height = 21
      FieldName = 'Kind'
      DisplayName = #1058#1080#1087' '#1090#1072#1073#1077#1083#1102
      Interval = 120
      Value = 1
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      OnChange = KindChange
      Default = '1'
      Items.Strings = (
        #1059#1089#1110' '#1090#1072#1073#1077#1083#1103
        #1058#1072#1073#1077#1083#1100' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091
        #1058#1072#1073#1077#1083#1100' '#1075#1088#1091#1087#1080' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1110#1074
        #1058#1072#1073#1077#1083#1100' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1091' ('#1079' '#1091#1089#1110#1084#1072' '#1087#1110#1076#1083#1077#1075#1083#1080#1084#1080')')
      ValuesString = '0,1,2,4'
    end
    object Id: TqFSpravControl
      Left = 16
      Top = 144
      Width = 390
      Height = 21
      FieldName = 'Id'
      DisplayName = #1055#1110#1076#1088#1086#1079#1076#1110#1083
      Interval = 120
      Value = Null
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      OnOpenSprav = IdOpenSprav
      DisplayTextField = 'Name_Object'
    end
  end
end
