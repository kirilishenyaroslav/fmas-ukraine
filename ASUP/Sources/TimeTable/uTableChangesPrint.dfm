object frmTableChangesPrint: TfrmTableChangesPrint
  Left = 175
  Top = 162
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1044#1088#1091#1082' '#1074#1110#1076#1086#1084#1086#1089#1090#1110' '#1082#1086#1088#1077#1082#1090#1091#1074#1072#1085#1100' '#1090#1072#1073#1077#1083#1102
  ClientHeight = 247
  ClientWidth = 663
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 16
    Top = 156
    Width = 112
    Height = 13
    Caption = #1044#1088#1091#1082#1091#1074#1072#1090#1080' '#1079#1084#1110#1085#1080' '#1079
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 274
    Top = 156
    Width = 15
    Height = 13
    Caption = #1087#1086
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 16
    Top = 184
    Width = 140
    Height = 13
    Caption = #1058#1080#1087' '#1076#1088#1091#1082#1086#1074#1072#1085#1086#1111' '#1092#1086#1088#1084#1080':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  inline SelectFrame: TfrmTableSelect
    Left = 14
    Top = 33
    Width = 640
    Height = 116
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    inherited YearPanel: TPanel
      Height = 40
      inherited WorkersCaptionLabel: TLabel
        Visible = False
      end
      inherited PeopleLabel: TLabel
        Visible = False
      end
      inherited SignLabel: TLabel
        Visible = False
      end
      inherited MonthBox: TComboBox
        OnChange = SelectFrameMonthBoxChange
      end
      inherited AcceptButton: TBitBtn
        Visible = False
        OnClick = SelectFrameAcceptButtonClick
      end
      inherited TableTypeBox: TComboBox
        Visible = False
      end
    end
  end
  object OkButton: TBitBtn
    Left = 84
    Top = 212
    Width = 119
    Height = 27
    Caption = #1044#1088#1091#1082#1091#1074#1072#1090#1080
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 5
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object ExportButton: TBitBtn
    Left = 272
    Top = 212
    Width = 105
    Height = 27
    Caption = #1045#1082#1089#1087#1086#1088#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 6
    OnClick = ExportButtonClick
    Glyph.Data = {
      
      33333333330A0333333333333330333333333333333033333333}
  end
  object CancelButton: TBitBtn
    Left = 455
    Top = 212
    Width = 97
    Height = 27
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 7
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object DateBegPicker: TDateTimePicker
    Left = 144
    Top = 152
    Width = 113
    Height = 21
    Date = 37747.567911956000000000
    Time = 37747.567911956000000000
    TabOrder = 2
  end
  object DateEndPicker: TDateTimePicker
    Left = 304
    Top = 152
    Width = 113
    Height = 21
    Date = 37747.568449606500000000
    Time = 37747.568449606500000000
    TabOrder = 3
  end
  object AllBox: TCheckBox
    Left = 16
    Top = 8
    Width = 425
    Height = 17
    Caption = #1059#1089#1110' '#1082#1086#1088#1077#1082#1090#1091#1074#1072#1085#1085#1103' '#1085#1077#1079#1072#1083#1077#1078#1085#1086' '#1074#1110#1076' '#1087#1110#1076#1088#1086#1079#1076#1110#1083#1072' '#1090#1072' '#1084#1110#1089#1103#1094#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = AllBoxClick
  end
  object TypeBox: TComboBox
    Left = 184
    Top = 180
    Width = 145
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ItemIndex = 1
    TabOrder = 4
    Text = #1057#1082#1086#1088#1086#1095#1077#1085#1072' ('#1040'4)'
    Items.Strings = (
      #1056#1086#1079#1096#1080#1088#1077#1085#1072' ('#1040'3)'
      #1057#1082#1086#1088#1086#1095#1077#1085#1072' ('#1040'4)')
  end
  object TableReport: TfrReport
    Dataset = MainDataset
    InitialZoom = pzDefault
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit, pbPageSetup]
    RebuildPrinter = False
    OnGetValue = TableReportGetValue
    Left = 224
    Top = 220
    ReportForm = {19000000}
  end
  object MainDataset: TfrDBDataSet
    DataSource = MainSource
    Left = 48
    Top = 220
  end
  object TableDataset: TfrUserDataset
    OnCheckEOF = TableDatasetCheckEOF
    OnFirst = TableDatasetFirst
    OnNext = TableDatasetNext
    OnPrior = TableDatasetPrior
    Left = 256
    Top = 196
  end
  object MainSource: TDataSource
    OnDataChange = MainSourceDataChange
    Left = 16
    Top = 220
  end
end
