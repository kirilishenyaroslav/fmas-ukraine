object ManWorkModeChangeForm: TManWorkModeChangeForm
  Left = 104
  Top = 92
  Width = 696
  Height = 480
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 688
    Height = 57
    Align = alTop
    TabOrder = 0
    object AddButton: TSpeedButton
      Left = 0
      Top = 0
      Width = 65
      Height = 57
      Caption = #1044#1086#1076#1072#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = AddButtonClick
    end
    object ModifyButton: TSpeedButton
      Left = 65
      Top = 0
      Width = 65
      Height = 57
      Caption = #1047#1084#1110#1085#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFF868686777777FFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = ModifyButtonClick
    end
    object DeleteButton: TSpeedButton
      Left = 130
      Top = 0
      Width = 65
      Height = 57
      Caption = #1042#1080#1083#1091#1095#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = DeleteButtonClick
    end
    object OrderItemsButton: TSpeedButton
      Left = 260
      Top = 0
      Width = 102
      Height = 57
      Caption = #1055#1091#1085#1082#1090#1080' '#1085#1072#1082#1072#1079#1091
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = OrderItemsButtonClick
    end
    object ViewButton: TSpeedButton
      Left = 195
      Top = 0
      Width = 65
      Height = 57
      Caption = #1055#1077#1088#1077#1075#1083#1103#1076
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = ViewButtonClick
    end
    object SpeedButton1: TSpeedButton
      Left = 623
      Top = 0
      Width = 65
      Height = 57
      Caption = #1047#1072#1082#1088#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = CancelButtonClick
    end
  end
  object BonusGrid: TDBGrid
    Left = 0
    Top = 57
    Width = 688
    Height = 344
    Align = alClient
    Color = clInfoBk
    Constraints.MinHeight = 128
    DataSource = ResultSource
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 1
    TitleFont.Charset = RUSSIAN_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'TN'
        Title.Caption = #1058#1053
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NUM_ITEM'
        Title.Caption = #1055'.'#1053'.'
        Width = 36
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FIO'
        Title.Caption = #1055'.'#1030'.'#1041'.'
        Width = 104
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DEPARTMENT_NAME'
        Title.Caption = #1055#1110#1076#1088#1086#1079#1076#1110#1083
        Width = 89
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'POST_NAME'
        Title.Caption = #1055#1086#1089#1072#1076#1072
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_BEG'
        Title.Caption = #1044#1072#1090#1072' '#1087#1086#1095#1072#1090#1082#1091
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_END'
        Title.Caption = #1044#1072#1090#1072' '#1082#1110#1085#1094#1103
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'WORK_MODE_NAME'
        Title.Caption = #1043#1088#1072#1092#1110#1082' '#1088#1086#1073#1086#1090#1080
        Width = 115
        Visible = True
      end>
  end
  object Panel3: TPanel
    Left = 0
    Top = 401
    Width = 688
    Height = 45
    Align = alBottom
    TabOrder = 2
    object Label2: TLabel
      Left = 8
      Top = 16
      Width = 43
      Height = 13
      Caption = #1055#1086#1096#1091#1082':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label9: TLabel
      Left = 256
      Top = 16
      Width = 103
      Height = 13
      Caption = '('#1055#1030#1041' '#1072#1073#1086' '#1090#1072#1073'. '#1085#1086#1084#1077#1088')'
    end
    object SearchEdit: TEdit
      Left = 56
      Top = 13
      Width = 193
      Height = 21
      TabOrder = 0
      OnChange = SearchEditChange
    end
  end
  object WriteQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    Left = 624
    Top = 208
  end
  object ResultQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    AfterOpen = ResultQueryAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * FROM DT_WORK_MODE_CHANGE_SELECT(:ID_ORDER)')
    Left = 584
    Top = 353
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID_ORDER'
        ParamType = ptUnknown
      end>
    object ResultQueryID_WORK_MODE_CHANGE: TIntegerField
      FieldName = 'ID_WORK_MODE_CHANGE'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.ID_WORK_MODE_CHANGE'
    end
    object ResultQueryID_MAN_MOVING: TIntegerField
      FieldName = 'ID_MAN_MOVING'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.ID_MAN_MOVING'
    end
    object ResultQueryRMOVING: TIntegerField
      FieldName = 'RMOVING'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.RMOVING'
    end
    object ResultQueryID_WORK_MODE: TIntegerField
      FieldName = 'ID_WORK_MODE'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.ID_WORK_MODE'
    end
    object ResultQuerySHIFT: TIntegerField
      FieldName = 'SHIFT'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.SHIFT'
    end
    object ResultQueryDATE_BEG: TDateField
      FieldName = 'DATE_BEG'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.DATE_BEG'
    end
    object ResultQueryDATE_END: TDateField
      FieldName = 'DATE_END'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.DATE_END'
    end
    object ResultQueryREASON: TIBStringField
      FieldName = 'REASON'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.REASON'
      Size = 255
    end
    object ResultQueryNUM_ITEM: TIntegerField
      FieldName = 'NUM_ITEM'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.NUM_ITEM'
    end
    object ResultQueryIS_ACTIVE: TIntegerField
      FieldName = 'IS_ACTIVE'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.IS_ACTIVE'
    end
    object ResultQueryPIDSTAVA: TIBStringField
      FieldName = 'PIDSTAVA'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.PIDSTAVA'
      Size = 255
    end
    object ResultQueryFIO: TIBStringField
      FieldName = 'FIO'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.FIO'
      Size = 62
    end
    object ResultQueryWHY: TIBStringField
      FieldName = 'WHY'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.WHY'
      Size = 255
    end
    object ResultQueryDEPARTMENT_NAME: TIBStringField
      FieldName = 'DEPARTMENT_NAME'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.DEPARTMENT_NAME'
      Size = 255
    end
    object ResultQueryPOST_NAME: TIBStringField
      FieldName = 'POST_NAME'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.POST_NAME'
      Size = 255
    end
    object ResultQueryWORK_MODE_NAME: TIBStringField
      FieldName = 'WORK_MODE_NAME'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.WORK_MODE_NAME'
      Size = 255
    end
    object ResultQueryTN: TIntegerField
      FieldName = 'TN'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.TN'
    end
    object ResultQueryID_PCARD: TIntegerField
      FieldName = 'ID_PCARD'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.ID_PCARD'
    end
    object ResultQueryID_HOSPITAL: TIntegerField
      FieldName = 'ID_HOSPITAL'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.ID_HOSPITAL'
    end
    object ResultQueryHOSP_STR: TIBStringField
      FieldName = 'HOSP_STR'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.HOSP_STR'
      Size = 255
    end
    object ResultQueryUSE_KOL_STAVOK: TIntegerField
      FieldName = 'USE_KOL_STAVOK'
      Origin = 'DT_WORK_MODE_CHANGE_SELECT.USE_KOL_STAVOK'
    end
  end
  object ResultSource: TDataSource
    DataSet = ResultQuery
    Left = 616
    Top = 353
  end
end
