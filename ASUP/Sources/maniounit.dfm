object ManIoForm: TManIoForm
  Left = 208
  Top = 175
  Width = 732
  Height = 480
  Caption = #1053#1072#1082#1072#1079' '#1087#1088#1086' '#1074#1080#1082#1086#1085#1072#1085#1085#1103' '#1086#1073#1086#1074'`'#1103#1079#1082#1110#1074
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 724
    Height = 57
    Align = alTop
    TabOrder = 0
    object AddButton: TSpeedButton
      Left = 0
      Top = 0
      Width = 65
      Height = 57
      Caption = #1044#1086#1076#1072#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = AddButtonClick
    end
    object ModifyButton: TSpeedButton
      Left = 65
      Top = 0
      Width = 65
      Height = 57
      Caption = #1047#1084#1110#1085#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFF868686777777FFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = ModifyButtonClick
    end
    object DeleteButton: TSpeedButton
      Left = 195
      Top = 0
      Width = 65
      Height = 57
      Caption = #1042#1080#1083#1091#1095#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = DeleteButtonClick
    end
    object CancelButton: TSpeedButton
      Left = 385
      Top = 0
      Width = 65
      Height = 57
      Caption = #1047#1072#1082#1088#1080#1090#1080
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = CancelButtonClick
    end
    object OrderItemsButton: TSpeedButton
      Left = 260
      Top = 0
      Width = 102
      Height = 57
      Caption = #1055#1091#1085#1082#1090#1080' '#1085#1072#1082#1072#1079#1091
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = OrderItemsButtonClick
    end
    object ViewButton: TSpeedButton
      Left = 130
      Top = 0
      Width = 65
      Height = 57
      Caption = #1055#1077#1088#1077#1075#1083#1103#1076
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = ViewButtonClick
    end
  end
  object ResultGrid: TDBGrid
    Left = 0
    Top = 57
    Width = 724
    Height = 343
    Align = alClient
    Color = clInfoBk
    Constraints.MinHeight = 128
    DataSource = ResultSource
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 1
    TitleFont.Charset = RUSSIAN_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Verdana'
    TitleFont.Style = [fsBold]
    OnKeyDown = ResultGridKeyDown
    OnKeyPress = ResultGridKeyPress
    Columns = <
      item
        Expanded = False
        FieldName = 'NUM_ITEM'
        Title.Caption = #1055#1053
        Width = 26
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FIO1'
        Title.Caption = #1055#1030#1041' '#1074#1110#1076#1089#1091#1090#1085#1100#1086#1075#1086
        Width = 111
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TN1'
        Title.Caption = #1058#1053
        Width = 49
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NAME_POSTDEP1'
        Title.Caption = #1076#1077' '#1087#1088#1072#1094#1102#1108
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FIO2'
        Title.Caption = #1055#1030#1041' '#1074#1080#1082'. '#1086#1073#1086#1074'.'
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TN2'
        Title.Caption = #1058#1053
        Width = 35
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NAME_POSTDEP2'
        Title.Caption = #1076#1077' '#1087#1088#1072#1094#1102#1108
        Width = 98
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_BEG'
        Title.Caption = #1079
        Width = 57
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_END'
        Title.Caption = #1087#1086
        Width = 56
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SUMMA_PERCENT_STR'
        Title.Caption = #1057#1091#1084#1084#1072' (%)'
        Width = 77
        Visible = True
      end>
  end
  object Panel3: TPanel
    Left = 0
    Top = 400
    Width = 724
    Height = 53
    Align = alBottom
    TabOrder = 2
    object Label2: TLabel
      Left = 8
      Top = 8
      Width = 43
      Height = 13
      Caption = #1055#1086#1096#1091#1082':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label9: TLabel
      Left = 256
      Top = 8
      Width = 103
      Height = 13
      Caption = '('#1055#1030#1041' '#1072#1073#1086' '#1090#1072#1073'. '#1085#1086#1084#1077#1088')'
    end
    object SearchEdit: TEdit
      Left = 56
      Top = 5
      Width = 193
      Height = 21
      TabOrder = 0
      OnChange = SearchEditChange
    end
    object RadioButton1: TRadioButton
      Left = 368
      Top = 8
      Width = 153
      Height = 17
      Caption = #1055#1086#1096#1091#1082' '#1089#1077#1088#1077#1076' '#1074#1110#1076#1089#1091#1090#1085#1110#1093
      Checked = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      TabStop = True
    end
    object RadioButton2: TRadioButton
      Left = 368
      Top = 32
      Width = 241
      Height = 17
      Caption = #1055#1086#1096#1091#1082' '#1089#1077#1088#1077#1076' '#1074#1080#1082#1086#1085#1091#1102#1095#1080#1093' '#1086#1073#1086#1074'`'#1103#1079#1082#1080
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
    end
  end
  object ResultQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    AfterOpen = ResultQueryAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * FROM DT_MAN_BONUS_IO_SELECT_BY_ORDER (:ID_ORDER)')
    Left = 216
    Top = 225
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID_ORDER'
        ParamType = ptUnknown
      end>
    object ResultQueryID_MAN_BONUS_IO: TIntegerField
      FieldName = 'ID_MAN_BONUS_IO'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_MAN_BONUS_IO'
    end
    object ResultQueryID_IO_REASON: TIntegerField
      FieldName = 'ID_IO_REASON'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_IO_REASON'
    end
    object ResultQueryID_MAN_MOVING1: TIntegerField
      FieldName = 'ID_MAN_MOVING1'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_MAN_MOVING1'
    end
    object ResultQueryID_IO_TYPE: TIntegerField
      FieldName = 'ID_IO_TYPE'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_IO_TYPE'
    end
    object ResultQueryID_MAN_BONUS: TIntegerField
      FieldName = 'ID_MAN_BONUS'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_MAN_BONUS'
    end
    object ResultQueryREASON: TIBStringField
      FieldName = 'REASON'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.REASON'
      Size = 255
    end
    object ResultQueryID_MAN_MOVING2: TIntegerField
      FieldName = 'ID_MAN_MOVING2'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_MAN_MOVING2'
    end
    object ResultQueryID_BONUS: TIntegerField
      FieldName = 'ID_BONUS'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.ID_BONUS'
    end
    object ResultQuerySUMMA: TIBBCDField
      FieldName = 'SUMMA'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.SUMMA'
      Precision = 9
      Size = 2
    end
    object ResultQueryDATE_BEG: TDateField
      FieldName = 'DATE_BEG'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.DATE_BEG'
    end
    object ResultQueryDATE_END: TDateField
      FieldName = 'DATE_END'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.DATE_END'
    end
    object ResultQueryNUM_ITEM: TIntegerField
      FieldName = 'NUM_ITEM'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.NUM_ITEM'
    end
    object ResultQueryFIO1: TIBStringField
      FieldName = 'FIO1'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.FIO1'
      Size = 62
    end
    object ResultQueryFIO2: TIBStringField
      FieldName = 'FIO2'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.FIO2'
      Size = 62
    end
    object ResultQueryNAME_POSTDEP1: TIBStringField
      FieldName = 'NAME_POSTDEP1'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.NAME_POSTDEP1'
      Size = 512
    end
    object ResultQueryNAME_POSTDEP2: TIBStringField
      FieldName = 'NAME_POSTDEP2'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.NAME_POSTDEP2'
      Size = 512
    end
    object ResultQueryNAME_BONUS: TIBStringField
      FieldName = 'NAME_BONUS'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.NAME_BONUS'
      Size = 255
    end
    object ResultQueryIO_REASON: TIBStringField
      FieldName = 'IO_REASON'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.IO_REASON'
      Size = 255
    end
    object ResultQueryIO_TYPE: TIBStringField
      FieldName = 'IO_TYPE'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.IO_TYPE'
      Size = 255
    end
    object ResultQueryTN1: TIntegerField
      FieldName = 'TN1'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.TN1'
    end
    object ResultQueryTN2: TIntegerField
      FieldName = 'TN2'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.TN2'
    end
    object ResultQuerySUMMA_PERCENT_STR: TIBStringField
      FieldName = 'SUMMA_PERCENT_STR'
      Origin = 'DT_MAN_BONUS_IO_SELECT_BY_ORDER.SUMMA_PERCENT_STR'
      Size = 255
    end
  end
  object ResultSource: TDataSource
    DataSet = ResultQuery
    Left = 248
    Top = 225
  end
  object WorkQuery: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    Left = 216
    Top = 192
  end
end
