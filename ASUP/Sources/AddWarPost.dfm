object AddWarpostForm: TAddWarpostForm
  Left = 205
  Top = 168
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1044#1086#1076#1072#1090#1080' '#1074#1110#1081#1089#1100#1082#1086#1074#1077' '#1079#1074#1072#1085#1085#1103
  ClientHeight = 131
  ClientWidth = 403
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object WarPostLabel: TLabel
    Left = 16
    Top = 16
    Width = 111
    Height = 13
    Caption = #1042#1110#1081#1089#1100#1082#1086#1074#1077' '#1079#1074#1072#1085#1085#1103':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object WarSostavLabel: TLabel
    Left = 16
    Top = 56
    Width = 111
    Height = 13
    Caption = #1042#1110#1081#1089#1100#1082#1086#1074#1080#1081' '#1089#1082#1083#1072#1076':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object WarPostFieldControl: TFieldControl
    Left = 376
    Top = 16
    Width = 18
    Height = 18
    Control = WarPostEdit
    ReadFromField = WarPostInfoQueryNAME_WARPOST
    Title = '"'#1042#1110#1081#1089#1082#1086#1074#1077' '#1079#1074#1072#1085#1085#1103' "'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object WarSostavFieldControl: TFieldControl
    Left = 376
    Top = 56
    Width = 17
    Height = 18
    Control = WarSostavBox
    ReadFromField = WarPostInfoQueryID_WARSOSTAV
    Title = '"'#1042#1110#1081#1089#1082#1086#1074#1110#1081' '#1089#1086#1089#1090#1072#1074'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object OkButton: TBitBtn
    Left = 86
    Top = 96
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 238
    Top = 96
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 3
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object WarPostEdit: TEdit
    Left = 144
    Top = 16
    Width = 225
    Height = 21
    MaxLength = 50
    TabOrder = 0
  end
  object WarSostavBox: TSpComboBox
    Left = 144
    Top = 56
    Width = 225
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    ItemCount = 5
    SpOpenItem = '* '#1044#1086#1074#1110#1076#1085#1080#1082' ...'
    AutoDrop = True
    SpParams.Table = 'SP_WARSOSTAV'
    SpParams.IdField = 'ID_WARSOSTAV'
    SpParams.SpFields = 'ID_WARSOSTAV, NAME_WARSOSTAV'
    SpParams.Title = #1042#1110#1081#1089#1100#1082#1086#1074#1080#1081' '#1089#1082#1083#1072#1076
    SpParams.ComboField = 'Name_WarSostav, Name_WarPostGr'
    SpParams.SelectProcName = 'Sp_WarSostav_Select'
    SpParams.DeleteProcName = 'WARASOSTAV_DELETE'
    SpParams.DontHideFirstColumn = False
    SpParams.SpMode = [spfAdd, spfModify, spfDelete, spfFind, spfSelect, spfExt]
    AlternativeForm = 'SpWarSostavForm'
  end
  object WarPostInfoQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Name_WarPost, Id_WarSostav'
      'FROM Sp_WarPost'
      'WHERE Id_WarPost = :Id_WarPost')
    Left = 16
    Top = 96
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_WarPost'
        ParamType = ptUnknown
      end>
    object WarPostInfoQueryNAME_WARPOST: TIBStringField
      FieldName = 'NAME_WARPOST'
      Required = True
      Size = 50
    end
    object WarPostInfoQueryID_WARSOSTAV: TIntegerField
      FieldName = 'ID_WARSOSTAV'
      Required = True
    end
  end
end
