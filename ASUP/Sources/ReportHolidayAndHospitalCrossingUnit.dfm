object ReportHolidayAndHospitalCrossingForm: TReportHolidayAndHospitalCrossingForm
  Left = 213
  Top = 168
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1047#1074#1110#1090' '#1087#1086' '#1087#1077#1088#1077#1090#1080#1085#1072#1084' '#1083#1110#1082#1072#1088#1085#1103#1085#1080#1093' '#1083#1080#1089#1090#1110#1074' '#1090#1072' '#1074#1110#1076#1087#1091#1089#1090#1086#1082
  ClientHeight = 111
  ClientWidth = 517
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label4: TLabel
    Left = 8
    Top = 41
    Width = 73
    Height = 13
    Caption = #1047#1072' '#1087#1077#1088#1110#1086#1076' '#1079':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 240
    Top = 41
    Width = 21
    Height = 13
    Caption = #1055#1086':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 8
    Top = 8
    Width = 62
    Height = 13
    Caption = #1055#1110#1076#1088#1086#1079#1076#1110#1083':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object OkButton: TBitBtn
    Left = 56
    Top = 73
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 345
    Top = 73
    Width = 113
    Height = 25
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object ExportButton: TBitBtn
    Left = 200
    Top = 73
    Width = 105
    Height = 25
    Caption = #1045#1082#1089#1087#1086#1088#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    OnClick = OkButtonClick
    Glyph.Data = {
      
      33333333330A0333333333333330333333333333333033333333}
  end
  object DateEndPicker: TDateTimePicker
    Left = 280
    Top = 39
    Width = 121
    Height = 21
    Date = 37744.245348842600000000
    Time = 37744.245348842600000000
    TabOrder = 3
  end
  object DateBegPicker: TDateTimePicker
    Left = 104
    Top = 39
    Width = 121
    Height = 21
    Date = 37744.245348842600000000
    Time = 37744.245348842600000000
    TabOrder = 4
  end
  object Edit1: TEdit
    Left = 93
    Top = 4
    Width = 356
    Height = 21
    TabStop = False
    ReadOnly = True
    TabOrder = 5
  end
  object Button1: TButton
    Left = 449
    Top = 2
    Width = 31
    Height = 25
    Caption = '...'
    TabOrder = 6
    TabStop = False
    OnClick = Button1Click
  end
  object Query: TIBQuery
    Database = Main.ChermetDatabase
    Transaction = Main.DefaultTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT *'
      
        'from REPORT_HOLIDAY_AND_HOSP_CROSSIN(:ID_DEPARTMENT, :DATE_BEG, ' +
        ':DATE_END);')
    Left = 288
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID_DEPARTMENT'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATE_BEG'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATE_END'
        ParamType = ptUnknown
      end>
    object QueryFIO: TIBStringField
      FieldName = 'FIO'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.FIO'
      Size = 62
    end
    object QueryPOST: TIBStringField
      FieldName = 'POST'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.POST'
      Size = 255
    end
    object QueryDEP: TIBStringField
      FieldName = 'DEP'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.DEP'
      Size = 255
    end
    object QueryHOLIDAY_NAME: TIBStringField
      FieldName = 'HOLIDAY_NAME'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.HOLIDAY_NAME'
      Size = 255
    end
    object QueryHOLIDAY_DATE_BEG: TDateField
      FieldName = 'HOLIDAY_DATE_BEG'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.HOLIDAY_DATE_BEG'
    end
    object QueryHOLIDAY_DATE_END: TDateField
      FieldName = 'HOLIDAY_DATE_END'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.HOLIDAY_DATE_END'
    end
    object QueryHOSP_BEG: TDateField
      FieldName = 'HOSP_BEG'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.HOSP_BEG'
    end
    object QueryHOSP_END: TDateField
      FieldName = 'HOSP_END'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.HOSP_END'
    end
    object QueryTN: TIntegerField
      FieldName = 'TN'
      Origin = 'REPORT_HOLIDAY_AND_HOSP_CROSSIN.TN'
    end
  end
  object Report: TfrReport
    Dataset = DataSet
    InitialZoom = pzDefault
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit]
    RebuildPrinter = False
    Left = 320
    ReportForm = {19000000}
  end
  object DataSet: TfrDBDataSet
    DataSet = Query
    Left = 352
  end
end
