object AddPrivForm: TAddPrivForm
  Left = 298
  Top = 277
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1044#1086#1076#1072#1090#1080' '#1087#1110#1083#1100#1075#1091
  ClientHeight = 134
  ClientWidth = 386
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDefault
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object WarPostLabel: TLabel
    Left = 16
    Top = 16
    Width = 44
    Height = 13
    Caption = #1055#1110#1083#1100#1075#1072':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object WarSostavLabel: TLabel
    Left = 16
    Top = 56
    Width = 68
    Height = 13
    Caption = #1058#1080#1087' '#1087#1110#1083#1100#1075#1080':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PrivFieldControl: TFieldControl
    Left = 360
    Top = 16
    Width = 18
    Height = 18
    Control = PrivEdit
    ReadFromField = PrivInfoQueryNAME_PRIV
    Title = '"'#1055#1110#1083#1100#1075#1072'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object PrivTypeFieldControl: TFieldControl
    Left = 360
    Top = 56
    Width = 17
    Height = 18
    Control = PrivTypeBox
    ReadFromField = PrivInfoQueryID_PRIV_TYPE
    Title = '"'#1058#1080#1087' '#1087#1110#1083#1100#1075#1080'"'
    MaxLength = 0
    ExactLength = 0
    CapitalizeStyle = csNone
  end
  object OkButton: TBitBtn
    Left = 126
    Top = 96
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 2
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 254
    Top = 96
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 3
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object PrivTypeBox: TSpComboBox
    Left = 144
    Top = 56
    Width = 217
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    ItemCount = 5
    SpOpenItem = '* '#1044#1086#1074#1110#1076#1085#1080#1082' ...'
    AutoDrop = True
    SpParams.Table = 'Sp_Priv_Type'
    SpParams.IdField = 'Id_Priv_Type'
    SpParams.SpFields = 'Name_Priv_Type'
    SpParams.Title = #1058#1080#1087#1080' '#1087#1110#1083#1100#1075
    SpParams.ComboField = 'Name_Priv_Type'
    SpParams.ColumnNames = #1058#1080#1087#1080' '#1087#1110#1083#1100#1075
    SpParams.InsertProcName = 'Sp_Priv_Type_Insert'
    SpParams.UpdateProcName = 'Sp_Priv_type_Update'
    SpParams.DeleteProcName = 'Sp_Priv_type_delete'
    SpParams.DontHideFirstColumn = False
    SpParams.SpMode = [spfAdd, spfModify, spfDelete, spfFind, spfSelect, spfExt]
  end
  object PrivEdit: TEdit
    Left = 144
    Top = 16
    Width = 217
    Height = 21
    MaxLength = 50
    TabOrder = 0
  end
  object PrivInfoQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT Name_Priv, Id_Priv_Type'
      'FROM Sp_Privelege'
      'WHERE Id_Privelege = :Id_Privelege')
    Left = 16
    Top = 96
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Id_Privelege'
        ParamType = ptUnknown
      end>
    object PrivInfoQueryNAME_PRIV: TIBStringField
      FieldName = 'NAME_PRIV'
      Size = 50
    end
    object PrivInfoQueryID_PRIV_TYPE: TIntegerField
      FieldName = 'ID_PRIV_TYPE'
      Required = True
    end
  end
end
