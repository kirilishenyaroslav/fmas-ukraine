object fmFormNewShtat: TfmFormNewShtat
  Left = 214
  Top = 126
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1057#1092#1086#1088#1084#1091#1074#1072#1090#1080' '#1085#1086#1074#1080#1081' '#1076#1086#1082#1091#1084#1077#1085#1090
  ClientHeight = 233
  ClientWidth = 552
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 32
    Top = 160
    Width = 223
    Height = 13
    Caption = '('#1076#1083#1103' '#1074#1080#1074#1086#1076#1091' '#1079' '#1074#1080#1082#1086#1088#1080#1089#1090#1072#1085#1085#1103' '#1089#1090#1072#1088#1080#1093' '#1086#1082#1083#1072#1076#1110#1074')'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object FormControl: TqFFormControl
    Left = 48
    Top = 184
    Width = 60
    Height = 21
    ControlsOwner = Owner
    InsertSQL.Strings = (
      
        'EXECUTE PROCEDURE ASUP_SHTAT_FORM_NEW_SHR :Old_SR, :Name, :Date_' +
        'Beg,'
      ':Date_End, :Inc_Percent')
    NewRecordAfterPrepare = FormControlNewRecordAfterPrepare
    Mode = fmAdd
    CheckWhere = True
    CloseForm = True
  end
  object qFLogicCheck1: TqFLogicCheck
    Left = 376
    Top = 40
    Width = 36
    Height = 21
    OnCheck = qFLogicCheck1Check
  end
  object OkButton: TBitBtn
    Left = 168
    Top = 185
    Width = 121
    Height = 27
    Caption = #1043#1072#1088#1072#1079#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 0
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 318
    Top = 185
    Width = 121
    Height = 27
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object qFCharControl1: TqFCharControl
    Left = 22
    Top = 16
    Width = 507
    Height = 21
    FieldName = 'NAME'
    DisplayName = #1053#1072#1079#1074#1072' '#1096#1090#1072#1090#1085#1086#1075#1086' '#1088#1086#1079#1082#1083#1072#1076#1091
    Interval = 210
    Value = ''
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 2
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object Date_Beg: TqFDateControl
    Left = 24
    Top = 56
    Width = 312
    Height = 21
    FieldName = 'DATE_BEG'
    DisplayName = #1055#1086#1095#1072#1090#1086#1082' '#1087#1077#1088#1110#1086#1076#1091' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103
    Interval = 210
    Value = 38470
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
  end
  object Date_End: TqFDateControl
    Left = 24
    Top = 96
    Width = 313
    Height = 21
    FieldName = 'DATE_END'
    DisplayName = #1050#1110#1085#1077#1094#1100' '#1087#1077#1088#1110#1086#1076#1091' '#1092#1091#1085#1082#1094#1110#1086#1085#1091#1074#1072#1085#1085#1103
    Interval = 210
    Value = 38470
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 4
    AutoSaveToRegistry = False
  end
  object Inc_Percent: TqFFloatControl
    Left = 24
    Top = 136
    Width = 305
    Height = 21
    FieldName = 'Inc_Percent'
    DisplayName = #1055#1088#1086#1094#1077#1085#1090' '#1087#1110#1076#1074#1080#1097#1077#1085#1085#1103
    Interval = 210
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 5
    AutoSaveToRegistry = False
    MaxLength = 0
    NegativeAllowed = False
    Precision = 2
    Format = '#########0.00'
  end
  object OLD_SR: TqFInvisControl
    Left = 376
    Top = 88
    Width = 200
    Height = 21
    FieldName = 'OLD_SR'
    DisplayName = #1053#1072#1079#1074#1072' '#1087#1086#1083#1103
    Interval = 120
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 6
    AutoSaveToRegistry = False
  end
end
