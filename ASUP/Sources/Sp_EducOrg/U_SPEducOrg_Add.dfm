object Form_SPEducOrg_Add: TForm_SPEducOrg_Add
  Left = 298
  Top = 153
  ActiveControl = CancelButton
  BorderStyle = bsDialog
  Caption = #1044#1086#1076#1072#1090#1080' '#1085#1072#1074#1095#1072#1083#1100#1085#1080#1081' '#1079#1072#1082#1083#1072#1076
  ClientHeight = 224
  ClientWidth = 573
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 573
    Height = 224
    Align = alClient
    TabOrder = 0
    object qFFC_EducOrg: TqFFormControl
      Left = 256
      Top = 192
      Width = 60
      Height = 21
      ControlsOwner = Owner
      InsertSQL.Strings = (
        
          'execute procedure ASUP_SP_EDUCORG_INSERT(:name_full,:name_short,' +
          ':id_adress,:akreditation,:IS_LEARNING,:IS_KVALIF,:ID_TYPE);')
      UpdateSQL.Strings = (
        
          'execute procedure ASUP_SP_EDUCORG_update(:name_full,:name_short,' +
          ':id_adress,:akreditation,:IS_LEARNING,:IS_KVALIF,:where,:ID_TYPE' +
          ');')
      SelectSQL.Strings = (
        'select * from asup_sp_educorg_get(:where);')
      AddCaption = #1044#1086#1076#1072#1090#1080' '#1085#1072#1074#1095#1072#1083#1100#1085#1080#1081' '#1079#1072#1082#1083#1072#1076
      ModifyCaption = #1047#1084#1110#1085#1080#1090#1080' '#1085#1072#1074#1095#1072#1083#1100#1085#1080#1081
      InfoCaption = #1053#1072#1074#1095#1072#1083#1100#1085#1080#1081' '#1079#1072#1082#1083#1072#1076
      Mode = fmAdd
      CheckWhere = True
      CloseForm = True
    end
    object OkButton: TBitBtn
      Left = 143
      Top = 191
      Width = 97
      Height = 26
      Caption = #1043#1072#1088#1072#1079#1076
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ModalResult = 1
      ParentFont = False
      TabOrder = 0
      OnClick = OkButtonClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object CancelButton: TBitBtn
      Left = 332
      Top = 191
      Width = 97
      Height = 26
      Cancel = True
      Caption = #1042#1110#1076#1084#1110#1085#1072
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ModalResult = 2
      ParentFont = False
      TabOrder = 1
      OnClick = CancelButtonClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object qFSC_type: TqFSpravControl
      Left = 8
      Top = 16
      Width = 560
      Height = 21
      FieldName = 'Id_type'
      DisplayName = #1043#1088#1091#1087#1072
      Interval = 120
      Value = Null
      LabelColor = clGreen
      Required = False
      Semicolon = True
      Asterisk = False
      Enabled = True
      Blocked = False
      TabOrder = 2
      OnOpenSprav = qFSC_typeOpenSprav
      DisplayTextField = 'type_name'
    end
    object qFBC_IsEduc: TqFBoolControl
      Left = 8
      Top = 112
      Width = 200
      Height = 21
      FieldName = 'IS_LEARNING'
      DisplayName = #1053#1072#1074#1095#1072#1108
      Interval = 120
      Value = True
      LabelColor = clGreen
      Required = True
      Semicolon = False
      Asterisk = False
      Enabled = True
      Default = 1
      Blocked = False
      TabOrder = 3
      StoreAs = StoreAs01
    end
    object qFBC_IsKval: TqFBoolControl
      Left = 8
      Top = 136
      Width = 200
      Height = 21
      FieldName = 'IS_KVALIF'
      DisplayName = #1055#1110#1076#1074#1080#1097#1091#1108' '#1082#1074#1072#1083#1110#1092#1110#1082#1072#1094#1110#1102
      Interval = 120
      Value = False
      LabelColor = clGreen
      Required = True
      Semicolon = False
      Asterisk = False
      Enabled = True
      Blocked = False
      TabOrder = 4
      StoreAs = StoreAs01
    end
    object qFIC_Akred: TqFIntControl
      Left = 8
      Top = 88
      Width = 200
      Height = 21
      FieldName = 'AKREDITATION'
      DisplayName = #1040#1082#1088#1077#1076#1080#1090#1072#1094#1110#1103
      Interval = 120
      Value = 0
      LabelColor = clGreen
      Required = False
      Semicolon = True
      Asterisk = False
      Enabled = True
      Default = 0
      Blocked = False
      TabOrder = 5
      MaxLength = 10
      NegativeAllowed = False
      ZeroAllowed = False
    end
    object qFCC_FName: TqFCharControl
      Left = 8
      Top = 40
      Width = 560
      Height = 21
      FieldName = 'name_full'
      DisplayName = #1055#1086#1074#1085#1072' '#1085#1072#1079#1074#1072
      Interval = 120
      Value = ''
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      Blocked = False
      TabOrder = 6
      MaxLength = 0
    end
    object qFCC_SName: TqFCharControl
      Left = 8
      Top = 64
      Width = 297
      Height = 21
      FieldName = 'name_short'
      DisplayName = #1057#1082#1086#1088#1086#1095#1077#1085#1072' '#1085#1072#1079#1074#1072
      Interval = 120
      Value = ''
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      Blocked = False
      TabOrder = 7
      MaxLength = 15
    end
    object qFSC_Adr: TqFSpravControl
      Left = 8
      Top = 160
      Width = 560
      Height = 21
      FieldName = 'ID_ADRESS'
      DisplayName = #1040#1076#1088#1077#1089#1072
      Interval = 120
      Value = Null
      LabelColor = clGreen
      Required = False
      Semicolon = True
      Asterisk = False
      Enabled = True
      Blocked = False
      TabOrder = 8
      OnOpenSprav = qFSC_AdrOpenSprav
      DisplayTextField = 'ADRESS_NAME'
    end
  end
end
