object AdressForm: TAdressForm
  Left = 255
  Top = 117
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1040#1076#1088#1077#1089#1072
  ClientHeight = 397
  ClientWidth = 429
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clGreen
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object OkButton: TBitBtn
    Left = 104
    Top = 359
    Width = 97
    Height = 27
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 1
    ParentFont = False
    TabOrder = 0
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 228
    Top = 359
    Width = 97
    Height = 27
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object AdrPanel: TPanel
    Left = 0
    Top = 8
    Width = 425
    Height = 337
    TabOrder = 2
    object Label1: TLabel
      Left = 8
      Top = 8
      Width = 45
      Height = 13
      Caption = #1050#1088#1072#1111#1085#1072':'
    end
    object CountryButton: TSpeedButton
      Left = 344
      Top = 8
      Width = 23
      Height = 22
      Caption = '...'
      OnClick = CountryButtonClick
    end
    object RegionButton: TSpeedButton
      Left = 344
      Top = 40
      Width = 23
      Height = 22
      Caption = '...'
      OnClick = RegionButtonClick
    end
    object PlaceButton: TSpeedButton
      Left = 344
      Top = 72
      Width = 23
      Height = 22
      Caption = '...'
      OnClick = PlaceButtonClick
    end
    object Label3: TLabel
      Left = 8
      Top = 72
      Width = 107
      Height = 13
      Caption = #1053#1072#1089#1077#1083#1077#1085#1080#1081' '#1087#1091#1085#1082#1090':'
    end
    object Bevel1: TBevel
      Left = -4
      Top = 98
      Width = 413
      Height = 10
      Shape = bsBottomLine
      Style = bsRaised
    end
    object Label4: TLabel
      Left = 8
      Top = 120
      Width = 41
      Height = 13
      Caption = #1056#1072#1081#1086#1085':'
    end
    object DistrictButton: TSpeedButton
      Left = 344
      Top = 120
      Width = 23
      Height = 22
      Caption = '...'
      OnClick = DistrictButtonClick
    end
    object Label6: TLabel
      Left = 8
      Top = 152
      Width = 47
      Height = 13
      Caption = #1042#1091#1083#1080#1094#1103':'
    end
    object Label7: TLabel
      Left = 8
      Top = 184
      Width = 47
      Height = 13
      Caption = #1050#1086#1088#1087#1091#1089':'
    end
    object Label8: TLabel
      Left = 8
      Top = 216
      Width = 54
      Height = 13
      Caption = #1041#1091#1076#1080#1085#1086#1082':'
    end
    object Label9: TLabel
      Left = 8
      Top = 248
      Width = 61
      Height = 13
      Caption = #1050#1074#1072#1088#1090#1080#1088#1072':'
    end
    object Label10: TLabel
      Left = 8
      Top = 312
      Width = 44
      Height = 13
      Caption = #1030#1085#1076#1077#1082#1089':'
    end
    object Label2: TLabel
      Left = 8
      Top = 40
      Width = 43
      Height = 13
      Caption = #1056#1077#1075#1110#1086#1085':'
    end
    object Label5: TLabel
      Left = 8
      Top = 280
      Width = 57
      Height = 13
      Caption = #1058#1077#1083#1077#1092#1086#1085':'
    end
    object CountryEdit: TCheckEdit
      Left = 144
      Top = 8
      Width = 193
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      InputSet = isAll
    end
    object RegionEdit: TCheckEdit
      Left = 144
      Top = 40
      Width = 193
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
      InputSet = isAll
    end
    object DistrictEdit: TCheckEdit
      Left = 144
      Top = 120
      Width = 193
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
      InputSet = isAll
    end
    object StreetEdit: TCheckEdit
      Left = 144
      Top = 152
      Width = 273
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      InputSet = isAll
    end
    object KorpusEdit: TCheckEdit
      Left = 144
      Top = 184
      Width = 65
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      InputSet = isAll
    end
    object HouseEdit: TCheckEdit
      Left = 144
      Top = 216
      Width = 65
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      InputSet = isAll
    end
    object FlatEdit: TCheckEdit
      Left = 144
      Top = 248
      Width = 65
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      InputSet = isAll
    end
    object ZIPEdit: TCheckEdit
      Left = 144
      Top = 312
      Width = 161
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
      InputSet = isAll
    end
    object PlaceEdit: TCheckEdit
      Left = 144
      Top = 72
      Width = 193
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 8
      InputSet = isAll
    end
    object CountryCheck: TCheckBox
      Left = 128
      Top = 10
      Width = 17
      Height = 17
      Caption = 'CountryCheck'
      Checked = True
      State = cbChecked
      TabOrder = 9
      OnClick = CountryCheckClick
    end
    object RegionCheck: TCheckBox
      Left = 128
      Top = 43
      Width = 17
      Height = 17
      Caption = 'RegionCheck'
      Checked = True
      State = cbChecked
      TabOrder = 10
      OnClick = RegionCheckClick
    end
    object PlaceCheck: TCheckBox
      Left = 128
      Top = 75
      Width = 17
      Height = 17
      Caption = 'PlaceCheck'
      Checked = True
      State = cbChecked
      TabOrder = 11
      OnClick = PlaceCheckClick
    end
    object DistrictCheck: TCheckBox
      Left = 128
      Top = 123
      Width = 17
      Height = 17
      Caption = 'DistrictCheck'
      Checked = True
      State = cbChecked
      TabOrder = 12
      OnClick = DistrictCheckClick
    end
    object PhoneEdit: TCheckEdit
      Left = 144
      Top = 280
      Width = 121
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 13
      InputSet = isAll
    end
  end
  object FullInfoQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * FROM ASUP_DT_LIN_ADR_INFO(:Id)')
    Left = 8
    Top = 352
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ID'
        ParamType = ptUnknown
      end>
    object FullInfoQueryNAME_DISTRICT: TIBStringField
      FieldName = 'NAME_DISTRICT'
      Origin = 'SP_DISTRICT.NAME_DISTRICT'
      Size = 30
    end
    object FullInfoQueryID_ADDRESS: TIntegerField
      FieldName = 'ID_ADDRESS'
      Origin = 'DT_ADDRESS.ID_ADDRESS'
      Required = True
    end
    object FullInfoQuerySTREET: TIBStringField
      FieldName = 'STREET'
      Origin = 'DT_ADDRESS.STREET'
      Size = 180
    end
    object FullInfoQueryNAME_REGION: TIBStringField
      FieldName = 'NAME_REGION'
      Origin = 'SP_REGION.NAME_REGION'
      Size = 30
    end
    object FullInfoQueryPLACE: TIBStringField
      FieldName = 'PLACE'
      Origin = 'PROC_SP_PLACE_SEL.PLACE'
      Size = 60
    end
    object FullInfoQueryKORPUS: TIBStringField
      FieldName = 'KORPUS'
      Origin = 'DT_ADDRESS.KORPUS'
      Size = 6
    end
    object FullInfoQueryHOUSE: TIBStringField
      FieldName = 'HOUSE'
      Origin = 'DT_ADDRESS.HOUSE'
      Size = 6
    end
    object FullInfoQueryFLAT: TIBStringField
      FieldName = 'FLAT'
      Origin = 'DT_ADDRESS.FLAT'
      Size = 6
    end
    object FullInfoQueryZIP: TIBStringField
      FieldName = 'ZIP'
      Origin = 'DT_ADDRESS.ZIP'
      Size = 30
    end
    object FullInfoQueryID_REGION: TIntegerField
      FieldName = 'ID_REGION'
      Origin = 'DT_ADDRESS.ID_REGION'
    end
    object FullInfoQueryID_PLACE: TIntegerField
      FieldName = 'ID_PLACE'
      Origin = 'DT_ADDRESS.ID_PLACE'
    end
    object FullInfoQueryID_DISTRICT: TIntegerField
      FieldName = 'ID_DISTRICT'
      Origin = 'DT_ADDRESS.ID_DISTRICT'
    end
    object FullInfoQueryPHONE: TIBStringField
      FieldName = 'PHONE'
      Origin = 'DT_ADDRESS.PHONE'
      Size = 16
    end
    object FullInfoQueryID_COUNTRY: TIntegerField
      FieldName = 'ID_COUNTRY'
      Origin = 'DT_ADDRESS.ID_COUNTRY'
    end
    object FullInfoQueryNAME_COUNTRY: TIBStringField
      FieldName = 'NAME_COUNTRY'
      Origin = 'PUB_SP_COUNTRY.NAME_COUNTRY'
      Size = 30
    end
  end
  object WorkQuery: TIBQuery
    BufferChunks = 1000
    CachedUpdates = False
    Left = 40
    Top = 352
  end
end
