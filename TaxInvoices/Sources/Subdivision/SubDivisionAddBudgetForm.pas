...
uses
  ...
  cxDropDownEdit, cxTextEdit, cxMaskEdit, StdCtrls, Mask, ToolEdit, cxTL,
  cxContainer, cxButtonEdit, cxButtons, ExtCtrls,
   CurrEdit, iBase, GlobalSPR,  dogLoaderUnit
  ;
...
type
  TfrmAddSmet = class(TForm)
    ...
    NameSubdivisionLabel: TLabel;
    NameSubdivisionValLabel: TLabel;
  ...
  function AddKosht(AOwner : TComponent; DBHANDLE : TISC_DB_HANDLE; sum_doc: Currency; Mode: Integer {1-добавить,2-редактировать};
                  id_s, id_r, id_st, id_k: Integer; s_smet: Currency; DateDog: TDate; NameSubdivision : String):Variant; stdcall;
  exports AddKosht;
...
function AddKosht(AOwner : TComponent; DBHANDLE : TISC_DB_HANDLE; sum_doc: Currency; Mode: Integer {1-добавить,2-редактировать};
                  id_s, id_r, id_st, id_k: Integer; s_smet: Currency; DateDog: TDate; NameSubdivision : String):Variant;
var
  spr: TfrmAddSmet;
  d_dog: TDate;
begin
  spr := TfrmAddSmet.Create(Aowner,DBHANDLE,sum_doc);
  spr.NameSubdivisionValLabel.Caption := NameSubdivision;
  res := Unassigned;
  _mode := mode;
  d_dog := DateDog;
  if _mode = 2 then
...
{  if  CurrencyEdit_sum_smet.Value < 0 then
  ...
  //sum_smet:=CurrencyEdit_sum_smet.Vlue;
