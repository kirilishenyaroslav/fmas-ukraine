unit cn_Common_WaitForm_progress;
...
uses
  ...
  cnConsts_Messages, cn_Common_Funcs, ComCtrls, cxProgressBar;

type TcnWaitFormType = (wfLoadPackage,wfLocateFunction,wfSelectData,wfPrepareData);

type
  TcnWaitForm_progress = class(TForm)
    Panel: TPanel;
    Image1: TImage;
...
function ShowWaitForm(AOwner: TForm;WaitFormType:TcnWaitFormType): TForm;
var
 wf : TcnWaitForm_progress;
begin
 wf := TcnWaitForm_progress.Create(AOwner);
...
procedure TcnWaitForm_progress.FormCreate(Sender: TObject);
var WindowRGN: HRGN;
begin
 BorderStyle:=bsNone;
