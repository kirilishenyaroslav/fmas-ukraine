...
uses
  ...
  cn_DM_TypeDocum, cxStyles, cxCustomData, cxFilter, cxData, cxDataStorage, DB,
...
uses cn_sp_TypeDocum_Unit_AE;

{$R *.dfm}

constructor TfrmTypeDocum.Create(AOwner:TComponent;DB_Handle:TISC_DB_HANDLE;is_admin:boolean);
begin
Screen.Cursor:=crHourGlass;
inherited Create(AOwner);
DM:=TDM_TypeDocum.Create(Self);
DM.DataSet.SQLs.SelectSQL.Text := 'select * from CN_SP_TYPE_DOCUM_SELECT';
...
procedure TfrmTypeDocum.FormIniLanguage;
begin
 PLanguageIndex:=           cn_Common_Funcs.cnLanguageIndex();
 name.caption:=                 cnConsts.cn_Name_Column[PLanguageIndex];

 //кэпшн формы
 Caption:=                      cnConsts.cn_Main_SpTypeDocum[PLanguageIndex];
 //названия кнопок
...
procedure TfrmTypeDocum.ExitButtonClick(Sender: TObject);
begin
close;
end;

procedure TfrmTypeDocum.FormClose(Sender: TObject;
...
procedure TfrmTypeDocum.SelectButtonClick(Sender: TObject);
var id_sp: int64;
begin
Res:=VarArrayCreate([0,2],varVariant);
id_sp:= DM.DataSet['ID_TYPE_DOCUM'];
Res[0]:= id_sp;
Res[1]:=DM.DataSet['NAME'];
ModalResult:=mrOk;
end;

procedure TfrmTypeDocum.FormShow(Sender: TObject);
begin
if FormStyle = fsMDIChild then  SelectButton.Enabled:=false;
end;

procedure TfrmTypeDocum.AddButtonClick(Sender: TObject);
 var
...
ViewForm.Caption := cnConsts.cn_InsertBtn_Caption[PLanguageIndex];
 ...
 begin
     with DM.StProc do
       try
        ...
        StoredProcName := 'CN_SP_TYPE_DOCUM_INSERT';
        Prepare;
        ParamByName('NAME').AsString       := ViewForm.Name_Edit.Text;
        ExecProc;
        New_id_Locator:=ParamByName('ID_TYPE_DOCUM').AsInt64;
        Transaction.Commit;
       except
        on E:Exception do
         begin
 ...
 DM.DataSet.Locate('ID_TYPE_DOCUM',New_id_Locator,[] );
   end;
end;



procedure TfrmTypeDocum.EditButtonClick(Sender: TObject);
var
ViewForm : TfrmAddEdit;
id_Locator : Int64;
begin
...
ViewForm.Name_Edit.Text:=  DM.DataSet['NAME'];
 if ViewForm.ShowModal = mrOk then
 begin
     id_Locator:=  DM.DataSet['ID_TYPE_DOCUM'];
     with DM.StProc do
       try
        Transaction.StartTransaction;
        StoredProcName := 'CN_SP_TYPE_DOCUM_UPDATE';
        Prepare;
        ParamByName('ID_TYPE_DOCUM').AsInt64     := DM.DataSet['ID_TYPE_DOCUM'];
        ParamByName('NAME').AsString           := ViewForm.Name_Edit.Text;
        ExecProc;
        Transaction.Commit;
       except
 ...
 DM.DataSet.CloseOpen(True);
 DM.DataSet.Locate('ID_TYPE_DOCUM', id_Locator ,[] );
   end;
end;

procedure TfrmTypeDocum.DeleteButtonClick(Sender: TObject);
 var
  i: byte;
 begin
   if not Is_Admin then
 ...
 if  GridDBView.DataController.RecordCount = 0 then exit;
  ...
  begin
     with DM.StProc do
       try
        Transaction.StartTransaction;
        StoredProcName := 'CN_SP_TYPE_DOCUM_DELETE';
        Prepare;
        ParamByName('ID_TYPE_DOCUM').AsInt64       :=  DM.DataSet['ID_TYPE_DOCUM'];
        ExecProc;
        Transaction.Commit;
...
procedure TfrmTypeDocum.RefreshButtonClick(Sender: TObject);
var
id_Locator : Int64;
begin
 if  GridDBView.DataController.RecordCount = 0 then exit;
