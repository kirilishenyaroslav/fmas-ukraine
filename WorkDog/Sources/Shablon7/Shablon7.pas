...
type
  TfrmShablon7 = class(TForm)
    ...
    pFIBDataSet_kowtoris_id: TpFIBDataSet;
...
constructor TfrmShablon7.Create (Aowner:TComponent;DBHANDLE : TISC_DB_HANDLE;frmStyle:TFormStyle; id_dokument: Variant; Pr_vizova: String; Id_t_d: Variant; data : PShablonInputData = nil);
var
  i,j: Integer;
  post: Variant;
...
begin
  ...
  if Pr_vizova = 'red' then
      begin
        ...
        if cxCheckBox_not_summa.Checked = true then
        begin
...
var
  sh1: TfrmShablon7;
begin
  sh1 := TfrmShablon7.Create(AOwner,DBHANDLE,frmStyle,id_dokument,Pr_vizova,id_t_d,data);
  if sh1.Flag then
  begin
    sh1.free;
    Exit;
...
procedure TfrmShablon7.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfrmShablon7.FormCreate(Sender: TObject);
var
  reg       : TRegistry;
...
procedure TfrmShablon7.cxButton_cancelClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmShablon7.cxButtonEdit_name_pPropertiesButtonClick(
...
procedure TfrmShablon7.cxButtonEdit_name_pPropertiesChange(
  Sender: TObject);
begin
...
procedure TfrmShablon7.cxMaskEdit_n_dogPropertiesChange(Sender: TObject);
begin
  cxTextEdit_def_n_dog2.Text := cxMaskEdit_n_dog.Text;
end;


procedure TfrmShablon7.Summa();
var
  i: Integer;
  temp: Variant;
  kol_vo_smet: Integer;
...
procedure TfrmShablon7.cxButton_okClick(Sender: TObject);
var
  i,j: Integer;
  action : TResAction;
...
begin
  ...
  if cxDateEdit_dog.Text = '' then
   ...
   { if  CurrencyEditsum_dog.Value <= 0 then
        begin
    ...
    begin //проверка на договор без суммы
     ...
     //   begin
          MessageDlg('Увага', errDtSumma, mtError, [mbOk]);
          Exit;
    //    end;
      end;
    end;
...
begin
    ...
    //конец добавления смет

    pFIBTransactionWrite.Commit;
    Close;
    end
    else
/////////////////////////////////////////Редактирование шаблона 7//////////////////////////////////////////////
    begin
    action := raEdit;
    pFIBTransactionWrite.StartTransaction;
...
procedure TfrmShablon7.cxDateEdit_dogPropertiesChange(Sender: TObject);
begin
  cxTextEdit_def_d_dog2.Text := cxDateEdit_dog.Text;
end;

procedure TfrmShablon7.CurrencyEditsum_dogChange(Sender: TObject);
begin
//
end;

procedure TfrmShablon7.cxDateEdit_date_endPropertiesChange(
  Sender: TObject);
begin
      if  cxDateEdit_date_beg.Date > cxDateEdit_date_end.Date then
        begin
...
procedure TfrmShablon7.cxGrid1DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  Arect: TRect;
...
procedure TfrmShablon7.cxCheckBox_not_summaPropertiesChange(
  Sender: TObject);
begin
  ButtonEn();
  if cxCheckBox_not_summa.Checked then
...
procedure TfrmShablon7.cxButtonEdit_name_unPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
  ret: Variant;
begin
...
procedure TfrmShablon7.FormShow(Sender: TObject);
  var
  Str:String;
begin
   pFIBDataSet_id_dokument.Active := false;
...
procedure TfrmShablon7.RxMemoryData_smetAfterPost(DataSet: TDataSet);
begin
  Summa();
  ButtonEn();
end;

procedure TfrmShablon7.RxMemoryData_smetAfterDelete(DataSet: TDataSet);
begin
  Summa();
  ButtonEn();
end;

procedure TfrmShablon7.ButtonEn();
begin
{  if cxCheckBox_not_summa.Checked then
  begin
    Exit;
...
procedure TfrmShablon7.ToolButton_delClick(Sender: TObject);
var
  id_sm: Variant;
begin
...
procedure TfrmShablon7.RxMemoryData_smetAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmShablon7.FormResize(Sender: TObject);
begin
  Panel1.Left := Width div 2 - Panel1.Width div 2 - 6;
  Panel1.Top := Height div 2 - Panel1.Height div 2 - 18;
end;

procedure TfrmShablon7.Label21Click(Sender: TObject);
begin
  if Priznak_vizova <> 'add' then
  ShowDogPayFulfil(Self, pFIBDatabase.Handle, pFIBTransactionRead.Handle, pFIBTransactionWrite.Handle, fsNormal, id_doc, -1);
end;

procedure TfrmShablon7.Label21MouseMove(Sender: TObject;
...
procedure TfrmShablon7.Label22MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova <> 'add' then
  begin
...
procedure TfrmShablon7.cxButton_histClick(Sender: TObject);
var
  Hist: Variant;
  Query: TpFIBDataSet;
begin
...
procedure TfrmShablon7.addExecute(Sender: TObject);
begin
  if PageControl1.ActivePageIndex = 1 then  ToolButton_addClick(Self);
end;

procedure TfrmShablon7.delExecute(Sender: TObject);
begin
  if PageControl1.ActivePageIndex = 1 then ToolButton_delClick(Self);
end;

procedure TfrmShablon7.cxGrid1DBTableView1KeyDown(Sender: TObject;
...
procedure TfrmShablon7.saveExecute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon7.save1Execute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon7.insExecute(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon7.CurrencyEdit_sum_po_smetChange(Sender: TObject);
begin
  if Priznak_vizova = 'add' then
  CurrencyEditsum_dog.Value := CurrencyEdit_sum_po_smet.Value;
end;

procedure TfrmShablon7.Label16Click(Sender: TObject);
var
  Ret : VAriant;
begin
...
procedure TfrmShablon7.Label16MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova = 'red' then
  begin
...
procedure TfrmShablon7.SumToSmet();
var
...
procedure TfrmShablon7.SpeedButton_redClick(Sender: TObject);
var
  Add: Variant;
  d_dogovor: TDate;
  cnt, i: Integer;
...
procedure TfrmShablon7.N3Click(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon7.editExecute(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon7.Label5Click(Sender: TObject);
var
  s       : Variant;
begin
...
procedure TfrmShablon7.cxButtonEdit_depPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
    sp: TSprav;
begin
...
procedure TfrmShablon7.cxButton1Click(Sender: TObject);
var
 i, o : TAArray;
begin
