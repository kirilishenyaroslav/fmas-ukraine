...
//Данные по счетам ДонНУ и  з моего справочника
...
{
 Добавление кошториса
 Изменено: Кошториси с одинаковым кекв, статьёй, разделом суммируются
 Автор изменений: Герасименко Роман
...
//      tmp_val:=RxMemoryData_smet.FieldByName('sum_smet').Value;
      ...
      begin
      { Добавление кошториса в датасет }
      for i := 0 to Cnt do
      begin
        id_smeta := Add[i][7];
...
//   field :=0;
   ...
   begin
     ...
     ShowMessage('Рахунок контрагента не функцінує в даному періоді');
...
{    if (cxDateEdit_dog.Date > cxDateEdit_krit_opl.Date) then
    ...
    begin
      ...
      begin
         ShowMessage('Некоректно задана сума до оплати.');
        CurrencyEdit_opl.SetFocus;
...
var
  s       : Variant;
begin
{  s := InputBox('Коментарі', 'Введіть коментарі', comment);
  comment := s;}
