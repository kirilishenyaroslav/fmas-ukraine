...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, BaseTypes, cxControls, cxContainer, cxEdit,
  cxTextEdit, cxMaskEdit, cxButtonEdit, cxLookAndFeelPainters, cxButtons,
  cxRadioGroup, FIBQuery, pFIBQuery, ActnList, cxCheckBox, FIBDatabase,
  pFIBDatabase;
...
{$R *.DFM}
uses sp_Object_Form, Variants, uCommonSp;
...
var
    sp: TSprav;
begin
    sp :=  GetSprav('Public\SpDepartment');
    if sp <> nil then
    begin
        with sp.Input do
        begin
...
begin
 if LevelRadioGroup.ItemIndex = 0 then begin
  if (Owner as Tsp_Object_Form1).TreeList.FocusedNode.Level = 0 then begin
   KindRadioGroup.Enabled := False;
   KindRadioGroup.ItemIndex := 0;
  end;
  if ((Owner as Tsp_Object_Form1).TreeList.FocusedNode.Parent <> nil) then begin
   if not VarIsNull((Owner as Tsp_Object_Form1).TreeList.FocusedNode.Parent.Values[1]) then begin
    KindRadioGroup.Enabled := False;
 ...
 if LevelRadioGroup.ItemIndex = 1 then begin
  ...
  if not VarIsNull((Owner as Tsp_Object_Form1).TreeList.FocusedNode.Values[1]) then begin
   KindRadioGroup.Enabled := False;
   KindRadioGroup.ItemIndex := 1;
  end;
 end;

 if not VarIsNull((Owner as Tsp_Object_Form1).TreeList.FocusedNode.Values[1]) and HasPlanCh then begin
...
begin
 if KindRadioGroup.ItemIndex = 0 then begin
  if ((Owner as Tsp_Object_Form1).TreeList.FocusedNode <> nil) then
   if not VarIsNull((Owner as Tsp_Object_Form1).TreeList.FocusedNode.Values[1]) then begin
    LevelRadioGroup.Enabled := False;
    LevelRadioGroup.ItemIndex := 0;
   end;
