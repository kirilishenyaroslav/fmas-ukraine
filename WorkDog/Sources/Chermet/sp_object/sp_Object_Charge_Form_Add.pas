...
uses
  ...
  StdCtrls, ExtCtrls, Buttons, ComCtrls, BaseTypes, Sys_Options,
...
uses sp_Object_Form, cxCustomData;
...
begin
 ...
 if not State then begin
  if (Owner as Tsp_Object_Form1).StringGrid1.DataController.RecordCount > 0 then begin
   min_date := StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[0, 4]);
   for i := 0 to (Owner as Tsp_Object_Form1).StringGrid1.DataController.RecordCount - 1 do begin
    if min_date < StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[i, 4]) then
    min_date := StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[i, 4]);
   end;
  end
  else min_date := SYS_MIN_TIMESTAMP;
 end
 else begin
  if (Owner as Tsp_Object_Form1).StringGrid1.DataController.FocusedRowIndex <> 0 then begin
   min_date := StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[0, 4]);
   for i := 0 to (Owner as Tsp_Object_Form1).StringGrid1.DataController.FocusedRowIndex - 1 do begin
    if min_date < StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[i, 4]) then
    min_date := StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[i, 4]);
   end;
  end
 ...
 if State
 and ((Owner as Tsp_Object_Form1).StringGrid1.DataController.FocusedRowIndex < (Owner as Tsp_Object_Form1).StringGrid1.DataController.RecordCount - 1)
 and (StrToDate((Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[(Owner as Tsp_Object_Form1).StringGrid1.DataController.FocusedRowIndex + 1, 3]) <=
 DateTimePicker2.Date) then begin
  ShowMessage('Дата закінчення повинна бути менше ніж '
  + (Owner as Tsp_Object_Form1).StringGrid1.DataController.Values[(Owner as Tsp_Object_Form1).StringGrid1.DataController.FocusedRowIndex + 1, 3]);
  Exit;
 end;
