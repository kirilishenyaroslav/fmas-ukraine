...
uses
  ...
  dxStatusBar,uCommonSp,AArray,LoaderUnit,ErrSchFrm5;
...
type
  TfrmShablon5 = class(TForm)
    ...
    Label25: TLabel;
    Label26: TLabel;
    CurrencyEdit_nds_opl: TCurrencyEdit;
...
constructor TfrmShablon5.Create(Aowner:TComponent ;DBHANDLE : TISC_DB_HANDLE;frmStyle:TFormStyle; id_dokument: Variant; Pr_vizova: String; Id_t_d: Variant; data : PShablonInputData = nil);
var
  i: Integer;
  nds_s : Currency;
  post: Variant;
...
//        ShowMessage(IntToStr(id_class));
        ...
        if Varisnull(pFIBDataSet_tip_dog.FieldByName('id_tip_dog').AsVariant) then
           id_tip_dog := pFIBDataSet_tip_dog.FieldByName('id_tip_dog').AsVariant
        else
          id_tip_dog := 0;
...
var
  sh5: TfrmShablon5;
begin
  sh5 := TfrmShablon5.Create(AOwner, DBHANDLE, frmStyle, id_dokument, Pr_vizova, id_t_d,data);
  if sh5.Flag then
  begin
    sh5.free;
    Exit;
...
procedure TfrmShablon5.FormClose(Sender: TObject;
...
procedure TfrmShablon5.cxButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  post: Variant;
begin
...
procedure TfrmShablon5.FormCreate(Sender: TObject);
var
  reg       : TRegistry;
...
procedure TfrmShablon5.cxButton_cancelClick(Sender: TObject);
begin
...
procedure TfrmShablon5.cxButtonEdit_name_unPropertiesButtonClick(
...
procedure TfrmShablon5.RxMemoryData_smetAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmShablon5.ToolButton_addClick(Sender: TObject);
var
  Add: Variant;
  cnt,i: Integer;
...
procedure TfrmShablon5.cxButton_okClick(Sender: TObject);
var
  i, j: Integer;
  nds, sum_pay: Variant;
...
begin
  ...
  if cxDateEdit_dog.Text = '' then
   begin
     ...
     // ------------------------------------------------------------------------
     //Пробная проверка на соответствие бюджетов нейтив счёта
//      StorProc := TpFIBStoredProc.Create(Self);
//      StorProc.Database := pFIBDatabase;
//      StorProc.Transaction := pFIBTransactionRead;
//      StorProc.Transaction.StartTransaction;
//
//      StorProc.StoredProcName:='BU_GET_ACC_BY_SM_PARAMS';
//      StorProc.ParamByName('ActualDate').AsDate:=now;
//      StorProc.ParamByName('ID_SMETA').AsInteger:=1;
//      StorProc.ExecProc;
//      ShowMessage(StorProc.FldByName['ID_RATE_ACCOUNT'].AsString);
//
//
//      if(StorProc.RecordCount <> 0) then
//      begin
//       ShowMessage('SomeThing Found)');
//      end;
...
//      StorProc.Transaction.Commit;
  ...
  {   if Priznak_vizova = 'add' then
     begin
     action := raAdd;
      if (cxDateEdit_dog.Date - 0.1 > Date) then
      begin
    ...
    if (s_doc*100 - (sum_k_opl*100 + s_pay*100))<0  then
    begin
        ShowMessage('Сума кошторису + сума оплачена не дорівнює сумі документу!');
...
{    if (cxDateEdit_dog.Date > cxDateEdit_krit_opl.Date) then
  ...
  //Проверка на корректность суммы к оплате
    ...
    begin
      ...
      begin
         ShowMessage('Некоректно задана сума до cплати!');
        CurrencyEdit_opl.SetFocus;
    ...
    begin
      ...
      if nds > sum_doc then
      begin
        ShowMessage('ПДВ не може превищувати суму документу.');
...
procedure TfrmShablon5.cxMaskEdit_n_dogKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmShablon5.SummaDoc();
var
  temp: Variant;
  i: Integer;
begin
...
procedure TfrmShablon5.RxMemoryData_smetAfterDelete(DataSet: TDataSet);
begin
  Summa();
//  SummaDoc();
//  SumNds();
...
procedure TfrmShablon5.RxMemoryData_smetAfterPost(DataSet: TDataSet);
begin
  Summa();
//  SummaDoc();
//  SumNds();
...
procedure TfrmShablon5.cxGrid1DBTableView1KeyDown(Sender: TObject;
...
procedure TfrmShablon5.cxGrid1DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  Arect: TRect;
...
procedure TfrmShablon5.ToolButton_delClick(Sender: TObject);
var
  id_sm: Variant;
begin
...
procedure TfrmShablon5.CurrencyEdit_sum_po_smetChange(Sender: TObject);
begin
  if Priznak_vizova = 'add' then
  CurrencyEdit_summa.Text:=CurrencyEdit_sum_po_smet.Text;
end;

procedure TfrmShablon5.CurrencyEdit_oplChange(Sender: TObject);
var
  sum_d: Currency;
begin
{  if CurrencyEdit_summa.Value < CurrencyEdit_opl.Value then
  ...
  if nds_procent<=0 then nds_procent := 20;
  sum_nds := (sum_d*nds_procent)/(100+nds_procent);
  CurrencyEdit_nds_opl.Text := VarToStr(sum_nds);
end;



procedure TfrmShablon5.CurrencyEdit_ndsExit(Sender: TObject);
var
  nds: Variant;
begin
{  nds:=StrToCurr(CurrencyEdit_nds.Text);
...
procedure TfrmShablon5.CurrencyEdit_oplExit(Sender: TObject);
var
  sum_k_opl: Currency;
  sum_pay: Currency;
  sum_doc: Currency;
...
begin
  ...
  //ShowMessage(CurrToStr(temp));
  //ShowMessage(CurrToStr(sum_k_opl));
...
procedure TfrmShablon5.Label15Click(Sender: TObject);
begin
  if Priznak_vizova <> 'add' then
  ShowDogPayFulfil(Self, pFIBDatabase.Handle, pFIBTransactionRead.Handle, pFIBTransactionWrite.Handle, fsNormal, id_doc, -1);
end;

procedure TfrmShablon5.Label15MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova <> 'add' then
  begin
...
procedure TfrmShablon5.add_smetExecute(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon5.del_smetExecute(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon5.N1Click(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon5.N2Click(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon5.FormShow(Sender: TObject);
  var
  Str:string;
begin
   pFIBDataSet_id_dokument.Active := false;
...
procedure TfrmShablon5.Label17MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova = 'red' then
  begin
...
procedure TfrmShablon5.Label17Click(Sender: TObject);
var
  Ret : VAriant;
begin
...
procedure TfrmShablon5.FormResize(Sender: TObject);
begin
  Panel1.Left := Width div 2 - Panel1.Width div 2 - 6;
  Panel1.Top := Height div 2 - Panel1.Height div 2 - 18;
end;

procedure TfrmShablon5.cxButton_histClick(Sender: TObject);
var
  Hist: Variant;
  Query: TpFIBDataSet;
begin
...
procedure TfrmShablon5.saveExecute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon5.save1Execute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon5.insExecute(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon5.Label1Click(Sender: TObject);
var
  Ret : VAriant;
begin
  if Priznak_vizova <> 'red' then Exit;
...
procedure TfrmShablon5.Label1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if Priznak_vizova = 'red' then
  begin
...
procedure TfrmShablon5.SpeedButton_redClick(Sender: TObject);
var
  Add: Variant;
  d_dogovor: TDate;
  cnt, i: Integer;
...
procedure TfrmShablon5.SumToSmet();
var
...
procedure TfrmShablon5.N3Click(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon5.editExecute(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon5.Label12Click(Sender: TObject);
var
  s       : Variant;
begin
...
procedure TfrmShablon5.cxButtonEdit_depPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
    sp: TSprav;
begin
...
procedure TfrmShablon5.delExecute(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon5.cxButton1Click(Sender: TObject);
var
 i, o : TAArray;
begin
...
// -----------------------------------------------------------------------------
function TfrmShablon5.DoCheckBgt(id_smeta:integer;rate_acc_native:integer):boolean;
var
 i,j,k,f : integer;
 field : integer;
...
begin
 ...
 if((DataSetCheck.RecordCount <> 0 )and (rate_acc_native<>0)) then
  begin
   ...
   begin
   f:=DataSetCheck.FieldByName('ID_RATE_ACCOUNT').AsInteger;
    if (f = rate_acc_native) then
    begin
   ...
   if (field = 0 ) then begin
    ...
    errorFrm:=TErrSchFrm.Create(self);
...
procedure TfrmShablon5.ClassEditPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
 i, o : TAArray;
begin
