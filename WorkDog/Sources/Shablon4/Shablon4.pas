...
uses
  ...
  dxStatusBar,uCommonSp,AArray,LoaderUnit,ErrFormSch4;
...
var
  frmShablon4: TfrmShablon4;
  errorFrm : TErrFormSh4;
implementation
uses
  Registry;
...
constructor TfrmShablon4.Create(Aowner:TComponent ;DBHANDLE : TISC_DB_HANDLE;frmStyle:TFormStyle; id_dokument: Variant; Pr_vizova: String;
...
//  data.bUse := true;
   ...
   //если прислана сумма и стоят параметры по умолчанию, то записываем все в грид
     if data^.summa > 0 then
     begin
       if (pFIBDataSet_id_dokument.FBN('use_def_values').AsInteger=1) and
          (pFIBDataSet_id_dokument.FBN('use_def_kekv').AsInteger=1) then
         begin
           RxMemoryData_smet.Open;
           RxMemoryData_smet.Insert;
           RxMemoryData_smet.FieldByName('id_smet').Value := pFIBDataSet_id_dokument.FBN('def_smeta').AsInteger;
           RxMemoryData_smet.FieldByName('id_razd').Value := pFIBDataSet_id_dokument.FBN('def_razd').AsInteger;
           RxMemoryData_smet.FieldByName('id_stat').Value := pFIBDataSet_id_dokument.FBN('def_stat').AsInteger;
           RxMemoryData_smet.FieldByName('sum_smet').Value := data^.summa;
           RxMemoryData_smet.FieldByName('name_smet').Value := pFIBDataSet_id_dokument.FBN('smeta_title').AsString;
           RxMemoryData_smet.FieldByName('name_razd').Value := pFIBDataSet_id_dokument.FBN('razd_title').AsString;
           RxMemoryData_smet.FieldByName('name_stat').Value := pFIBDataSet_id_dokument.FBN('st_title').AsString;
           RxMemoryData_smet.FieldByName('kod_smety').Value := pFIBDataSet_id_dokument.FBN('smeta_kod').AsInteger;
           RxMemoryData_smet.FieldByName('n_razd').Value := pFIBDataSet_id_dokument.FBN('razd_kod').AsInteger;
           RxMemoryData_smet.FieldByName('n_stat').Value := pFIBDataSet_id_dokument.FBN('st_kod').AsInteger;
           RxMemoryData_smet.FieldByName('id_kekv').Value := pFIBDataSet_id_dokument.FBN('def_kekv').AsInteger;
           RxMemoryData_smet.FieldByName('kod_kekv').Value := pFIBDataSet_id_dokument.FBN('kekv_kode').AsInteger;
           RxMemoryData_smet.FieldByName('name_kekv').Value := pFIBDataSet_id_dokument.FBN('kekv_title').AsString;
           RxMemoryData_smet.FieldByName('id').Value := 0;
           RxMemoryData_smet.FieldByName('flag_del').Value := 0;
           RxMemoryData_smet.Post;
           CurrencyEdit_summa.Value := data^.summa;
           SumNds();
         end;
     end;
...
{Если нет контрагента по умолчанию, то выдается сразу справочник на выбор контрагнета}
 ...
 if (rate_acc_p=Null) then
  begin
    ...
    t['DataBase']     := Integer(pFIBDatabase.Handle);
...
var
  sh5: TfrmShablon4;
begin
  sh5 := TfrmShablon4.Create(AOwner, DBHANDLE, frmStyle, id_dokument, Pr_vizova, id_t_d, data);
  if sh5.Flag then
  begin
    sh5.free;
    Exit;
...
procedure TfrmShablon4.FormClose(Sender: TObject;
...
procedure TfrmShablon4.cxButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  post: Variant;
  i, o : TSpravParams;
...
procedure TfrmShablon4.FormCreate(Sender: TObject);
var
  reg       : TRegistry;
...
procedure TfrmShablon4.cxButton_cancelClick(Sender: TObject);
begin
...
procedure TfrmShablon4.cxButtonEdit_name_unPropertiesButtonClick(
...
procedure TfrmShablon4.RxMemoryData_smetAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmShablon4.ToolButton_addClick(Sender: TObject);
var
  Add: Variant;
  cnt,i: Integer;
  tmp_val:double;
...
procedure TfrmShablon4.cxButton_okClick(Sender: TObject);
var
  i, j: Integer;
  nds, sum_pay: Variant;
...
begin
  ...
  if cxDateEdit_dog.Text = '' then
   begin
     ...
     ShowMessage('Рахунок контрагента не функціонує в даному періоді');
     DataSet_rate.Free;
...
procedure TfrmShablon4.cxMaskEdit_n_dogKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmShablon4.SummaDoc();
var
  temp: Variant;
  i: Integer;
begin
...
procedure TfrmShablon4.RxMemoryData_smetAfterDelete(DataSet: TDataSet);
begin
  Summa();
end;

procedure TfrmShablon4.RxMemoryData_smetAfterPost(DataSet: TDataSet);
begin
  Summa();
end;

procedure TfrmShablon4.SumNds();
var
  sum_d: Currency;
begin
...
procedure TfrmShablon4.cxGrid1DBTableView1KeyDown(Sender: TObject;
...
procedure TfrmShablon4.cxGrid1DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  Arect: TRect;
...
procedure TfrmShablon4.ToolButton_delClick(Sender: TObject);
var
  id_sm: Variant;
begin
...
procedure TfrmShablon4.CurrencyEdit_sum_po_smetChange(Sender: TObject);
begin
if Priznak_vizova = 'add' then
  CurrencyEdit_summa.Text:=CurrencyEdit_sum_po_smet.Text;
end;

procedure TfrmShablon4.Label14Click(Sender: TObject);
begin
  if Priznak_vizova <> 'add' then
  ShowDogPayFulfil(Self, pFIBDatabase.Handle, pFIBTransactionRead.Handle, pFIBTransactionWrite.Handle, fsNormal, id_doc, -1);
end;

procedure TfrmShablon4.Label14MouseMove(Sender: TObject;
...
procedure TfrmShablon4.Label15Click(Sender: TObject);
begin
  if Priznak_vizova <> 'add' then
  ShowDogPayFulfil(Self, pFIBDatabase.Handle, pFIBTransactionRead.Handle, pFIBTransactionWrite.Handle, fsNormal, id_doc, -1);
end;

procedure TfrmShablon4.Label15MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova <> 'add' then
  begin
...
procedure TfrmShablon4.add_smetExecute(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon4.del_smetExecute(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon4.N1Click(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon4.N2Click(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon4.Label17MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if Priznak_vizova = 'red' then
  begin
...
procedure TfrmShablon4.FormShow(Sender: TObject);
  var
  Str:String;
begin
   pFIBDataSet_id_dokument.Active := false;
...
procedure TfrmShablon4.FormResize(Sender: TObject);
begin
  Panel1.Left := Width div 2 - Panel1.Width div 2 - 6;
  Panel1.Top := Height div 2 - Panel1.Height div 2 - 18;
end;

procedure TfrmShablon4.cxButton_histClick(Sender: TObject);
var
  Hist: Variant;
  Query: TpFIBDataSet;
begin
...
procedure TfrmShablon4.saveExecute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon4.save1Execute(Sender: TObject);
begin
  cxButton_okClick(Self);
end;

procedure TfrmShablon4.insExecute(Sender: TObject);
begin
  ToolButton_addClick(Self);
end;

procedure TfrmShablon4.Label17Click(Sender: TObject);
var
  Ret : VAriant;
begin
  if Priznak_vizova <> 'red' then Exit;
  if not pFIBDataSet_id_dokument.Active then ShowMessage('no active');
  if pFIBDataSet_id_dokument.RecordCount = 0 then ShowMessage('no rec');
...
procedure TfrmShablon4.Label1Click(Sender: TObject);
var
  Ret : VAriant;
begin
  if Priznak_vizova <> 'red' then Exit;
...
procedure TfrmShablon4.Label1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if Priznak_vizova = 'red' then
  begin
...
procedure TfrmShablon4.SumToSmet();
var
...
procedure TfrmShablon4.SpeedButton_redClick(Sender: TObject);
var
  Add: Variant;
  d_dogovor: TDate;
  cnt, i: Integer;
...
procedure TfrmShablon4.editExecute(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon4.N3Click(Sender: TObject);
begin
  SpeedButton_redClick(Self);
end;

procedure TfrmShablon4.Label12Click(Sender: TObject);
var
  s       : Variant;
begin
...
procedure TfrmShablon4.cxButtonEdit_depPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
    sp: TSprav;
begin
...
procedure TfrmShablon4.delExecute(Sender: TObject);
begin
  ToolButton_delClick(Self);
end;

procedure TfrmShablon4.cxButton1Click(Sender: TObject);
var
 i, o : TAArray;
begin
...
// -----------------------------------------------------------------------------
function TfrmShablon4.DoCheckBgt(id_smeta:integer;rate_acc_native:integer):boolean;
var
 i,j,k : integer;
 field : integer;
 ...
 if((DataSetCheck.RecordCount <> 0)and(rate_acc_native<>0)) then
  begin
   ...
   if (field = 0 ) then begin
    ...
    errorFrm:=TErrFormSh4.Create(self);
...
procedure TfrmShablon4.ClassEditPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
 i, o : TAArray;
begin
