object MatSchExForm: TMatSchExForm
  Left = 299
  Top = 166
  Width = 570
  Height = 400
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object TopPanel: TPanel
    Left = 0
    Top = 0
    Width = 562
    Height = 51
    Align = alTop
    TabOrder = 1
    object OkButton: TSpeedButton
      Left = 441
      Top = 0
      Width = 60
      Height = 50
      Caption = #1042#1099#1073#1088#1072#1090#1100
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = OkButtonClick
    end
    object RefreshButton: TSpeedButton
      Left = 4
      Top = 0
      Width = 60
      Height = 50
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        8176B88285FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      OnClick = RefreshButtonClick
    end
    object CancelButton: TSpeedButton
      Left = 501
      Top = 0
      Width = 60
      Height = 50
      Caption = #1047#1072#1082#1088#1099#1090#1100
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      OnClick = CancelButtonClick
    end
    object LabelPeriod: TLabel
      Left = 88
      Top = 18
      Width = 68
      Height = 13
      Caption = 'LabelPeriod'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object cxMonth: TcxComboBox
      Left = 168
      Top = 14
      Width = 97
      Height = 21
      ParentFont = False
      Properties.DropDownListStyle = lsFixedList
      Properties.DropDownRows = 12
      Properties.Items.Strings = (
        #1103#1085#1074#1072#1088#1100
        #1092#1077#1074#1088#1072#1083#1100
        #1084#1072#1088#1090
        #1072#1087#1088#1077#1083#1100
        #1084#1072#1081
        #1080#1102#1085#1100
        #1080#1102#1083#1100
        #1072#1074#1075#1091#1089#1090
        #1089#1077#1085#1090#1103#1073#1088#1100
        #1086#1082#1090#1103#1073#1088#1100
        #1085#1086#1103#1073#1088#1100
        #1076#1077#1082#1072#1073#1088#1100)
      Properties.ReadOnly = False
      Style.Color = clInfoBk
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -11
      Style.Font.Name = 'MS Sans Serif'
      Style.Font.Style = [fsBold]
      TabOrder = 0
    end
    object cxYear: TcxSpinEdit
      Left = 272
      Top = 14
      Width = 57
      Height = 21
      ParentFont = False
      Style.Color = clInfoBk
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -11
      Style.Font.Name = 'MS Sans Serif'
      Style.Font.Style = [fsBold]
      TabOrder = 1
    end
  end
  object cxDBTreeList1: TcxDBTreeList
    Left = 0
    Top = 51
    Width = 562
    Height = 322
    Styles.Background = cxStyle2
    Styles.Content = cxStyle2
    Styles.ColumnHeader = cxStyle11
    Align = alClient
    Bands = <
      item
        Caption.Text = 'Band1'
        Width = 1900
      end>
    BufferedPaint = False
    DataController.DataSource = MatSchDataSource
    DataController.ParentField = 'LINK_TO'
    DataController.KeyField = 'ID_SCH'
    OptionsBehavior.IncSearchItem = cxDBTreeList1SCH_NUMBER
    OptionsBehavior.MultiSort = False
    OptionsBehavior.IncSearch = True
    OptionsSelection.CellSelect = False
    OptionsView.ColumnAutoWidth = True
    OptionsView.GridLines = tlglBoth
    RootValue = -1
    TabOrder = 0
    OnDblClick = cxDBTreeList1DblClick
    OnKeyDown = cxDBTreeList1KeyDown
    object cxDBTreeList1ID_SCH: TcxDBTreeListColumn
      Caption.Text = 'ID_SCH'
      DataBinding.FieldName = 'ID_SCH'
      Position.ColIndex = 2
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1SCH_NUMBER: TcxDBTreeListColumn
      Caption.Text = 'SCH_NUMBER'
      DataBinding.FieldName = 'SCH_NUMBER'
      Position.ColIndex = 0
      Position.RowIndex = 0
      Position.BandIndex = 0
      SortOrder = soAscending
      Width = 99
    end
    object cxDBTreeList1SCH_TITLE: TcxDBTreeListColumn
      Caption.Text = 'SCH_TITLE'
      DataBinding.FieldName = 'SCH_TITLE'
      Position.ColIndex = 1
      Position.RowIndex = 0
      Position.BandIndex = 0
      Width = 461
    end
    object cxDBTreeList1SCH_TYPE: TcxDBTreeListColumn
      Caption.Text = 'SCH_TYPE'
      DataBinding.FieldName = 'SCH_TYPE'
      Position.ColIndex = 3
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1LINK_TO: TcxDBTreeListColumn
      Caption.Text = 'LINK_TO'
      DataBinding.FieldName = 'LINK_TO'
      Position.ColIndex = 4
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1DATE_BEG: TcxDBTreeListColumn
      Caption.Text = 'DATE_BEG'
      DataBinding.FieldName = 'DATE_BEG'
      Position.ColIndex = 5
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1DATE_END: TcxDBTreeListColumn
      Caption.Text = 'DATE_END'
      DataBinding.FieldName = 'DATE_END'
      Position.ColIndex = 6
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1TYPE_OBJECT: TcxDBTreeListColumn
      Caption.Text = 'TYPE_OBJECT'
      DataBinding.FieldName = 'TYPE_OBJECT'
      Position.ColIndex = 7
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1SCH_OBJ_TYPE_TITLE: TcxDBTreeListColumn
      Caption.Text = 'SCH_OBJ_TYPE_TITLE'
      DataBinding.FieldName = 'SCH_OBJ_TYPE_TITLE'
      Position.ColIndex = 8
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1SCH_TYPE_TITLE: TcxDBTreeListColumn
      Caption.Text = 'SCH_TYPE_TITLE'
      DataBinding.FieldName = 'SCH_TYPE_TITLE'
      Position.ColIndex = 9
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1ID_SYSTEM: TcxDBTreeListColumn
      Caption.Text = 'ID_SYSTEM'
      DataBinding.FieldName = 'ID_SYSTEM'
      Position.ColIndex = 10
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1BAL_ID_SCH: TcxDBTreeListColumn
      Caption.Text = 'BAL_ID_SCH'
      DataBinding.FieldName = 'BAL_ID_SCH'
      Position.ColIndex = 11
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1BAL_SCH_NUMBER: TcxDBTreeListColumn
      Caption.Text = 'BAL_SCH_NUMBER'
      DataBinding.FieldName = 'BAL_SCH_NUMBER'
      Position.ColIndex = 12
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1BAL_SCH_TITLE: TcxDBTreeListColumn
      Caption.Text = 'BAL_SCH_TITLE'
      DataBinding.FieldName = 'BAL_SCH_TITLE'
      Position.ColIndex = 13
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1SYSTEM_TEXT: TcxDBTreeListColumn
      Caption.Text = 'SYSTEM_TEXT'
      DataBinding.FieldName = 'SYSTEM_TEXT'
      Position.ColIndex = 14
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1KOD_SYSTEM: TcxDBTreeListColumn
      Caption.Text = 'KOD_SYSTEM'
      DataBinding.FieldName = 'KOD_SYSTEM'
      Position.ColIndex = 15
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1IS_BLOCKED: TcxDBTreeListColumn
      Caption.Text = 'IS_BLOCKED'
      DataBinding.FieldName = 'IS_BLOCKED'
      Position.ColIndex = 16
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1OPEN_SCH: TcxDBTreeListColumn
      Caption.Text = 'OPEN_SCH'
      DataBinding.FieldName = 'OPEN_SCH'
      Position.ColIndex = 17
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
    object cxDBTreeList1ID_REG_UCH: TcxDBTreeListColumn
      Caption.Text = 'ID_REG_UCH'
      DataBinding.FieldName = 'ID_REG_UCH'
      Position.ColIndex = 18
      Position.RowIndex = 0
      Position.BandIndex = 0
      Visible = False
    end
  end
  object fdbMatSch: TpFIBDatabase
    DBParams.Strings = (
      'user_name=SYSDBA'
      'password=masterkey'
      'lc_ctype=WIN1251')
    DefaultTransaction = ReadTransaction
    DefaultUpdateTransaction = WriteTransaction
    SQLDialect = 3
    Timeout = 0
    WaitForRestoreConnect = 0
    Left = 16
    Top = 144
  end
  object ReadTransaction: TpFIBTransaction
    DefaultDatabase = fdbMatSch
    TimeoutAction = TACommit
    Left = 56
    Top = 128
  end
  object WriteTransaction: TpFIBTransaction
    DefaultDatabase = fdbMatSch
    TimeoutAction = TARollback
    TRParams.Strings = (
      'write'
      'nowait'
      'rec_version'
      'read_committed')
    TPBMode = tpbDefault
    Left = 56
    Top = 160
  end
  object MatSchDataSet: TpFIBDataSet
    Database = fdbMatSch
    Transaction = ReadTransaction
    SelectSQL.Strings = (
      'SELECT * FROM MAT_SP_SCH_SELECT_EX(:DATE)'
      ' ORDER BY SCH_NUMBER'
      '')
    Left = 96
    Top = 160
    poSQLINT64ToBCD = True
    object MatSchDataSetID_SCH: TFIBIntegerField
      FieldName = 'ID_SCH'
    end
    object MatSchDataSetSCH_TITLE: TFIBStringField
      FieldName = 'SCH_TITLE'
      Size = 60
      EmptyStrToNull = True
    end
    object MatSchDataSetSCH_NUMBER: TFIBStringField
      FieldName = 'SCH_NUMBER'
      Size = 30
      EmptyStrToNull = True
    end
    object MatSchDataSetSCH_TYPE: TFIBIntegerField
      FieldName = 'SCH_TYPE'
    end
    object MatSchDataSetLINK_TO: TFIBIntegerField
      FieldName = 'LINK_TO'
    end
    object MatSchDataSetDATE_BEG: TFIBDateTimeField
      FieldName = 'DATE_BEG'
    end
    object MatSchDataSetDATE_END: TFIBDateTimeField
      FieldName = 'DATE_END'
    end
    object MatSchDataSetTYPE_OBJECT: TFIBIntegerField
      FieldName = 'TYPE_OBJECT'
    end
    object MatSchDataSetSCH_OBJ_TYPE_TITLE: TFIBStringField
      FieldName = 'SCH_OBJ_TYPE_TITLE'
      Size = 30
      EmptyStrToNull = True
    end
    object MatSchDataSetSCH_TYPE_TITLE: TFIBStringField
      FieldName = 'SCH_TYPE_TITLE'
      Size = 30
      EmptyStrToNull = True
    end
    object MatSchDataSetID_SYSTEM: TFIBIntegerField
      FieldName = 'ID_SYSTEM'
    end
    object MatSchDataSetBAL_ID_SCH: TFIBIntegerField
      FieldName = 'BAL_ID_SCH'
    end
    object MatSchDataSetBAL_SCH_NUMBER: TFIBStringField
      FieldName = 'BAL_SCH_NUMBER'
      Size = 30
      EmptyStrToNull = True
    end
    object MatSchDataSetBAL_SCH_TITLE: TFIBStringField
      FieldName = 'BAL_SCH_TITLE'
      Size = 60
      EmptyStrToNull = True
    end
    object MatSchDataSetSYSTEM_TEXT: TFIBStringField
      FieldName = 'SYSTEM_TEXT'
      Size = 100
      EmptyStrToNull = True
    end
    object MatSchDataSetKOD_SYSTEM: TFIBIntegerField
      FieldName = 'KOD_SYSTEM'
    end
    object MatSchDataSetIS_BLOCKED: TFIBIntegerField
      FieldName = 'IS_BLOCKED'
    end
    object MatSchDataSetOPEN_SCH: TFIBDateField
      FieldName = 'OPEN_SCH'
    end
    object MatSchDataSetID_REG_UCH: TFIBIntegerField
      FieldName = 'ID_REG_UCH'
    end
    object MatSchDataSetTIP_SCH: TFIBIntegerField
      FieldName = 'TIP_SCH'
    end
  end
  object MatSchDataSource: TDataSource
    DataSet = MatSchDataSet
    Left = 128
    Top = 160
  end
  object StyleRepository: TcxStyleRepository
    Left = 168
    Top = 72
    object cxStyle1: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle3: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle4: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle5: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clGradientInactiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle6: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clNavy
    end
    object cxStyle7: TcxStyle
      AssignedValues = [svColor]
      Color = 14872561
    end
    object cxStyle8: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 4707838
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle9: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 12937777
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object cxStyle10: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle11: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 4707838
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle12: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle13: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object cxStyle14: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 12937777
      TextColor = clWhite
    end
    object GridTableViewStyleSheetDevExpress: TcxGridTableViewStyleSheet
      Caption = 'DevExpress'
      Styles.Background = cxStyle1
      Styles.Content = cxStyle2
      Styles.ContentEven = cxStyle3
      Styles.ContentOdd = cxStyle4
      Styles.Inactive = cxStyle10
      Styles.IncSearch = cxStyle11
      Styles.Selection = cxStyle14
      Styles.FilterBox = cxStyle5
      Styles.Footer = cxStyle6
      Styles.Group = cxStyle7
      Styles.GroupByBox = cxStyle8
      Styles.Header = cxStyle9
      Styles.Indicator = cxStyle12
      Styles.Preview = cxStyle13
      BuiltIn = True
    end
  end
  object PubSchDataSet: TpFIBDataSet
    Database = fdbMatSch
    Transaction = ReadTransaction
    SelectSQL.Strings = (
      'SELECT'
      '    P.ID_SCH,'
      '    P.SCH_TITLE,'
      '    P.SCH_NUMBER,'
      '    P.SCH_TYPE,'
      '    P.LINK_TO,'
      '    P.DATE_BEG,'
      '    P.DATE_END,'
      '    P.TYPE_OBJECT,'
      '    P.SCH_OBJ_TYPE_TITLE,'
      '    P.SCH_TYPE_TITLE,'
      '    P.ID_SYSTEM,'
      '    P.SYSTEM_TEXT'
      'FROM PUB_SP_MAIN_SCH_SELECT(:P_DATE, 1) P'
      
        'WHERE NOT EXISTS(SELECT * FROM PUB_SP_MAIN_SCH S WHERE S.LINK_TO' +
        '=P.ID_SCH);'
      '')
    Left = 96
    Top = 216
    poSQLINT64ToBCD = True
    object PubSchDataSetID_SCH: TFIBIntegerField
      FieldName = 'ID_SCH'
    end
    object PubSchDataSetSCH_TITLE: TFIBStringField
      FieldName = 'SCH_TITLE'
      Size = 60
      EmptyStrToNull = True
    end
    object PubSchDataSetSCH_NUMBER: TFIBStringField
      FieldName = 'SCH_NUMBER'
      Size = 30
      EmptyStrToNull = True
    end
    object PubSchDataSetSCH_TYPE: TFIBIntegerField
      FieldName = 'SCH_TYPE'
    end
    object PubSchDataSetLINK_TO: TFIBIntegerField
      FieldName = 'LINK_TO'
    end
    object PubSchDataSetDATE_BEG: TFIBDateTimeField
      FieldName = 'DATE_BEG'
      DisplayFormat = 'dd.mm.yyyy hh:mm AMPM'
    end
    object PubSchDataSetDATE_END: TFIBDateTimeField
      FieldName = 'DATE_END'
      DisplayFormat = 'dd.mm.yyyy hh:mm AMPM'
    end
    object PubSchDataSetTYPE_OBJECT: TFIBIntegerField
      FieldName = 'TYPE_OBJECT'
    end
    object PubSchDataSetSCH_OBJ_TYPE_TITLE: TFIBStringField
      FieldName = 'SCH_OBJ_TYPE_TITLE'
      Size = 30
      EmptyStrToNull = True
    end
    object PubSchDataSetSCH_TYPE_TITLE: TFIBStringField
      FieldName = 'SCH_TYPE_TITLE'
      Size = 30
      EmptyStrToNull = True
    end
    object PubSchDataSetID_SYSTEM: TFIBIntegerField
      FieldName = 'ID_SYSTEM'
    end
    object PubSchDataSetSYSTEM_TEXT: TFIBStringField
      FieldName = 'SYSTEM_TEXT'
      Size = 100
      EmptyStrToNull = True
    end
  end
end
