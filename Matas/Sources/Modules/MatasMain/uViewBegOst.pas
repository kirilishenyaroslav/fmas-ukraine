...
type
  ...
  public
   ...
   ID_SM, ID_RZ, ID_ST, ID_KEKV,summa: integer;
   ID_NOMN, ID_SCH_1, ID_UNIT, is_filtr_gr_tmc: Integer;
...
var
  BegOstForm: TBegOstForm;

implementation

uses uPrnForm, uEditBegOst, uViewOstFilter, uViewOstInvKart, uViewOstMnaKart;
{$R *.dfm}

procedure ShowBegOst(AOwner:TComponent; DBHANDLE : TISC_DB_HANDLE; aID_USER : INT64);stdcall;
begin
 TBegOstForm.Create(AOwner, DBHANDLE, aID_USER);
end;

constructor TBegOstForm.Create(AOwner:TComponent; DBHANDLE : TISC_DB_HANDLE; ID_USER : Int64);
begin
...
procedure TBegOstForm.cxMatOtvPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  Res : Variant;
...
procedure TBegOstForm.FormCreate(Sender: TObject);
var
  dd, dm, dy: Word;
begin
...
procedure TBegOstForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TBegOstForm.cxLookupSchKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TBegOstForm.cxLookupSchPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  Res : Variant;
  mr: boolean;
...
procedure TBegOstForm.N2Click(Sender: TObject);
begin
...
procedure TBegOstForm.cxMatOtvKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key=VK_BACK) or (Key=VK_DELETE) then
 begin
...
procedure TBegOstForm.DocOstDataSetCalcFields(DataSet: TDataSet);
begin
...
procedure TBegOstForm.cxDBButtonEdit1PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
 uPackageManager.ViewDoc(self, DBHANDLE, DocOstDataSet.FieldByName('ID_DOC_PRH').AsInteger);
end;

procedure TBegOstForm.NInvKartClick(Sender: TObject);
var
 T: TOstInvKartForm;
 M: TOstMnaKartForm;
begin
...
procedure TBegOstForm.cxGrid1DBTableView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
 if Key = VK_SPACE then OstFilterCheck;
end;

procedure TBegOstForm.OstFilterCheck;
begin
   try
...
procedure TBegOstForm.cxGrid1DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
 ARect: TRect;
...
procedure TBegOstForm.acExitExecute(Sender: TObject);
begin
 WorkQuery.Close;
 WorkQuery.SQL.Clear;
...
procedure TBegOstForm.acFilterExecute(Sender: TObject);
var
 F: TOstFilterForm;
begin
...
//  F_GROUP_TMC:=F.cxLINKTO.Text;
  ...
  acShowExecute(self);
   ...
   WorkProc.ParamByName('ID_SESSION').AsInt64:=FILTER_ID_SESSION;
...
procedure TBegOstForm.acShowExecute(Sender: TObject);
var
 dm, dy: Word;
 prefix: string;
 w: TForm;
...
{ if F_LINKTO>0 then
 ...
 DocOstDataSet.ParamByName('PERIOD').Value:=StartOfTheMonth(_MATAS_OST_PERIOD);
...
procedure TBegOstForm.acPrintExecute(Sender: TObject);
var
 fp : TPrintForm;
 id: integer;
 prefix: string;
...
procedure TBegOstForm.acFieldsExecute(Sender: TObject);
begin
 ShowDataSetFields(self, DocOstDataSet);
end;

procedure TBegOstForm.acExportExecute(Sender: TObject);
begin
 ExportDataSet(self, DocOstDataSet);
end;

procedure TBegOstForm.acAddExecute(Sender: TObject);
var
 f: TEditBegOstForm;
...
procedure TBegOstForm.acChangeExecute(Sender: TObject);
var
 fe: TEditBegOstForm;
 vals: array of Variant;
begin
 if DocOstDataSet.IsEmpty then
  exit;
 fe:=TEditBegOstForm.Create(self);
 ...
 ID_NOMN:=DocOstDataSet.FieldByName('ID_NOMN').value;
 ID_SCH_1:=DocOstDataSet.FieldByName('ID_SCH').value;
 ID_UNIT:=DocOstDataSet.FieldByName('ID_UNIT').value;;

 if fe.ShowModal=mrOk then
 begin
   if fe.ID_NOMN <>0 then ID_NOMN:=fe.ID_NOMN;
   if fe.ID_SCH <>0 then ID_SCH_1:=fe.ID_SCH;
   if fe.ID_UNIT <>0 then ID_UNIT:=fe.ID_UNIT;
  try
    SetLength(Vals, 7);
    Vals[0] := fe.ID_OST;
    Vals[1] := ID_NOMN;
...
procedure TBegOstForm.acDelExecute(Sender: TObject);
var
 ID_OSTB: integer;
 ID_ISTFIN: integer;
begin
...
procedure TBegOstForm.acAddBExecute(Sender: TObject);
var
 fe: TEditBegOstForm;
 vals: array of Variant;
 ID_ISTFIN:Integer;
...
procedure TBegOstForm.acChangeBExecute(Sender: TObject);
var
 fe: TEditBegOstForm;
 vals: array of Variant;
 ID_ISTFIN:Integer;
begin
 ...
 ID_SM:=IstfinDataSet.FieldByName('ISTFIN_ID_SM').value;
 ID_RZ:=IstfinDataSet.FieldByName('ISTFIN_ID_RZ').value;;
 ID_ST:=IstfinDataSet.FieldByName('ISTFIN_ID_ST').value;;
 ID_KEKV:=IstfinDataSet.FieldByName('ISTFIN_ID_KEKV').value;;
 fe.DBHANDLE:=DBHANDLE;
 if fe.ShowModal=mrOk then
 begin
   if fe.ID_SM<>0 then ID_SM:=fe.ID_SM;
   if fe.ID_RZ<>0 then ID_RZ:=fe.ID_RZ;
   if fe.ID_ST<>0 then ID_ST:=fe.ID_ST;
   if fe.ID_KEKV<>0 then ID_KEKV:=fe.ID_KEKV;
   ID_ISTFIN := IstfinDataSetISTFIN_ID.AsInteger;
  try
    SetLength(Vals, 7);
    Vals[0] := ID_ISTFIN;
...
procedure TBegOstForm.acDelBExecute(Sender: TObject);
