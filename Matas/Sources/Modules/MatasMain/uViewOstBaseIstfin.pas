...
uses
  ...
  pFIBStoredProc, uSpMatOtv;

type
  TOstBaseIstfinForm = class(TForm)
    Panel1: TPanel;
    LabelPeriod: TLabel;
    LabelSch: TLabel;
    cxMonth: TcxComboBox;
  ...
  exports ShowOstBaseIstFin;

var
  OstBaseIstfinForm: TOstBaseIstfinForm;
...
procedure ShowOstBaseIstFin(AOwner:TComponent; DBHANDLE : TISC_DB_HANDLE; aID_USER : INT64);stdcall;
begin
 TOstBaseIstfinForm.Create(AOwner, DBHANDLE, aID_USER);
end;

constructor TOstBaseIstfinForm.Create(AOwner: TComponent; DBHANDLE: TISC_DB_HANDLE; ID_USER: int64);
begin
...
procedure TOstBaseIstfinForm.cxLookupSchKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TOstBaseIstfinForm.cxLookupSchPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  Res : Variant;
  mr: boolean;
...
procedure TOstBaseIstfinForm.ApplyButtonClick(Sender: TObject);
var
 dm, dy: Word;
 prefix: string;
 w: TForm;
begin
 ...
 DocostDataSet.SelectSql.Add('SELECT * FROM MAT_GET_OST_MAT_BY_BASE_ISTFIN(:PERIOD, :PID_SESSION, :ID_MO) ');
...
procedure TOstBaseIstfinForm.ExitButtonClick(Sender: TObject);
begin
 Close;
end;

procedure TOstBaseIstfinForm.FormClose(Sender: TObject;
...
procedure TOstBaseIstfinForm.FormCreate(Sender: TObject);
var
  dd, dm, dy: Word;
begin
...
procedure TOstBaseIstfinForm.FormDestroy(Sender: TObject);
begin
// IstFinFilter.Free;
end;

procedure TOstBaseIstfinForm.FilterButtonClick(Sender: TObject);
begin
// IstFinFilter.F_DB_SCH:=false;
// IstFinFilter.ShowModal;
end;

procedure TOstBaseIstfinForm.PrintButtonClick(Sender: TObject);
...
begin
 ...
 begin
   ...
   begin
    pFIBDataSetForReport.SelectSql.Add('SELECT * FROM MAT_GET_OST_MAT_BY_BASE_ISTFIN(:PERIOD, :PID_SESSION, :ID_MO)');
...
procedure TOstBaseIstfinForm.cxGroupPropertiesChange(Sender: TObject);
begin
...
procedure TOstBaseIstfinForm.cxSmetaPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
 cxSmeta.Text:=VarToStr(uPackageManager.GetTMC(Self,DBHANDLE, ID_USER,FILTER_ID_SESSION,8,_MATAS_PERIOD,'бюджетам'));
end;

procedure TOstBaseIstfinForm.cxRzPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TOstBaseIstfinForm.cxKekvPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  cxKekv.Text:=VarToStr(uPackageManager.GetTMC(Self,DBHANDLE, ID_USER,FILTER_ID_SESSION,10,_MATAS_PERIOD,'КЕКВам'));
end;

procedure TOstBaseIstfinForm.cxButtonEdit1PropertiesButtonClick(
...
procedure TOstBaseIstfinForm.cxMatOtvKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key=VK_BACK) or (Key=VK_DELETE) then
 begin
...
procedure TOstBaseIstfinForm.cxSmetaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key=VK_BACK) or (Key=VK_DELETE) then
 begin
...
procedure TOstBaseIstfinForm.cxRzKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key=VK_BACK) or (Key=VK_DELETE) then
 begin
...
procedure TOstBaseIstfinForm.cxKekvKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (Key=VK_BACK) or (Key=VK_DELETE) then
 begin
...
procedure TOstBaseIstfinForm.cxGrid1DBTableView1CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var   Arect : TRect;
begin
...
procedure TOstBaseIstfinForm.cxButton1Click(Sender: TObject);
begin
 try
   TR.StartTransaction;
   StPr.StoredProcName:='MAT_CLEAR_TMP_BY_SESSION';
...
procedure TOstBaseIstfinForm.DocOstDataSetKOL_MATGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TOstBaseIstfinForm.DocOstDataSetUNITGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
if DocOstDataSetRASHOD_POS.AsInteger = 1 then text:=''
...
procedure TOstBaseIstfinForm.DocOstDataSetSM_KODGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
if DocOstDataSetRASHOD_POS.AsInteger = 1 then text:=''
...
procedure TOstBaseIstfinForm.DocOstDataSetRZ_KODGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
if DocOstDataSetRASHOD_POS.AsInteger = 1 then text:=''
...
procedure TOstBaseIstfinForm.DocOstDataSetST_KODGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
if DocOstDataSetRASHOD_POS.AsInteger = 1 then text:=''
...
procedure TOstBaseIstfinForm.DocOstDataSetKEKV_KODGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
if DocOstDataSetRASHOD_POS.AsInteger = 1 then text:=''
