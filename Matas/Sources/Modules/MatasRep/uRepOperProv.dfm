object OperRepForm: TOperRepForm
  Left = 433
  Top = 298
  BorderStyle = bsDialog
  Caption = #1054#1090#1095#1077#1090' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1080#1103#1084
  ClientHeight = 200
  ClientWidth = 416
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label4: TLabel
    Left = 224
    Top = 115
    Width = 91
    Height = 13
    Caption = #1050#1110#1083#1100#1082#1110#1089#1090#1100' '#1082#1086#1087#1110#1081
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 8
    Top = 84
    Width = 25
    Height = 13
    Caption = #1047#1074#1110#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 10
    Top = 19
    Width = 52
    Height = 13
    Caption = #1055#1077#1088#1110#1086#1076' '#1079
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 230
    Top = 19
    Width = 15
    Height = 13
    Caption = #1087#1086
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object LabelSch: TLabel
    Left = 6
    Top = 52
    Width = 49
    Height = 13
    Caption = #1056#1072#1093#1091#1085#1086#1082
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object OkButton: TcxButton
    Left = 88
    Top = 165
    Width = 125
    Height = 25
    Caption = #1055#1077#1095#1072#1090#1100
    Default = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 9
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
    UseSystemPaint = False
  end
  object CancelButton: TcxButton
    Left = 232
    Top = 165
    Width = 125
    Height = 25
    Caption = #1054#1090#1084#1077#1085#1080#1090#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 10
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
    NumGlyphs = 2
    UseSystemPaint = False
  end
  object cxCheckBox1: TcxCheckBox
    Left = 0
    Top = 180
    Width = 25
    Height = 21
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = ''
    TabOrder = 11
  end
  object cxCheckOnPrinter: TcxCheckBox
    Left = 40
    Top = 112
    Width = 169
    Height = 21
    ParentFont = False
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = #1044#1088#1091#1082#1091#1074#1072#1090#1080' '#1085#1072' '#1087#1088#1080#1085#1090#1077#1088
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 6
  end
  object cxSelectPrinter: TcxCheckBox
    Left = 40
    Top = 136
    Width = 153
    Height = 21
    ParentFont = False
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = #1042#1080#1073#1088#1072#1090#1080' '#1087#1088#1080#1085#1090#1077#1088
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 7
  end
  object cxCopies: TcxSpinEdit
    Left = 326
    Top = 112
    Width = 49
    Height = 21
    BeepOnEnter = False
    ParentFont = False
    Properties.MaxValue = 999.000000000000000000
    Properties.MinValue = 1.000000000000000000
    Properties.ReadOnly = False
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 8
    Value = 1
  end
  object cxReport: TcxComboBox
    Left = 48
    Top = 80
    Width = 361
    Height = 21
    ParentFont = False
    Properties.DropDownListStyle = lsFixedList
    Properties.Items.Strings = (
      #1047#1074#1110#1090' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1110#1103#1084' ('#1087#1086#1074#1085#1080#1081')'
      #1047#1074#1110#1090' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1110#1103#1084' ('#1089#1090#1080#1089#1083#1080#1081')'
      #1047#1074#1110#1090' '#1087#1086' '#1086#1087#1077#1088#1072#1094#1110#1103#1084' ('#1087#1110#1076#1089#1091#1084#1082#1080')')
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 5
  end
  object cxMonthBeg: TcxComboBox
    Left = 64
    Top = 16
    Width = 89
    Height = 21
    ParentFont = False
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 12
    Properties.Items.Strings = (
      #1103#1085#1074#1072#1088#1100
      #1092#1077#1074#1088#1072#1083#1100
      #1084#1072#1088#1090
      #1072#1087#1088#1077#1083#1100
      #1084#1072#1081
      #1080#1102#1085#1100
      #1080#1102#1083#1100
      #1072#1074#1075#1091#1089#1090
      #1089#1077#1085#1090#1103#1073#1088#1100
      #1086#1082#1090#1103#1073#1088#1100
      #1085#1086#1103#1073#1088#1100
      #1076#1077#1082#1072#1073#1088#1100)
    Properties.ReadOnly = False
    Style.Color = clInfoBk
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    Style.LookAndFeel.Kind = lfUltraFlat
    TabOrder = 0
  end
  object cxYearBeg: TcxSpinEdit
    Left = 160
    Top = 16
    Width = 65
    Height = 21
    ParentFont = False
    Style.Color = clInfoBk
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    Style.LookAndFeel.Kind = lfUltraFlat
    TabOrder = 1
  end
  object cxMonthEnd: TcxComboBox
    Left = 248
    Top = 16
    Width = 89
    Height = 21
    ParentFont = False
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 12
    Properties.Items.Strings = (
      #1103#1085#1074#1072#1088#1100
      #1092#1077#1074#1088#1072#1083#1100
      #1084#1072#1088#1090
      #1072#1087#1088#1077#1083#1100
      #1084#1072#1081
      #1080#1102#1085#1100
      #1080#1102#1083#1100
      #1072#1074#1075#1091#1089#1090
      #1089#1077#1085#1090#1103#1073#1088#1100
      #1086#1082#1090#1103#1073#1088#1100
      #1085#1086#1103#1073#1088#1100
      #1076#1077#1082#1072#1073#1088#1100)
    Properties.ReadOnly = False
    Style.Color = clInfoBk
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    Style.LookAndFeel.Kind = lfUltraFlat
    TabOrder = 2
  end
  object cxYearEnd: TcxSpinEdit
    Left = 344
    Top = 16
    Width = 65
    Height = 21
    ParentFont = False
    Style.Color = clInfoBk
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    Style.LookAndFeel.Kind = lfUltraFlat
    TabOrder = 3
  end
  object cxLookupSch: TcxButtonEdit
    Left = 64
    Top = 48
    Width = 344
    Height = 21
    ParentFont = False
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = cxLookupSchPropertiesButtonClick
    Style.Color = clWindow
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 4
    OnKeyDown = cxLookupSchKeyDown
  end
  object OperRepDataSet: TpFIBDataSet
    Database = WorkDatabase
    Transaction = WorkTransaction
    SelectSQL.Strings = (
      'SELECT'
      #9'MAT_SEL_OPER_BY_PERIOD.ID_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.ID_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_ID_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_ID_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH,'
      #9'SUM(MAT_SEL_OPER_BY_PERIOD.SUMMA) as SUMMA'
      'FROM MAT_SEL_OPER_BY_PERIOD(:DATE_BEG, :DATE_END)'
      'GROUP BY '#9'MAT_SEL_OPER_BY_PERIOD.ID_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.ID_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_ID_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_ID_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH'
      'ORDER BY '
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_OUT,'
      #9'MAT_SEL_OPER_BY_PERIOD.FIO_MO_IN,'
      #9'MAT_SEL_OPER_BY_PERIOD.DEP_MO_IN'
      '')
    Left = 64
    Top = 112
    poSQLINT64ToBCD = True
  end
  object frDBDataSet1: TfrDBDataSet
    DataSet = OperRepDataSet
    Left = 160
    Top = 112
  end
  object frReport1: TfrReport
    Dataset = frDBDataSet1
    InitialZoom = pzDefault
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit]
    RebuildPrinter = False
    Left = 128
    Top = 112
    ReportForm = {19000000}
  end
  object WorkDatabase: TpFIBDatabase
    DBParams.Strings = (
      'lc_ctype=WIN1251'
      'user_name=SYSDBA'
      'password=masterkey')
    DefaultTransaction = WorkTransaction
    DefaultUpdateTransaction = WorkTransaction
    SQLDialect = 3
    Timeout = 0
    WaitForRestoreConnect = 0
    Left = 384
    Top = 112
  end
  object WorkTransaction: TpFIBTransaction
    DefaultDatabase = WorkDatabase
    TimeoutAction = TARollback
    Left = 352
    Top = 112
  end
  object ItogRepDataset: TpFIBDataSet
    Database = WorkDatabase
    Transaction = WorkTransaction
    SelectSQL.Strings = (
      'SELECT'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH,'
      #9'SUM(MAT_SEL_OPER_BY_PERIOD.SUMMA) as SUMMA'
      'FROM MAT_SEL_OPER_BY_PERIOD(:DATE_BEG, :DATE_END)'
      'GROUP BY '#9
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH'
      'ORDER BY '
      #9'MAT_SEL_OPER_BY_PERIOD.KR_SCH,'
      #9'MAT_SEL_OPER_BY_PERIOD.DB_SCH')
    Left = 96
    Top = 112
    poSQLINT64ToBCD = True
  end
  object frDBDataSet2: TfrDBDataSet
    DataSet = ItogRepDataset
    Left = 200
    Top = 112
  end
  object KeyActionList: TActionList
    Left = 256
    Top = 112
    object ActionCancel: TAction
      Caption = 'ActionCancel'
      ShortCut = 27
      OnExecute = ActionCancelExecute
    end
    object ActionOk: TAction
      Caption = 'ActionOk'
      ShortCut = 16419
      OnExecute = ActionOkExecute
    end
  end
end
