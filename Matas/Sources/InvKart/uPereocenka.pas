...
begin
    ...
    begin
     ...
     //       LabelOst_iznos.Visible      := False;
        ...
        if Kakaya_oper = 21 then begin cxTextEditVid_oper.Text := 'Дооцiнка'; end;
        if Kakaya_oper = 22 then begin cxTextEditVid_oper.Text := 'Комплектацiя'; end;
        if Kakaya_oper = 23 then begin cxTextEditVid_oper.Text := 'Разукомплектацiя'; end;
        if Kakaya_oper = 24 then begin cxTextEditVid_oper.Text := 'Часткове списання'; end;
    end else
    begin
   ...
   if (undeloper = 1) then
    begin
        ...
        //************* Вызов процедуры формирования списка МОЛов для операции массовой переоценки ******
         try
           StoredProc.Close;
           StoredProc.Transaction := WriteTransaction;
           StoredProc.Transaction.StartTransaction;
    ...
    begin
        ...
        begin
                try
                    ...
                    StoredProc.ParamByName('TIP_OPER').AsInteger    := Kakaya_oper;
                    StoredProc.ExecProc;
                    if (StoredProc.FieldByName('comeback').AsInteger = 1) then ShowMessage('Сума на яку ви намагаєтесь змінити суму проводки, разом з сумою другої проводки є більшою суми за документом!');
                except on E : Exception do
                 begin
                     ShowMessage(E.Message);
                     StoredProc.Transaction.Rollback;
...
begin
    ...
    begin
     ...
     Begin
             ...
             begin
                 ShowMessage('Операцiя з нульовою сумою не маэ сенсу! Тому не буде проведана! Виправте суму, та спробуйте ще.');
                 Exit;
             end else
             begin
                 try
    ...
    //**************   Временное перемещение куска кода для проверки корректности функционирования
    { DataSetGridWatch.Next;
    end;}
    //**************
...
begin
    ...
    begin
         ...
         //************ Записываем документ в буффер документов *****
         try
          StoredProc.Transaction := WriteTransaction;
          StoredProc.Transaction.StartTransaction;
          StoredProc.StoredProcName:='MAT_INV_PUB_DOC_INSERT';
         ...
         //************ Правим сумму в буфере договоров ************
         try
          StoredProc.Transaction := WriteTransaction;
          StoredProc.Transaction.StartTransaction;
