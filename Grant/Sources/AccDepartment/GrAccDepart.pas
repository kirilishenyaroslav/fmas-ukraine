...
uses
  ...
  dxStatusBar, Dates, gr_dmCommonStyles, gr_uWaitForm;
...
type
  ...
  function View_grAccDepart(AParameter:TObject):Variant;stdcall;
  exports View_grAccDepart;
...
function View_grAccDepart(AParameter:TObject):Variant;
var ViewForm:TfGrAccDepart;
begin
  if AParameter.ClassType<>TgrSimpleParam then Exit;
  ViewForm  :=  TfGrAccDepart.Create(TgrSimpleParam(AParameter).Owner,TgrSimpleParam(AParameter).DB_Handle);
  ViewForm.ShowModal;
  Result:=ViewForm.Result;
  ViewForm.Free;
end;
...
begin
 ...
 PKodSetup:=PeriodToKodSetup(YearSpinEdit.IntValue,MonthCombo.ItemIndex+1);
 DSet1.ParamByName('KOD_SETUP').AsInteger:=PKodSetup;
