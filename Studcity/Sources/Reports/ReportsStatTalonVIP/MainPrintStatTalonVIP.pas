...
type
  TfrmMainPrintStatTalonVIP = class(TForm)
...
function PrintStatTalonVIP(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;stdcall;
exports PrintStatTalonVIP;
var
  frmMainPrintStatTalonVIP: TfrmMainPrintStatTalonVIP;
  print_page:Integer;
...
function PrintStatTalonVIP(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;
var
  PrProp:TfrmMainPrintStatTalonVIP;
begin
  PrProp:=TfrmMainPrintStatTalonVIP.Create(AOwner,DB,id_people,type_doc_print);
  PrProp.ShowModal;
  PrProp.Free;
end;


constructor  TfrmMainPrintStatTalonVIP.Create (Aowner:Tcomponent;DBL:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer);
begin
  Inherited Create(Aowner);
  pFIBDatabase.Handle:=DBL;

  Caption:='Печать статического талона убытия.';

  pFIBDataSet.Active:=false;
  pFIBDataSet.ParamByName('param_people').AsInt64:=id_people;
  pFIBDataSet.Active:=true;

  cxLabelStatus.Caption:=StRep_StatVIPP1;
  designer_rep:=0;
  print_page:=0;
...
procedure TfrmMainPrintStatTalonVIP.cxButton2Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmMainPrintStatTalonVIP.cxButton1Click(Sender: TObject);
begin
if print_page=0 then
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'PeopleStatTalonVIP.fr3');
    frxReport.Variables.Clear;
    frxReport.Variables['StRep_Stat_N1']:=''''+StRep_Stat_N1+'''';
    frxReport.Variables['StRep_Stat_N2']:=''''+StRep_StatVIP_N2+'''';
    ...
    frxReport.Variables['StRep_StatN9']:=''''+StRep_StatVIPN9+'''';
    frxReport.Variables['StRep_StatN91']:=''''+StRep_StatVIPN91+'''';
    frxReport.Variables['StRep_StatN10']:=''''+StRep_StatVIPN10+'''';
    ...
    frxReport.Variables['StRep_StatN12']:=''''+StRep_StatVIPN12+'''';
    ...
    frxReport.Variables['StRep_StatN175']:=''''+StRep_StatVIPN175+'''';
    ...
    cxLabelStatus.Caption:=StRep_StatVIPP2;
    print_page:=1;
  ...
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'PeopleStatTalonVIP_Page2.fr3');
    frxReport.Variables.Clear;
    frxReport.Variables['StRep_Stat_N1']:=''''+StRep_Stat_N1+'''';
...
procedure TfrmMainPrintStatTalonVIP.DesignerExecute(Sender: TObject);
begin
