...
type
  TfrmMainPrintStatTalon = class(TForm)
...
function PrintRegTalonVIP(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;stdcall;
exports PrintRegTalonVIP;

var
  frmMainPrintStatTalon: TfrmMainPrintStatTalon;
  print_page:Integer;
...
function PrintRegTalonVIP(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;
var
  PrProp:TfrmMainPrintStatTalon;
begin
  PrProp:=TfrmMainPrintStatTalon.Create(AOwner,DB,id_people,type_doc_print);
  PrProp.ShowModal;
  PrProp.Free;
end;


constructor  TfrmMainPrintStatTalon.Create (Aowner:Tcomponent;DBL:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer);
begin
  Inherited Create(Aowner);
  pFIBDatabase.Handle:=DBL;

  Caption:='Печать талона регистрации.';
...
procedure TfrmMainPrintStatTalon.cxButton2Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmMainPrintStatTalon.cxButton1Click(Sender: TObject);
begin
if print_page=0 then
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'PeopleRegTalonVIP.fr3');
  ...
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'PeopleRegTalonVIP_Page2.fr3');
    frxReport.Variables.Clear;
    frxReport.Variables['StRep_Stat_N1']:=''''+StRep_Stat_N1+'''';
    frxReport.Variables['StRep_Stat_N2']:=''''+StRep_Stat_N2+'''';
    frxReport.Variables['StRep_StatN1']:=''''+StRep_StatN1+'''';
...
procedure TfrmMainPrintStatTalon.DesignerExecute(Sender: TObject);
begin
