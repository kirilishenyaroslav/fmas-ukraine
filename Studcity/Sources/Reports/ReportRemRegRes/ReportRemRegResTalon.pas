...
type
  TfrmRemRegResTalon = class(TForm)
...
function ReportRemRegRes(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;stdcall;
exports ReportRemRegRes;

var
  frmRemRegResTalon: TfrmRemRegResTalon;
  print_page:Integer;
...
function ReportRemRegRes(AOwner:TComponent;DB:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer):Integer;
var
  PrProp:TfrmRemRegResTalon;
begin
  PrProp:=TfrmRemRegResTalon.Create(AOwner,DB,id_people,type_doc_print);
  PrProp.ShowModal;
  PrProp.Free;
end;


constructor  TfrmRemRegResTalon.Create (Aowner:Tcomponent;DBL:TISC_DB_HANDLE;id_people:Int64;type_doc_print:integer);
begin
  Inherited Create(Aowner);
  pFIBDatabase.Handle:=DBL;

  Caption:='Талон снятия с учета регистрации по месту проживания';
...
procedure TfrmRemRegResTalon.cxButton2Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmRemRegResTalon.cxButton1Click(Sender: TObject);
begin
if print_page=0 then
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'ReportRemRegRes.fr3');
  ...
  begin
    frxReport.Clear;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Studcity\'+'ReportRemRegRes_Page2.fr3');
    frxReport.Variables.Clear;
    frxReport.Variables['StRep_Stat_N1']:=''''+StRep_Stat_N1+'''';
    frxReport.Variables['StRep_Stat_N2']:=''''+StRep_Stat_N2+'''';
    frxReport.Variables['StRep_StatN1']:=''''+StRep_StatN1+'''';
...
procedure TfrmRemRegResTalon.DesignerExecute(Sender: TObject);
begin
