...
uses
  ...
  pFIBDatabase, cxContainer,St_ser_function, ActnList, st_ConstUnit,
  st_common_funcs;
  ...
  for i:=0 to selectAll.RecordCount-1 do
    begin
       ...
       With SP do
          begin
          try
          StoredProcName:='ST_DT_REPORT_PAY_LGOT';
          Database:=Database;
...
begin
  ...
  Database.Handle:=DB;
  Lang := stLanguageIndex;
  Caption := st_ConstUnit.st_report_lgot[Lang];
...
begin
    ...
    if j>0 then
      begin
        sql_master:='select distinct '+sql_master + 'ID_TRANSACTION from ST_DT_REPORT_PAY where (LGOT_SUMMA<>0 or DATE_LGOT>'''+DateToStr(DateTimePickerBegin.Date)+''') and Date_Lgot is not null and ID_TRANSACTION='+IntToStr(ID_TRANSACTION)+' order by '+sql_master+' ID_TRANSACTION';
      end
      else
      begin
    ...
    query:='select familia||'''+' '+'''||imya||'''+' '+'''||otchestvo as FIO, ST_DT_REPORT_PAY.* from ST_DT_REPORT_PAY where (LGOT_SUMMA<>0 or DATE_LGOT>'''+DateToStr(DateTimePickerBegin.Date)+''') and Date_Lgot is not null and ID_TRANSACTION='+IntToStr(ID_TRANSACTION)+propi;
    ...
    FieldNameReport[0] := VarArrayOf([DateTimePickerBegin.DateTime, DateTimePickerEnd.DateTime]);

    LoadPackegeStudCity.LoadReportsView(self,Database.Handle,0,sql_master,query,FieldGroup,NFieldView,FieldNameReport,'RPayLgot',0, is_debug);
end;

procedure TfrmMainReportsLgotPay.act_debugExecute(Sender: TObject);
begin
  if is_debug then
   begin
    is_debug := false;
