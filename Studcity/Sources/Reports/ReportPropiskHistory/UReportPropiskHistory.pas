...
uses
  ...
  LoadPackegeStudCity,St_ser_function, cxContainer, st_ConstUnit;

type
  TfReportPropiskHistory = class(TForm)
...
function ReportPropiskHistoryDPK(AOwner : TComponent;DB:TISC_DB_HANDLE):Integer;stdcall;
exports ReportPropiskHistoryDPK;

var
  fReportPropiskHistory: TfReportPropiskHistory;
  ParamReport:Variant;
  ID_TRANSACTION:Variant;
...
begin
  ...
  selectAll.SQLs.SelectSQL.Add('select *  from ST_DT_REPORT_LIVER where ID_TRANSACTION=:param_transaction');
...
function ReportPropiskHistoryDPK(AOwner : TComponent;DB:TISC_DB_HANDLE):Integer;
var
  Reports:TfReportPropiskHistory;
begin
  Reports:=TfReportPropiskHistory.Create(AOwner, DB);
end;

constructor TfReportPropiskHistory.Create(AOwner : TComponent;DB:TISC_DB_HANDLE);
var
  i:Integer;
  dd,mm,yyyy:Word;
begin
  ...
  Caption := st_report_hist_propisk[lng];
...
procedure TfReportPropiskHistory.BitBtnCancelClick(Sender: TObject);
begin
  report.Terminate;
...
procedure TfReportPropiskHistory.FormClose(Sender: TObject;
...
procedure TfReportPropiskHistory.ButtonMainParamClick(Sender: TObject);
var
  Style:Variant;
begin
...
procedure TfReportPropiskHistory.BitBtnOKClick(Sender: TObject);
begin
...
procedure TfReportPropiskHistory.BitBtnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfReportPropiskHistory.FormDestroy(Sender: TObject);
begin
  if report<>nil then
    begin
      report.Terminate;
...
procedure TfReportPropiskHistory.DateTimePickerRunChange(Sender: TObject);
begin
ButtonView.Enabled:=false;
end;

procedure TfReportPropiskHistory.Timer1Timer(Sender: TObject);
begin
...
procedure TfReportPropiskHistory.ButtonViewClick(Sender: TObject);
var
...
if Doc_R=1 then
  begin
    ...
    if j>0 then
      begin
        sql_master:='select FIO, BEG_PROPIS, END_PROPIS, BEFOR_STAT, BEFO_PUNKT, '+
        'BEFOR_RAJ, BEFOR_TOWN, BEFOR_SELO, BEFOR_STR, BEFOR_NOM, BEFOR_KV'+
        ' FROM ST_DT_REPORT_PROPISK('+IntToStr(ID_TRANSACTION)+')';
      end;
      {else
      begin
    ...
    {query:='select familia||'''+' '+'''||imya||'''+' '+'''||otchestvo as FIO,ST_DT_REPORT_LIVER.*,st_dt_report_liver_narush.* from st_dt_report_liver,st_dt_report_liver_narush where ID_TRANSACTION='+IntToStr(ID_TRANSACTION);
    ...
    LoadPackegeStudCity.LoadReportsView(self,Database.Handle,0,sql_master,query,FieldGroup, NFieldView,FieldNameReport,'RPropiskHistory',0);
  end;
end;
