...
type
    TNamePrKSprav = (PrK_SP_DERG_ZAKAZ, PrK_SP_EXZFORM, PrK_SP_GURTOG, PrK_SP_IN_LANG,
        ...
        PrK_SP_DOLGNOST_PK, PrK_SP_KAT_PRI_D, PrK_SP_TYPE_ZACH);
type
    TNamePrKSpravEdit = (PrKSpravEditStandart, PrKSpravEditFobProt);
type
...
function CsMessageDlg(const Caption: string; const Msg: string; const DlgType: TMsgDlgType;
    const Buttons: TMsgDlgButtons; aIndexLanguage: integer): word; stdcall;
//exports  agMessageDlg;

implementation

uses Forms, SysUtils, Windows, Variants, registry, uCS_Constants,
    AccMgmt;
{ TDataPrKSprav }
...
begin
   ...
   //чтение из регистра
    ...
    try
        ...
        if reg.OpenKey('\Software\CS\Languages\', false) then
        begin
...
begin
   ...
   //чтение из регистра
    ...
    try
        reg.RootKey := HKEY_CURRENT_USER;
        if reg.OpenKey('\Software\CS\ShemaColor\', false) then
        begin
...
function CsMessageDlg(const Caption: string; const Msg: string;
