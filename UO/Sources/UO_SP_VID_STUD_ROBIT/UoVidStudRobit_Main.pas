...
type
  TfmUoVidStudRobit_Main = class(TForm)
    ...
    DSID_SP_VID_STUD_ROBIT: TFIBBCDField;
    DSNAME: TFIBStringField;
    DSSHORT_NAME: TFIBStringField;
...
uses UoVidStudRobit_Add, {UWResourcesUnit,} Registry;

{$R *.dfm}

procedure ShowAllUoBpl(aOwner: TComponent; aParam : TAArray);
var
   T : TfmUoVidStudRobit_Main;
begin
    T := TfmUoVidStudRobit_Main.Create(aOwner, aParam);
...
constructor TfmUoVidStudRobit_Main.Create(aOwner: TComponent;
  aParam: TAArray);
var
    s : string;
begin
    ...
    begin
        ...
        LoadCaption;
        DS.close;
        DS.Sqls.SelectSQL.Text := 'SELECT * FROM UO_SP_VID_STUD_ROBIT_SEL';
        DS.Open;
    end
    else begin
...
procedure TfmUoVidStudRobit_Main.ActionADDExecute(Sender: TObject);
var
    T : TfmUoVidStudRobit_Add;
begin
    T := TfmUoVidStudRobit_Add.Create(Self, DataBase_uo__sp_type, 1, Self);
    T.ShowModal;
    T.Free;
end;

procedure TfmUoVidStudRobit_Main.ActionObnovExecute(Sender: TObject);
var
    id : int64;
begin
    TRY ID := StrToInt64(DS.fbn('ID_SP_VID_STUD_ROBIT').AsString); except id := -1; end;
    DS.CloseOpen(false);
    DS.Locate('ID_SP_VID_STUD_ROBIT', id, []);
end;

procedure TfmUoVidStudRobit_Main.ActionChangeExecute(Sender: TObject);
var
    T : TfmUoVidStudRobit_Add;
begin
    if DS.IsEmpty then exit;
    T := TfmUoVidStudRobit_Add.Create(Self, DataBase_uo__sp_type, 2, Self);
    T.ShowModal;
    T.Free;
end;

procedure TfmUoVidStudRobit_Main.ActionDELExecute(Sender: TObject);
var
...
begin
    ...
    begin
        s1 := 'Ви дійсно бажаєте видалити вид робот ';//UWResourcesUnit.V2_TYPE_DELETE[index_lang];
        ...
        begin
            id_del_t           := StrToInt64(DS.FieldByName('ID_SP_VID_STUD_ROBIT').AsString);
            cxGrid_uo__sp_typeDBTableView1.Controller.GoToPrev(false);
            id_del_l           := StrToInt64(DS.FieldByName('ID_SP_VID_STUD_ROBIT').AsString);
            St.Database        := DataBase_uo__sp_type;
            St.Transaction     := TransactionWrite_uo_sp_type;
            try
                TransactionWrite_uo_sp_type.StartTransaction;
                St.StoredProcName := 'UO_SP_VID_STUD_ROBIT_DEL';
                St.ParamByName('ID_SP_VID_STUD_ROBIT').AsInt64 := id_del_t;
                St.ExecProc;
            ...
            DS.Locate('ID_SP_VID_STUD_ROBIT', id_del_l, []);
        end;
    end;
end;

procedure TfmUoVidStudRobit_Main.ActionViewExecute(Sender: TObject);
var
    T : TfmUoVidStudRobit_Add;
begin
    if DS.IsEmpty then exit;
    T := TfmUoVidStudRobit_Add.Create(Self, DataBase_uo__sp_type, 3, Self);
    T.ShowModal;
    T.Free;
end;

procedure TfmUoVidStudRobit_Main.ActionOtmenaExecute(Sender: TObject);
begin
    close;
end;

procedure TfmUoVidStudRobit_Main.ActionVibratExecute(Sender: TObject);
begin
    if not DS.IsEmpty then
    begin
...
//            FillArrayFromDataSet(ResultArray['OutPut'], DS);
            ResultArray['OutPut']['ID_SP_VID_STUD_ROBIT'].AsInt64  := strToInt64(DS.fbn('ID_SP_VID_STUD_ROBIT').AsString);
...
procedure TfmUoVidStudRobit_Main.cxGrid_uo__sp_typeDBTableView1DblClick(
  Sender: TObject);
begin
    if ActionVibrat.Visible then ActionVibratExecute(SENDER);
end;

procedure TfmUoVidStudRobit_Main.FormClose(Sender: TObject;
...
procedure TfmUoVidStudRobit_Main.LoadCaption;
//var
//    reg : TRegistry;
begin
{    reg := TRegistry.Create;
    ...
    Caption                 := 'Довідник видів робіт студентів';//UWResourcesUnit.V2_TYPE_NAME_CAPTION[index_lang];
    ...
    ActionADD.Hint          := 'Додати вид робіт студентів';//UWResourcesUnit.MY_CONST_ADD[index_lang];
    ActionChange.Hint       := 'Змінити вид робіт студентів';//UWResourcesUnit.MY_CONST_UPDATE[index_lang];
    ActionDEL.Hint          := 'Видалити вид робіт студентів';//UWResourcesUnit.MY_CONST_DELETE[index_lang];
    ActionView.Hint         := 'Переглянути вид робіт студентів';//UWResourcesUnit.MY_CONST_SHOW[index_lang];
    ActionObnov.Hint        := 'Оновити данні з серверу студентів';//UWResourcesUnit.MY_CONST_REFRESH[index_lang];
    ActionVibrat.Hint       := 'Вибрати вид робіт студентів';//UWResourcesUnit.MY_CONST_CHOOSE[index_lang];
    ActionOtmena.Hint       := 'Закрити довідник студентів';//UWResourcesUnit.MY_CONST_VIH[index_lang];

    ActionPrint.Caption     := 'Друкувати';//UWResourcesUnit.MY_CONST_PRINT[index_lang];
    ActionPrint.Hint        := 'Друкувати реєстр видів робіт студентів';//UWResourcesUnit.MY_CONST_PRINT[index_lang];
...
procedure TfmUoVidStudRobit_Main.ActionPrintExecute(Sender: TObject);
//var
...
procedure TfmUoVidStudRobit_Main.cxTextEdit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmUoVidStudRobit_Main.SelectLang;
//var
//    reg : TRegistry;
begin
{    reg := TRegistry.Create;
...
procedure TfmUoVidStudRobit_Main.Action2Execute(Sender: TObject);
begin
    SelectLang;
end;

procedure TfmUoVidStudRobit_Main.cxGrid_uo__sp_typeDBTableView1KeyDown(
