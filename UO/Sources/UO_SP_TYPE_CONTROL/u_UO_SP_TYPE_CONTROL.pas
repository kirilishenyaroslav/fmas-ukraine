...
uses
  ...
  frxExportRTF, frxDBSet, cxCheckBox;

type
  Tfm_UO_SP_TYPE_CONTROL = class(TForm)
    ...
    DataSet_uo__sp_typeID_SP_TYPE_CONTROL: TFIBBCDField;
    ...
    DataSet_uo__sp_typeSHOW_BY_SEM: TFIBSmallIntField;
    DataSet_uo__sp_typeID_SP_VID_NAVCH_ROBIT: TFIBBCDField;
    DataSet_uo__sp_typeSHORT_NAME_ROBIT: TFIBStringField;
    DataSet_uo__sp_typeNAME_ROBIT: TFIBStringField;
...
//    npp_get: Integer;
  ...
  public
    ...
    temp_SP_TYPE_CONTROL: Int64;
    type_kod: String;
    type_npp: String;
    temp_type_npp: Integer;
    cap : Integer;

    id_SP_TYPE_CONTROL : Int64;
    Name_SP_TYPE_CONTROL: String;
    Short_Name_SP_TYPE_CONTROL: String;
    Kod_SP_TYPE_CONTROL: Integer;
    Npp_SP_TYPE_CONTROL: Integer;
    { Public declarations }
...
uses u_UO_SP_TYPE_CONTROL_EDIT;

{$R *.dfm}
procedure ShowAllUoBpl(aOwner: TComponent; aParam : TAArray);
var
   T : Tfm_UO_SP_TYPE_CONTROL;
begin
   T := Tfm_UO_SP_TYPE_CONTROL.Create(aOwner, aParam);
...
constructor Tfm_UO_SP_TYPE_CONTROL.Create(aOwner: TComponent;
  aParam: TAArray);
begin
...
procedure Tfm_UO_SP_TYPE_CONTROL.proverkakod(kod: integer);
begin
    DataSet_prov_kod.Close;
    DataSet_prov_kod.Sqls.SelectSQL.Text := 'select max(kod) as kod from UO_SP_TYPE_CONTROL';
...
PROCEDURE Tfm_UO_SP_TYPE_CONTROL.proverkanpp(npp: integer);
begin
    DataSet_prov_npp.Close;
    DataSet_prov_npp.Sqls.SelectSQL.Text := 'select max(npp) as npp from UO_SP_TYPE_CONTROL';
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionADDExecute(Sender: TObject);
var
   T: Tfm_UO_SP_TYPE_CONTROL_EDIT;
   TryAgain :boolean;
begin
   cap:=0;
   TryAgain:=false;
  ...
  { if DataSetPrKSostPK.FieldValues[ID_NAME]<>Null
   ...
   T := Tfm_UO_SP_TYPE_CONTROL_EDIT.Create(self);
   proverkakod(1);
   proverkanpp(1);
   ...
   if T.ShowModal=MrOk then
    begin
       ...
       begin
            ...
            StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_CONTROL_ADD';
            ...
            StoredProc_uo_sp_type.ParamByName('ID_SP_VID_NAVCH_ROBIT').AsInt64 := T.id_robit;
            if T.cxCheckBox1.Checked
                then StoredProc_uo_sp_type.ParamByName('SHOW_BY_SEM').AsInteger  := 1
                else StoredProc_uo_sp_type.ParamByName('SHOW_BY_SEM').AsInteger  := 0;
       ...
       begin
            ...
            temp_SP_TYPE_CONTROL:= id_SP_TYPE_CONTROL;
       end;
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionObnovExecute(Sender: TObject);
begin
      temp_SP_TYPE_CONTROL:=DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsInt64;
      DataSet_uo__sp_type.Active:=false;
      DataSet_uo__sp_type.Active:=true;
      DataSet_uo__sp_type.Locate('ID_SP_TYPE_CONTROL',temp_SP_TYPE_CONTROL,[]);
end;

procedure Tfm_UO_SP_TYPE_CONTROL.ActionChangeExecute(Sender: TObject);
var 
   T:Tfm_UO_SP_TYPE_CONTROL_EDIT;
   TryAgain :boolean;
begin
   ...
   begin
          ...
          T.id_robit            := Strtoint64(DataSet_uo__sp_type.fbn('ID_SP_VID_NAVCH_ROBIT').AsString);
          T.cxButtonEdit1.Text  := DataSet_uo__sp_type.fbn('NAME_ROBIT').AsString;
          if DataSet_uo__sp_type.FBN('SHOW_BY_SEM').AsInteger = 1
              then T.cxCheckBox1.Checked := true
              else T.cxCheckBox1.Checked := false;
   end;
       if T.ShowModal=MrOk then
        begin
       ...
       begin
           StoredProc_uo_sp_type.Transaction.StartTransaction;
           StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_CONTROL_CHANGE';
           StoredProc_uo_sp_type.Prepare;
           StoredProc_uo_sp_type.ParamByName('ID_SP_TYPE_CONTROL').AsInt64          :=DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsInt64;
           StoredProc_uo_sp_type.ParamByName('NAME').AsString         :=t.cxTextEditName.Text;
           StoredProc_uo_sp_type.ParamByName('SHORT_NAME').AsString   :=t.cxTextEditShortName.Text;
           StoredProc_uo_sp_type.ParamByName('KOD').Value             :=T.cxSpinEditKod.Value;
           StoredProc_uo_sp_type.ParamByName('NPP').Value              :=t.cxSpinEditNPP.Value;
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionDELExecute(Sender: TObject);
begin
   if DataSet_uo__sp_type.IsEmpty then exit;
     if DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsString<>Null then
     begin
       if MessageDlg('Ви дійсно бажаете видалити цей запис',mtInformation,mbOKCancel,0)=mrOK then
         begin
            StoredProc_uo_sp_type.Transaction.StartTransaction;
            StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_CONTROL_DEL';
            StoredProc_uo_sp_type.Prepare;
            StoredProc_uo_sp_type.ParamByName('ID_SP_TYPE_CONTROL').AsInt64:= DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsInt64;
            try
              StoredProc_uo_sp_type.ExecProc;
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionViewExecute(Sender: TObject);
var
   T:Tfm_UO_SP_TYPE_CONTROL_EDIT;
begin
    cap:=2;
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionOtmenaExecute(Sender: TObject);
begin
      Close;
end;

procedure Tfm_UO_SP_TYPE_CONTROL.ActionVibratExecute(Sender: TObject);
begin
  if DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsString<>NULL then
    begin
      if DataSet_uo__sp_typeID_SP_TYPE_CONTROL.AsString='-1' then exit;
...
procedure Tfm_UO_SP_TYPE_CONTROL.cxGrid_uo__sp_typeDBTableView1DblClick(
  Sender: TObject);
begin
...
procedure Tfm_UO_SP_TYPE_CONTROL.FormClose(Sender: TObject;
...
procedure Tfm_UO_SP_TYPE_CONTROL.LoadCaption;
begin
    Caption:= 'Довідник типів контролю';
...
procedure Tfm_UO_SP_TYPE_CONTROL.ActionPrintExecute(Sender: TObject);
var
...
begin
    ...
    Memo1.Text:='Довідник типів контролю';
    Memo6.Text:='Назва';
