...
type
  Tfmmain_sp_norm_chisl_asp = class(TForm)
    ...
    DSID_SP_NORM_CHISL_ASP: TFIBBCDField;
...
implementation
uses add_sp_norm_chisl_asp;
{$R *.dfm}

procedure ShowAllUoBpl(aOwner: TComponent; aParam : TAArray);
var
    T : Tfmmain_sp_norm_chisl_asp;
begin
    T             := Tfmmain_sp_norm_chisl_asp.Create(aOwner, aParam);
...
constructor Tfmmain_sp_norm_chisl_asp.Create(aOwner: TComponent;
  aParam: TAArray);
begin
    ...
    begin
        ...
        LoadCaption;
        DS.Close;
        DS.SQLs.SelectSQL.Text := 'select * from UO_SP_NORM_CHISL_ASP_SELECT';
        DS.Open;
    end
    else ShowMessage('Ошибка Handl`a Базы');
end;

procedure Tfmmain_sp_norm_chisl_asp.LoadCaption;
begin
    Caption := 'Довідник нормативів чисельності аспірантів';
...
procedure Tfmmain_sp_norm_chisl_asp.AExitExecute(Sender: TObject);
begin
    Close;
end;

procedure Tfmmain_sp_norm_chisl_asp.ADelExecute(Sender: TObject);
var
    id_del_l : int64;
begin
    ...
    begin
        ...
        begin
            id_del_l           := StrToInt64(DS.FieldByName('ID_SP_NORM_CHISL_ASP').AsString);
            Stored.Transaction := TWr;
            try
                TWr.StartTransaction;
                Stored.StoredProcName := 'UO_SP_NORM_CHISL_ASP_DELETE';
                Stored.ParamByName('ID_SP_NORM_CHISL_ASP').AsInt64 := StrToInt64(ds.FieldByName('ID_SP_NORM_CHISL_ASP').AsString);
            ...
            DS.Locate('ID_SP_NORM_CHISL_ASP', id_del_l, []);
        end;
    end;
end;

procedure Tfmmain_sp_norm_chisl_asp.AAddExecute(Sender: TObject);
var
    T : Tfmadd_sp_norm_chisl_asp;
begin
    T := Tfmadd_sp_norm_chisl_asp.Create(Self, Self, 1);
    T.ShowModal;
    T.Free;
end;

procedure Tfmmain_sp_norm_chisl_asp.AChangeExecute(Sender: TObject);
var
    T : Tfmadd_sp_norm_chisl_asp;
begin
    if not DS.IsEmpty then
    begin
        T := Tfmadd_sp_norm_chisl_asp.Create(Self, Self, 2);
        T.ShowModal;
        T.Free;
    end;
end;

procedure Tfmmain_sp_norm_chisl_asp.AVeiwExecute(Sender: TObject);
var
    T : Tfmadd_sp_norm_chisl_asp;
begin
    if not DS.IsEmpty then
    begin
        T := Tfmadd_sp_norm_chisl_asp.Create(Self, Self, 3);
        T.ShowModal;
        T.Free;
    end;
end;

procedure Tfmmain_sp_norm_chisl_asp.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
    if FormStyle = fsMDICHild then Action := CaFree;
end;

procedure Tfmmain_sp_norm_chisl_asp.ARefreshExecute(Sender: TObject);
var
    id_ref : int64;
begin
    try id_ref := StrToInt64(ds.fbn('ID_SP_NORM_CHISL_ASP').AsString); except id_ref := -1; end;
    DS.CloseOpen(false);
    DS.Locate('ID_SP_NORM_CHISL_ASP', id_ref, []);
end;
