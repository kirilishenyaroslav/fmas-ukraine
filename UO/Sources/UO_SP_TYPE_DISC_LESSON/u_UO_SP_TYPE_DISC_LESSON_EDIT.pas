unit u_UO_SP_TYPE_DISC_LESSON_EDIT;
...
type
  Tfm_UO_SP_TYPE_DISC_LESSON_EDIT = class(TForm)
...
var
  fm_UO_SP_TYPE_DISC_LESSON_EDIT: Tfm_UO_SP_TYPE_DISC_LESSON_EDIT;

implementation

uses  u_UO_SP_TYPE_DISC_LESSON;

{$R *.dfm}



procedure Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.ActionOKExecute(Sender: TObject);
begin
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.ActionCanselExecute(Sender: TObject);
begin
   ModalResult:=mrCancel;
end;

procedure Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.FormCreate(Sender: TObject);
begin
    if Tfm_UO_SP_TYPE_DISC_LESSON(self.Owner).cap=0 then
        Caption:='Äîäàòè';
    if Tfm_UO_SP_TYPE_DISC_LESSON(self.Owner).cap=1 then
        Caption:='Çì³íèòè';
    if Tfm_UO_SP_TYPE_DISC_LESSON(self.Owner).cap=2 then
        Caption:='Ïåğåãëÿä';
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.cxSpinEditNPPKeyPress(Sender: TObject;
  var Key: Char);
var
x: Integer;
begin
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.cxSpinEditKodKeyPress(Sender: TObject;
  var Key: Char);
var
x : Integer;
begin
