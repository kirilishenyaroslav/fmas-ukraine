...
uses
  ...
  frxClass, frxExportRTF, frxDBSet, cxCheckBox;

type
  Tfm_UO_SP_TYPE_DISC_LESSON = class(TForm)
    ...
    DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON: TFIBBCDField;
...
var
  fm_UO_SP_TYPE_DISC_LESSON: Tfm_UO_SP_TYPE_DISC_LESSON;

implementation

uses u_UO_SP_TYPE_DISC_LESSON_EDIT;

{$R *.dfm}

procedure ShowAllUoBpl(aOwner: TComponent; aParam : TAArray);
var
   T : Tfm_UO_SP_TYPE_DISC_LESSON;
begin
   T := Tfm_UO_SP_TYPE_DISC_LESSON.Create(aOwner, aParam);
...
constructor Tfm_UO_SP_TYPE_DISC_LESSON.Create(aOwner: TComponent;
  aParam: TAArray);
begin
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.proverkakod(kod: integer);
begin
    DataSet_prov_kod.Close;
    DataSet_prov_kod.Sqls.SelectSQL.Text := 'select max(kod) as kod from UO_SP_TYPE_DISC_LESSON';
...
PROCEDURE Tfm_UO_SP_TYPE_DISC_LESSON.proverkanpp(npp: integer);
begin
    DataSet_prov_npp.Close;
    DataSet_prov_npp.Sqls.SelectSQL.Text := 'select max(npp) as npp from UO_SP_TYPE_DISC_LESSON';
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionADDExecute(Sender: TObject);
var
   T: Tfm_UO_SP_TYPE_DISC_LESSON_EDIT;
   TryAgain :boolean;
begin
   cap:=0;
   TryAgain:=false;
  ...
  { if DataSetPrKSostPK.FieldValues[ID_NAME]<>Null
   ...
   T := Tfm_UO_SP_TYPE_DISC_LESSON_EDIT.Create(self);
   proverkakod(1);
   proverkanpp(1);
   ...
   if T.ShowModal=MrOk then
    begin
       ...
       StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_DISC_LESSON_ADD';
       ...
       if T.cxCheckBox1.Checked
           then StoredProc_uo_sp_type.ParamByName('SHOW_BY_SEM').Value  := 1
           else StoredProc_uo_sp_type.ParamByName('SHOW_BY_SEM').Value  := 0;
       ...
       begin
            ...
            temp_SP_TYPE_DISC_LESSON:= id_SP_TYPE_DISC_LESSON;
       end;
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionObnovExecute(Sender: TObject);
begin
    temp_SP_TYPE_DISC_LESSON:=DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsInt64;
    DataSet_uo__sp_type.Active:=false;
    DataSet_uo__sp_type.Active:=true;
    DataSet_uo__sp_type.Locate('ID_SP_TYPE_DISC_LESSON',temp_SP_TYPE_DISC_LESSON,[]);
end;

procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionChangeExecute(Sender: TObject);
var 
   T:Tfm_UO_SP_TYPE_DISC_LESSON_EDIT;
   TryAgain :boolean;
begin
   ...
   begin
          ...
          if DataSet_uo__sp_type.fbn('SHOW_BY_SEM').AsInteger = 1 then
       ...
       begin
           StoredProc_uo_sp_type.Transaction.StartTransaction;
           StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_DISC_LESSON_CHANGE';
           StoredProc_uo_sp_type.Prepare;
           StoredProc_uo_sp_type.ParamByName('ID_SP_TYPE_DISC_LESSON').AsInt64          :=DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsInt64;
       StoredProc_uo_sp_type.ParamByName('NAME').AsString         :=t.cxTextEditName.Text;
       StoredProc_uo_sp_type.ParamByName('SHORT_NAME').AsString   :=t.cxTextEditShortName.Text;
       StoredProc_uo_sp_type.ParamByName('KOD').Value             :=T.cxSpinEditKod.Value;
       StoredProc_uo_sp_type.ParamByName('NPP').Value             :=t.cxSpinEditNPP.Value;
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionDELExecute(Sender: TObject);
begin
   if DataSet_uo__sp_type.IsEmpty then exit;
     if DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsString<>Null then
     begin
       if MessageDlg('Ви дійсно бажаете видалити цей запис',mtInformation,mbOKCancel,0)=mrOK then
         begin
            StoredProc_uo_sp_type.Transaction.StartTransaction;
            StoredProc_uo_sp_type.StoredProcName:='UO_SP_TYPE_DISC_LESSON_DEL';
            StoredProc_uo_sp_type.Prepare;
            StoredProc_uo_sp_type.ParamByName('ID_SP_TYPE_DISC_LESSON').AsInt64:= DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsInt64;
            try
              StoredProc_uo_sp_type.ExecProc;
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionViewExecute(Sender: TObject);
var
   T:Tfm_UO_SP_TYPE_DISC_LESSON_EDIT;
begin
    cap:=2;
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionOtmenaExecute(Sender: TObject);
begin
    close;
end;

procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionVibratExecute(Sender: TObject);
begin
  if DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsString<>NULL then
    begin
      if DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsString='-1' then exit;
...
{             Name_SP_TYPE_DISC_LESSON:=fm_UO_SP_TYPE_DISC_LESSON.DataSet_uo__sp_typeNAME.Value;
              ...
              id_SP_TYPE_DISC_LESSON := fm_UO_SP_TYPE_DISC_LESSON.DataSet_uo__sp_typeID_SP_TYPE_DISC_LESSON.AsInt64;
             }
        close;
        end
      else
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.cxGrid_uo__sp_typeDBTableView1DblClick(
  Sender: TObject);
begin
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.FormClose(Sender: TObject;
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.LoadCaption;
begin
    Caption:= 'Довідник типів занять з дисциплін';
...
procedure Tfm_UO_SP_TYPE_DISC_LESSON.ActionPrintExecute(Sender: TObject);
var
...
begin
    ...
    Memo1.Text:='Довідник типів занять з дисциплін';
    Memo6.Text:='Назва';
