unit uo_kontingent_4_potok_add;
...
uses
  ...
  ActnList, uo_kontingent_4_potok, FIBQuery, pFIBQuery, pFIBStoredProc,
  FIBDatabase, pFIBDatabase, DB, FIBDataSet, pFIBDataSet;

type
  Tfmuo_kontingent_4_potok_add = class(TForm)
  ...
  private
    myform : Tfmuo_kontingent_4_potok;
    reg    : smallint;
    id_zakaz, ID_SP_SPEC, {id_group,} id_type{, id_kurs} : int64;
  public
    constructor Create(aOwner: TComponent; mform : Tfmuo_kontingent_4_potok; r : smallint); reintroduce; overload;
  end;
...
uses uPrK_Loader,
     ...
     AArray,
     uUO_Loader,
     uo_kontingent_4_vibor_disp;
{$R *.dfm}

{ Tfmuo_kontingent_2_potok_add }

constructor Tfmuo_kontingent_4_potok_add.Create(aOwner: TComponent;
  mform: Tfmuo_kontingent_4_potok; r: smallint);
begin
...
procedure Tfmuo_kontingent_4_potok_add.cxTextEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tfmuo_kontingent_4_potok_add.cxButtonEdit1PropertiesButtonClick(
...
procedure Tfmuo_kontingent_4_potok_add.cxButtonEdit3PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
    res : Variant;
begin
...
procedure Tfmuo_kontingent_4_potok_add.cxButton2Click(Sender: TObject);
begin
    Close;
end;

procedure Tfmuo_kontingent_4_potok_add.cxButton1Click(Sender: TObject);
begin
    Action1Execute(Sender);
end;

procedure Tfmuo_kontingent_4_potok_add.Action1Execute(Sender: TObject);
var
...
begin
    ...
    begin
        ...
        try
            ...
            DataSet.Close;
            DataSet.SQLs.SelectSQL.Text := 'select * from UO_DT_KONTINGENT_4_INSERT('+InttoStr(id_god)+', '+InttoStr(id_fak)+', '+InttoStr(id_form)+', '+InttoStr(id_kat)+', '+InttoStr(id_kurs)+', '+InttoStr(ID_SP_Spec)+', '+IntToStr(id_potok)+', '+IntToStr(id_zakaz)+', '+InttoStr(ID_type)+', '''+cxTextEdit1.Text+''')';
            DataSet.Open;
    ...
    begin
        ...
        try
            ...
            Stored.StoredProcName := 'UO_DT_KONTINGENT_4_UPDATE';
            Stored.ParamByName('ID_DT_KONTINGENT_PRAKT').AsInt64   := id;
