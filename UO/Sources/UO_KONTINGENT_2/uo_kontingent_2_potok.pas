...
uses
  ...
  cxMaskEdit, cxButtonEdit, cxContainer, cxLabel, ExtCtrls, uo_kontingent_2_main, AArray,
  FIBQuery, pFIBQuery, pFIBStoredProc, cxGridBandedTableView,
  cxGridDBBandedTableView, cxSplitter;

type
  Tfmuo_kontingent_2_potok = class(TForm)
  ...
  private
    myform : Tfm_uo_kontingent_2_main;
    reg : smallint;
  public
    id_potok : int64;
    id_kon, id_god, id_fak, id_form, id_kat, id_kurs, id_spec : int64;
    date_work, name_spec : string;
    constructor Create(aOwner: TComponent; mform : Tfm_uo_kontingent_2_main; r : smallint); reintroduce; overload;
  end;

implementation
uses uUO_Loader,
     uo_kontingent_2_potok_add,
     uCommonSp;
{$R *.dfm}

constructor Tfmuo_kontingent_2_potok.Create(aOwner: TComponent;
  mform: Tfm_uo_kontingent_2_main; r: smallint);
begin
...
procedure Tfmuo_kontingent_2_potok.cxButtonEdit1PropertiesButtonClick(
...
procedure Tfmuo_kontingent_2_potok.dxBarLargeButtonOtmenaClick(Sender: TObject);
begin
    Close;
end;

procedure Tfmuo_kontingent_2_potok.ActionRefreshExecute(Sender: TObject);
begin
    if id_potok > 0 then
    begin
        DS.Close;
...
procedure Tfmuo_kontingent_2_potok.ActionDeleteExecute(Sender: TObject);
var
    ...
    begin
        ...
        begin
            ...
            try
                ...
                Stored.StoredProcName := 'UO_DT_KONTINGENT_2_DELETE';
...
procedure Tfmuo_kontingent_2_potok.ActionAddExecute(Sender: TObject);
var
    T : Tfmuo_kontingent_2_potok_add;
begin
    if id_potok <= 0 then
    begin
...
{    if not DS.IsEmpty then
    ...
    T := Tfmuo_kontingent_2_potok_add.Create(Self, Self, 1);
    T.Showmodal;
    T.Free;

end;

procedure Tfmuo_kontingent_2_potok.ActionChangeExecute(Sender: TObject);
var
    T : Tfmuo_kontingent_2_potok_add;
begin
    if not DS.IsEmpty then
    begin
        T := Tfmuo_kontingent_2_potok_add.Create(Self, Self, 2);
        T.Showmodal;
        T.Free;
    end;
end;

procedure Tfmuo_kontingent_2_potok.ActionCloseExecute(Sender: TObject);
begin
    Close;
end;
