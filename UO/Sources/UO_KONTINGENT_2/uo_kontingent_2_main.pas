...
type
  Tfm_uo_kontingent_2_main = class(TForm)
...
implementation
uses uo_kontingent_2_potok;
{$R *.dfm}

procedure ShowAllUoBpl(aOwner: TComponent; aParam : TAArray);
var
    T : Tfm_uo_kontingent_2_main;
begin
    T := Tfm_uo_kontingent_2_main.Create(aOwner, aParam);
...
constructor Tfm_uo_kontingent_2_main.Create(aOwner: TComponent;
  aParam: TAArray);
begin
    ...
    begin
        ...
        LoadCaption;
        DS.Close;
        DS.SQLs.SelectSQL.Text := 'select * from UO_DT_KONTINGENT_2_SELECT';
        DS.Open;
...
procedure Tfm_uo_kontingent_2_main.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
    if FormStyle = fsMDIChild then Action := caFree;
end;

procedure Tfm_uo_kontingent_2_main.LoadCaption;
begin
    Caption                 := 'Контінгент';
...
procedure Tfm_uo_kontingent_2_main.ActionCloseExecute(Sender: TObject);
begin
    Close;
end;

procedure Tfm_uo_kontingent_2_main.ActionRefreshExecute(Sender: TObject);
var
    id : int64;
begin
    if not DS.isEmpty then
...
procedure Tfm_uo_kontingent_2_main.DSAfterScroll(DataSet: TDataSet);
begin
    if not DS.isEmpty then
    begin
...
procedure Tfm_uo_kontingent_2_main.ActionAddExecute(Sender: TObject);
var
    T : Tfmuo_kontingent_2_potok;
begin
    if DS.isEmpty then exit;
    T := Tfmuo_kontingent_2_potok.Create(Self, Self, 1);
    T.ShowModal;
    T.Free;
    ActionRefreshExecute(Sender);
end;

procedure Tfm_uo_kontingent_2_main.ActionChangeExecute(Sender: TObject);
var
    T : Tfmuo_kontingent_2_potok;
begin
    if DS.isEmpty then exit;
    T := Tfmuo_kontingent_2_potok.Create(Self, Self, 2);
    T.ShowModal;
    T.Free;
    ActionRefreshExecute(Sender);
end;

procedure Tfm_uo_kontingent_2_main.ActionViewExecute(Sender: TObject);
var
    T : Tfmuo_kontingent_2_potok;
begin
    if DS.isEmpty then exit;
    T := Tfmuo_kontingent_2_potok.Create(Self, Self, 3);
    T.ShowModal;
    T.Free;
end;
