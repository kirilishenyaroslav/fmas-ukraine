...
type
  Tfmuo_sp_type_cikl_pidg_add = class(TForm)
...
var
  fmuo_sp_type_cikl_pidg_add: Tfmuo_sp_type_cikl_pidg_add;

implementation

uses  uo_sp_type_cikl_pidg_main;

{$R *.dfm}



procedure Tfmuo_sp_type_cikl_pidg_add.ActionOKExecute(Sender: TObject);
begin
...
procedure Tfmuo_sp_type_cikl_pidg_add.ActionCanselExecute(Sender: TObject);
begin
   ModalResult:=mrCancel;
end;

procedure Tfmuo_sp_type_cikl_pidg_add.FormCreate(Sender: TObject);
begin
    if Tfm_uo_sp_type_cikl_pidg_main(self.Owner).cap=0 then
        Caption:='Äîäàòè';
    if Tfm_uo_sp_type_cikl_pidg_main(self.Owner).cap=1 then
        Caption:='Çì³íèòè';
    if Tfm_uo_sp_type_cikl_pidg_main(self.Owner).cap=2 then
        Caption:='Ïåğåãëÿä';
...
procedure Tfmuo_sp_type_cikl_pidg_add.cxSpinEditNPPKeyPress(Sender: TObject;
  var Key: Char);
var
    x: Integer;
begin
...
procedure Tfmuo_sp_type_cikl_pidg_add.cxSpinEditKodKeyPress(Sender: TObject;
  var Key: Char);
var
x : Integer;
begin
