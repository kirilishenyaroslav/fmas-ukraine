...
uses
  ...
  cxGridDBTableView, cxGrid, KassaFindForm, FIBDataSet, pFIBDataSet,
  FIBDatabase, pFIBDatabase, Un_R_file_Alex;

type
  TfmKassaSchSelect = class(TForm)
  ...
  private
    myform : TfmKassaFindForm;
  public
  end;

function ShowSchKassa(Owner : TComponent; m : TfmKassaFindForm; var Res : Variant) : boolean;
...
var
    ...
    number, title : string;
begin
    T := TfmKassaSchSelect.Create(nil);
    T.myform := m;
    T.pFIBDatabase1                    := m.d;
...
procedure TfmKassaSchSelect.cxGrid1DBTableView1DblClick(Sender: TObject);
begin
     ModalResult := mrOK;
end;

procedure TfmKassaSchSelect.cxGrid1DBTableView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
    if key = VK_RETURN then  ModalResult := mrOK;
end;

procedure TfmKassaSchSelect.LoadCaption;
begin
