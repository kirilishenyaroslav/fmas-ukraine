...
type
  TfmMain = class(TForm)
    ...
    chNotDismiss: TCheckBox;
    lblNotDismiss: TcxLabel;
...
uses DmEndWorkDog, NagScreenUnit;
...
destructor TEndWorkDog.Destroy;
begin
    inherited Destroy;
end;
...
function CreateSprav: TSprav;stdcall;
begin
    Result := TEndWorkDog.Create;
end;

constructor TEndWorkDog.Create;
begin
    inherited Create;
...
begin
   ...
   // DateWork:=self.WorkEdit.Date;
    ...
    begin
        ...
        DSetData.SelectSQL.Text:='select distinct * from UP_REPORT_END_WORKDOG_INFO(:IN_ID_REPORT_SESSION, :NOT_DISMISS) Order By DISPLAY_2, DISPLAY_1, FIO collate win1251_ua';

        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            if chNotDismiss.Checked then DSetData.ParamByName('NOT_DISMISS').AsString:='T'
            else  DSetData.ParamByName('NOT_DISMISS').AsString:='F';
