...
uses dmReportWorkingPensioners,
     NagScreenUnit;
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text        := 'select distinct * from UP_REPORT_WORKING_PENSIONERS(:IN_ID_REPORT_SESSION) Order By FIO_PENTION Collate Win1251_Ua';
        try
        ...
        Report.Clear;
        dm.DSetGlobalData.Close;
        dm.DSetGlobalData.SQLs.SelectSQL.Text:='select id_server from pub_sys_data';
        dm.DSetGlobalData.Open;
        if dm.DSetGlobalData['id_server']=93 then
        Report.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Up\UpReportWorkPensionerP.fr3',True)
        else
