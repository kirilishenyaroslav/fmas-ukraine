...
begin
    ...
    Name_Rep := Dm.DSet.FieldValues['NAME_REPORT'];
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text := 'select distinct * from UP_REPORT_FORMA_6_PV(:IN_ID_REPORT_SESSION)';
        ...
        DsPension.Active := False;
        DsPension.SelectSQL.Text := 'select distinct * from UP_REPORT_PENSION_COUNT(:IN_ID_REPORT_SESSION, :USE_TYPE)';

        try
            DsPension.ParamByName('IN_ID_REPORT_SESSION').AsInt64 := id_session;
            DsPension.ParamByName('USE_TYPE').AsInt64 := 1;
            DsPension.Active := True;
        except
            on E: Exception do
            begin
                messageBox(0, PChar('Помилка при підготовці звіту!'), 'Помилка!', MB_OK);
        ...
        m1 := TfrxMasterData(Report.FindObject('MasterData2'));
        if m1 <> nil then
        begin
            m1.Font.Name := PFontNames;
            m1.Font.Size := PFontSizes;
            m1.Font.Color := PFontColors;
            m1.Font.Style := PFontStyles;
        end;
