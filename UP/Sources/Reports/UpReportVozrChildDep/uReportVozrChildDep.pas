...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text := 'select distinct * from UP_REPORT_VOZRCHILDDEP(:IN_ID_REPORT_SESSION, :SHOW_WITHOUT_AGE) Order By DISPLAY_ORDER_2, DISPLAY_ORDER_1, NAME_FACULTY, NAME_CAFEDRA, FIO_REB';
        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64 := id_session;
            if cx_ShowWithoutAge.Checked then
                DSetData.ParamByName('SHOW_WITHOUT_AGE').AsInt64 := 1
            else
                DSetData.ParamByName('SHOW_WITHOUT_AGE').AsInt64 := 0;
