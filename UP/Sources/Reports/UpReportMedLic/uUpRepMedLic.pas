...
begin
    ...
    chPeriod.OnClick(Self);
    PeriodBeg.Date:=EncodeDate(YearOf(Date), 1, 1);
    PeriodEnd.Date:=Date;
end;
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text:='select distinct * from UP_REPORT_MED_LIC(:IN_ID_REPORT_SESSION, :IS_USE_PERIOD, :PERIOD_BEG, :PERIOD_END) Order By display_2, display_1,  Fio collate win1251_ua';

        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            if chPeriod.Checked then DSetData.ParamByName('IS_USE_PERIOD').AsString:='T'
            else DSetData.ParamByName('IS_USE_PERIOD').AsString:='F';
            DSetData.ParamByName('PERIOD_BEG').AsDate:=PeriodBeg.Date;
            DSetData.ParamByName('PERIOD_END').AsDate:=PeriodEnd.Date;
...
procedure TfmMain.chPeriodClick(Sender: TObject);
begin
   PeriodBeg.Enabled:=chPeriod.Checked;
   PeriodEnd.Enabled:=chPeriod.Checked;
end;
