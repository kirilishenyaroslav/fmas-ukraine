...
type
  TfrmUPReportLongAgeMain = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportLongAgeMain: TfrmUPReportLongAgeMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportLongAgeMain;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportLongAgeMain.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportLongAgeMain.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportLongAgeMain.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportLongAgeMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportLongAgeMain.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  DSetData.SQLs.SelectSQL.Text:='select * from UP_DT_REPORT_SV_LONG_AGE(:IN_ID_REPORT_SESSION) order by name_faculty collate win1251_ua,name_cafedra collate win1251_ua';
...
procedure TfrmUPReportLongAgeMain.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
...
procedure TfrmUPReportLongAgeMain.FontActionExecute(Sender: TObject);
begin
