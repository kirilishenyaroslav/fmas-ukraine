...
type
  TfrmUpReportForma4Main = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUpReportForma4Main: TfrmUpReportForma4Main;
...
var
    hnd: Integer;
    Print: TfrmUpReportForma4Main;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUpReportForma4Main.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUpReportForma4Main.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUpReportForma4Main.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUpReportForma4Main.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUpReportForma4Main.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  DataSet.Close;
  DataSet.SQLs.SelectSQL.Text:='execute procedure UP_REPORT_FORM4B_POST_INS(:in_id_report_session)';
  DataSet.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
  try
    DataSet.Open;
  ...
  DsetData.Close;
  DSetData.SQLs.SelectSQL.Text:='select * from UP_REPORT_FORMA4B(:IN_ID_REPORT_SESSION)';
  ...
  DataSet.Close;
  DataSet.SQLs.SelectSQL.Text:='DELETE FROM UP_REPORT_FORM4B_POST_TEMP  WHERE ID_SESSION=:IN_ID_REPORT_SESSION';
  DataSet.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
  try
    DataSet.Open;
    DataSet.Transaction.Commit;
...
procedure TfrmUpReportForma4Main.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
