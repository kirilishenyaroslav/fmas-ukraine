...
type
  TfrmUPReportSowmMain = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportSowmMain: TfrmUPReportSowmMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportSowmMain;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportSowmMain.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportSowmMain.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportSowmMain.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportSowmMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportSowmMain.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  NameRep:=NameReport['Name_Report'];

  DSetData.SQLs.SelectSQL.Text:='select distinct * from UP_DT_REPORT_SPISOK_SOVM(:IN_ID_REPORT_SESSION) order by display_2, display_1, fio collate win1251_ua';
  ...
  try
   ...
   Report.Variables['Name_Report']:=QuotedStr(NameRep);
...
procedure TfrmUPReportSowmMain.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
...
procedure TfrmUPReportSowmMain.FontActionExecute(Sender: TObject);
begin
