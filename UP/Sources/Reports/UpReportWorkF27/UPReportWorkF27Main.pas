...
type
  TfrmUPReportWorkF27 = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportWorkF27: TfrmUPReportWorkF27;
...
var
    hnd: Integer;
    Print: TfrmUPReportWorkF27;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportWorkF27.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportWorkF27.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportWorkF27.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportWorkF27.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportWorkF27.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  DSetData.SQLs.SelectSQL.Text:='select * from UP_DT_REPORT_WORK_F27(:IN_ID_REPORT_SESSION)  order by FIO collate win1251_ua';
  ...
  try
   ...
   m := TfrxMasterData(Report.FindObject('DetailData1'));                                            
   if m <> nil then
   begin
...
procedure TfrmUPReportWorkF27.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
...
procedure TfrmUPReportWorkF27.FontActionExecute(Sender: TObject);
begin
