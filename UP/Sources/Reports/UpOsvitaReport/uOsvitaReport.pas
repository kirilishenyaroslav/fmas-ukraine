...
uses
    ...
    Dialogs, cxLookAndFeelPainters, ComCtrls, ActnList, StdCtrls, cxButtons,
    uCommonSp, iBase, DB, FIBDatabase, pFIBDatabase, Gauges, pFIBStoredProc,
    FIBQuery, pFIBQuery, frxDesgn, frxClass, frxDBSet, frxExportXLS,
    FIBDataSet, pFIBDataSet, frxExportHTML, frxExportRTF;

type
    TUP_OsvitaReport = class(TSprav)
    public
        constructor Create;
...
destructor TUP_OsvitaReport.Destroy;
begin
    inherited Destroy;
end;
...
function CreateSprav: TSprav; stdcall;
begin
    Result := TUP_OsvitaReport.Create;
end;

constructor TUP_OsvitaReport.Create;
begin
    inherited Create;
...
procedure TUP_OsvitaReport.Show;
var
    hnd: Integer;
    Print: TfmMain;
begin
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text := 'select distinct * from UP_OSVITA_REPORT(:IN_ID_REPORT_SESSION) Order By Display_Order_Report asc';
        ...
        Report.Variables['DATE_REPORT'] := QuotedStr(DSetData['DATE_REPORT']);
