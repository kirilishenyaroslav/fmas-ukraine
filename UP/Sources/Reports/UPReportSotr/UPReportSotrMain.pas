...
type
    TfrmUPReportSotrMain = class(TForm)
        DB: TpFIBDatabase;
...
var
    frmUPReportSotrMain: TfrmUPReportSotrMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportSotrMain;
begin
    hnd := Input['DBHandle'];
    id_sp_report := Input['id_sp_report'];
    IsDesign := Input['DesignReport'];
    Print := TfrmUPReportSotrMain.Create(Application.MainForm, hnd);
    Print.ShowModal;
    Print.Free;
end;

constructor TfrmUPReportSotrMain.Create(aOwner: TComponent; HandleDb: Integer);
begin
...
procedure TfrmUPReportSotrMain.cxButtonCloseClick(Sender: TObject);
begin
    Close;
end;

procedure TfrmUPReportSotrMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
    try
...
procedure TfrmUPReportSotrMain.PrintActionExecute(Sender: TObject);
var
    SP_DataSet: TpFIBDataSet;
    SP: TpFIBStoredProc;
...
begin
    ...
    DSetData.SQLs.SelectSQL.Text := 'select distinct * from UP_DT_REPORT_SOTR(:IN_ID_REPORT_SESSION) order by category collate win1251_ua,ORDER_DEP,fio collate win1251_ua';
...
procedure TfrmUPReportSotrMain.FormDestroy(Sender: TObject);
begin
    WriteTransaction.StartTransaction;
...
procedure TfrmUPReportSotrMain.FontActionExecute(Sender: TObject);
begin
