...
var
  fmReportBirthday: TfmReportBirthday;
...
uses dmReportBirthdayList, DateUtils;
...
begin
    ...
    Print := TfmReportBirthday.Create(Application.MainForm, hnd);
    Print.ShowModal;
    Print.Free;
    Dm.Free;
end;

constructor TfmReportBirthday.Create(aOwner : TComponent; HandleDb : Integer);
begin
...
procedure TfmReportBirthday.cxButtonCloseClick(Sender: TObject);
begin
    Close;
end;

procedure TfmReportBirthday.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
    try
...
procedure TfmReportBirthday.PrintActionExecute(Sender: TObject);
var
...
begin
    ...
    with Dm do
    begin
        DPeriod.Active                := False;
        DPeriod.SelectSQL.Text        := 'select distinct * from UP_DT_REPORT_VAL_PARAM_START(:IN_ID_REPORT_SESSION)';
        try
            DPeriod.ParamByName('IN_ID_REPORT_SESSION').AsInt64 := id_session;
            DPeriod.Active:=True;
            d_Beg:= DPeriodOUT_DATE_BEG.Value;
            d_end:= DPeriodOUT_DATE_END.Value;
            if  d_Beg > d_end then
            begin
                messageBox(0,PChar('Початок періоду повинен бути меньшим за кінець!'), 'Помилка!', MB_OK);
                exit;
            end;
            d_b:= YearOf(d_Beg);
            d_e:= YearOf(d_end);
            //messageBox(0,PChar(IntToStr(d_b)+' '+IntToStr(d_e)), 'Помилка!', MB_OK);

            if  d_b <> d_e then
            begin
                messageBox(0,PChar('Період повинен бути за один рік!'), 'Помилка!', MB_OK);
                exit;
            end;
            DPeriod.Active:=False;
            except
        ...
        DSetData.SelectSQL.Text        := 'SELECT * FROM UP_REPORT_LIST_BIRTHDAY(:IN_ID_REPORT_SESSION) ORDER BY  EXTRACTMONTH(BIRTH_DATE), EXTRACTDAY(BIRTH_DATE), FIO COLLATE WIN1251_UA';
        try
...
procedure TfmReportBirthday.FontActionExecute(Sender: TObject);
begin
