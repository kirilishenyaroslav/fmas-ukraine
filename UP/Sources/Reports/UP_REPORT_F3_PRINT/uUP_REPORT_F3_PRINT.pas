...
type
  TfmMain = class(TForm)
    ...
    procedure UP_REPORT_F3_PRINT(Sender: TObject);
...
uses dmUP_REPORT_F3_PRINT, NagScreenUnit;
...
procedure TfmMain.UP_REPORT_F3_PRINT(Sender: TObject);
var
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text:='select distinct * from UP_REPORT_F3_PRINT(:IN_ID_REPORT_SESSION) Order by NAME_FACULTY, NAME_CAFEDRA, NUMBER_PERS, FIO Collate Win1251_Ua';

        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            DSetData.Active:=True;

            DSetDataDetail.Active:=False;
            DSetDataDetail.SelectSQL.Text:='select distinct * from UP_REPORT_F3_STAVOK(:IN_ID_REPORT_SESSION, ?loc_id_department, ?loc_date_check) ';
            DSetDataDetail.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            DSetDataDetail.Active:=True;

            DSetFac.Active:=False;
            DSetFac.SelectSQL.Text:='select distinct * from UP_REPORT_F3_STAVOK_FAC(:IN_ID_REPORT_SESSION, ?id_fac_department, ?loc_date_check) ';
            DSetFac.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            DSetFac.Active:=True;

            DSetRoot.Active:=False;
            DSetRoot.SelectSQL.Text:='select distinct * from UP_REPORT_F3_STAVOK_FAC(:IN_ID_REPORT_SESSION, ?id_root_department, ?loc_date_check) ';
            DSetRoot.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            DSetRoot.Active:=True;
            except
            on E:Exception do
            begin
