...
type
  TfrmUPReportTempWorkersMain = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportTempWorkersMain: TfrmUPReportTempWorkersMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportTempWorkersMain;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportTempWorkersMain.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportTempWorkersMain.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportTempWorkersMain.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportTempWorkersMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportTempWorkersMain.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  NameReport.Active:=true;
   ...
   DSetDepartments.SQLs.SelectSQL.Text :='SELECT * FROM UP_DT_REPORT_TEMP_WORKERS(null,:IN_ID_REPORT_SESSION)';
   DSetDepartments.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
   DSetData.SQLs.SelectSQL.Text :='SELECT * FROM UP_DT_REPORT_TEMP_WORKERS_FIOS(?OUT_DATE_CHECK,?OUT_DATE_CHECK_END,?ID_DEPARTMENT,:IN_ID_REPORT_SESSION)';
...
procedure TfrmUPReportTempWorkersMain.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
