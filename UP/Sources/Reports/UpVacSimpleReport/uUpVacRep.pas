...
type
  TfmMain = class(TForm)
    ...
    CBWith_Child: TCheckBox;
    CBWith_Hide: TCheckBox;
...
begin
    ...
    begin
        ...
        DSetData.SelectSQL.Text:='select distinct * from UP_SHTAT_VAC_SIMPLE_SELECT(:IN_ID_REPORT_SESSION,:WITH_CHILD) Order By NAME_DEPARTMENT_UPPER,NAME_DEPARTMENT, NAME_TYPE_POST, POST_NAME COLLATE WIN1251_UA,SMETA_TITLE';
        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64 := id_session;
           { if CBWith_Child.Checked then
                DSetData.ParamByName('WITH_CHILD').AsInt64 := 1
            else DSetData.ParamByName('WITH_CHILD').AsInt64 := 0; }
            DSetData.ParamByName('WITH_CHILD').AsInt64 := 1;
            {if CBWith_Hide.Checked then
                DSetData.ParamByName('WITH_HIDE').AsInt64 := 1
            else DSetData.ParamByName('WITH_HIDE').AsInt64 := 0;}
