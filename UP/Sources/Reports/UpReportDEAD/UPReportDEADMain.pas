...
type
    TfrmUPReportDEADMain = class(TForm)
        DB: TpFIBDatabase;
...
var
    frmUPReportDEADMain: TfrmUPReportDEADMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportDEADMain;
begin
    hnd := Input['DBHandle'];
    id_sp_report := Input['id_sp_report'];
    IsDesign := Input['DesignReport'];
    Print := TfrmUPReportDEADMain.Create(Application.MainForm, hnd);
    Print.ShowModal;
    Print.Free;
end;

constructor TfrmUPReportDEADMain.Create(aOwner: TComponent; HandleDb: Integer);
begin
...
procedure TfrmUPReportDEADMain.cxButtonCloseClick(Sender: TObject);
begin
    Close;
end;

procedure TfrmUPReportDEADMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
    try
...
procedure TfrmUPReportDEADMain.PrintActionExecute(Sender: TObject);
var
    SP_DataSet: TpFIBDataSet;
    SP: TpFIBStoredProc;
...
try
    ...
    NameReport.Open;
    FileFr3 := NameReport.FieldByName('name_fr3').AsString;
    NameReport.Free;
except on E: Exception do ShowMessage(E.Message);
end;
    DSetData.Close;
    DSetGlobalData.Close;
    DSetData.SQLs.SelectSQL.Text := 'select distinct * from UP_DT_REPORT_DEAD(:IN_ID_REPORT_SESSION) order by FIO collate win1251_ua';
    ...
    Report.Clear;
    Report.LoadFromFile(ExtractFilePath(Application.ExeName) + 'Reports\UP\' + FileFr3 + '.fr3');
...
procedure TfrmUPReportDEADMain.FormDestroy(Sender: TObject);
begin
    WriteTransaction.StartTransaction;
...
procedure TfrmUPReportDEADMain.FontActionExecute(Sender: TObject);
begin
