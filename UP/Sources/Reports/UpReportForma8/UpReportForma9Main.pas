...
uses
  ...
  ASUP_LoaderPrintDocs_Messages,ASUP_LoaderPrintDocs_Consts;
...
type
  TfrmUpReportForma9Main = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUpReportForma9Main: TfrmUpReportForma9Main;
...
var
    hnd: Integer;
    Print: TfrmUpReportForma9Main;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUpReportForma9Main.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUpReportForma9Main.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUpReportForma9Main.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUpReportForma9Main.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUpReportForma9Main.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  NameReport.Active:=true;
   ...
   DSetDepartments.SQLs.SelectSQL.Text :='SELECT * FROM UP_REPORT_FORMA9_DEPARTMENTS(null,:IN_ID_REPORT_SESSION)';
   DSetDepartments.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
   DSetData.SQLs.SelectSQL.Text :='SELECT * FROM UP_REPORT_FORMA9_REASONS(?IN_DATE_CHECK,?ID_DEPARTMENT,?USE_REASONS,?USE_DETALISATION,:IN_ID_REPORT_SESSION)';
...
{  DSetData.SQLs.SelectSQL.Text:='select * from UP_REPORT_FORMA4(:IN_ID_REPORT_SESSION)';
  ...
  try
   ...
   Report.Clear;
   Report.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\UP\'+NameReport.FieldByName('name_bpl_fr3').AsString+'.fr3');
...
procedure TfrmUpReportForma9Main.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
