...
type
  TfmMain = class(TForm)
    ...
    RadioGroupCenters: TcxRadioGroup;
...
destructor TUP_UpForma14.Destroy;
begin
    inherited Destroy;
end;
...
function CreateSprav: TSprav;stdcall;
begin
    Result := TUP_UpForma14.Create;
end;

constructor TUP_UpForma14.Create;
begin
    inherited Create;
    ...
    try
        ...
        begin
            ...
            begin
                ...
                if self.RadioGroupCenters.ItemIndex=1
                then FieldValues['view_short_department'] :=1;
                Post;
            end;
            sp.Show;
...
begin
    ...
    if self.RadioGroupCenters.ItemIndex=0 then PID_Centers :='T';
    if self.RadioGroupCenters.ItemIndex=1 then PID_Centers :='F';

    with Dm do
    begin
        DSetData.Active:=False;
        DSetData.SelectSQL.Text:='select distinct * from UP_REPORT_FORMA_14(:IN_ID_REPORT_SESSION,:ID_Centers) Order By NAME_DEPARTMENT';
        try
            DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
            DSetData.ParamByName('ID_Centers').AsString:=PID_Centers;
        ...
        Dm.DSetData1.Close;
        dm.DSetData1.SelectSQL.Text:='select distinct * from UP_REPORT_FORMA_14_2(:IN_ID_REPORT_SESSION)';
        DM.DSetData1.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
        DM.DSetData1.Open;
        Screen.Cursor := crDefault;
        NagScreen.Free;
