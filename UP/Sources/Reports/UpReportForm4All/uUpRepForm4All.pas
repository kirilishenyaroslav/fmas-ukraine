...
begin
    ...
    begin
        DSetData.Close;
        DSetData.SQLs.SelectSQL.Text:='execute procedure UP_REP_FRM4_POST_ALL_INS(:in_id_report_session, :Is_Fact)';
        DSetData.ParamByName('IN_ID_REPORT_SESSION').AsInt64:=id_session;
        DSetData.ParamByName('Is_Fact').AsShort:=Integer(Boolean(isFactPost.Checked));
        try
          DSetData.Open;
          DSetData.Transaction.Commit;
          except on e:exception
          do begin
                MessageDlg('Помилка при підготовці звіту!',mtInformation, [mbOK],0);
        ...
        try
          ...
          on E:Exception do
          begin
              messageBox(0,PChar('Помилка при підготовці звіту!'),'Увага!',MB_OK);
              Exit;
          end;
        end;
