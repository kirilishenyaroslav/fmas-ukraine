...
type
  TfrmUPReportDISMISSEDMain = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportDISMISSEDMain: TfrmUPReportDISMISSEDMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportDISMISSEDMain;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportDISMISSEDMain.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportDISMISSEDMain.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportDISMISSEDMain.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportDISMISSEDMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportDISMISSEDMain.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  DSetData.Close;
  DSetData.SQLs.SelectSQL.Text:='select distinct * from UP_DT_REPORT_DISMISSED(:IN_ID_REPORT_SESSION) order by display_2, display_1, fio collate win1251_ua';
...
procedure TfrmUPReportDISMISSEDMain.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
...
procedure TfrmUPReportDISMISSEDMain.FontActionExecute(Sender: TObject);
begin
