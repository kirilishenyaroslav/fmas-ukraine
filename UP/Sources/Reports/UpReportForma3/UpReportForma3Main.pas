...
type
    TfrmUpReportForma3Main = class(TForm)
        DB: TpFIBDatabase;
...
var
    frmUpReportForma3Main: TfrmUpReportForma3Main;
  ...
  try
    ...
    try
        ...
        begin
            ...
            messageBox(0, PChar('Помилка під час виконання процедури'), 'Помилка!!!', MB_OK);
            Exit;
        end;
    end;
    Transaction.Commit;
    ...
    begin
        messageBox(0, PChar('За такими даними співробітників не знайдено'), 'Увага!!!', MB_OK);
    end;
end;
...
var
    hnd: Integer;
    Print: TfrmUpReportForma3Main;
begin
    hnd := Input['DBHandle'];
    id_sp_report := Input['id_sp_report'];
    IsDesign := Input['DesignReport'];
    Print := TfrmUpReportForma3Main.Create(Application.MainForm, hnd);
    Print.ShowModal;
    Print.Free;
end;

constructor TfrmUpReportForma3Main.Create(aOwner: TComponent; HandleDb: Integer);
begin
...
procedure TfrmUpReportForma3Main.cxButtonCloseClick(Sender: TObject);
begin
    Close;
end;

procedure TfrmUpReportForma3Main.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
    try
...
procedure TfrmUpReportForma3Main.PrintActionExecute(Sender: TObject);
var
    SP_DataSet: TpFIBDataSet;
    SP: TpFIBStoredProc;
...
begin
    ...
    NameReport.SelectSQL.Text := 'select R.name_fr3, name_report from up_dt_report_sp_report  R where R.id_sp_report=:id_sp_report';
    NameReport.ParamByName('id_sp_report').AsInt64 := id_sp_report;
    NameReport.Active := true;
    Name_Rep := NameReport['name_report'];

    DSetData.SQLs.SelectSQL.Text := 'select * from UP_REPORT_FORMA3(:IN_ID_REPORT_SESSION, NULL) order by DISPLAY_2, DISPLAY_1';
    ...
    Report.Variables['NAME_REPORT'] := QuotedStr(Name_Rep);
...
procedure TfrmUpReportForma3Main.FormDestroy(Sender: TObject);
begin
    WriteTransaction.StartTransaction;
...
procedure TfrmUpReportForma3Main.FontActionExecute(Sender: TObject);
begin
