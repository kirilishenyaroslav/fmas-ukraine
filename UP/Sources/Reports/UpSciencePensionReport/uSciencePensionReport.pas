...
//        DSetGlobalData.Active          := False;
        DSetData.SelectSQL.Text        := 'select distinct * from UP_REPORT_DISS_SCIENCE_PENSION(:IN_ID_REPORT_SESSION) Order By FIO';
//        DSetGlobalData.SelectSQL.Text  := 'SELECT FIRM_NAME FROM INI_ASUP_CONSTS';
        try
...
//            DSetGlobalData.Active:=True;
        ...
        begin
            messageBox(0, PChar('За такими данними детей співробітників не знайдено'), 'Увага!', MB_OK);
            Exit;
        end;
...
//        Report.Variables['REGARD_NAME']:=QuotedStr(DSetData['REGARD_NAME']);
