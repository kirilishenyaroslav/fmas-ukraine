...
uses dmReportAwayHoliday, NagScreenUnit;
...
begin
    ...
    NameReport.SelectSQL.Text:='select R.name_bpl from up_dt_report_sp_report  R where R.id_sp_report=:id_sp_report';
    ...
    ReportOptions[0]:=VarArrayOf([NameReport.FieldByName('name_bpl').AsString]);
    ReportOptions[1]:=VarArrayOf([IsDesign]);

    with Dm do
    begin
        DSetData.Active:=False;
        DSetData.SelectSQL.Text:='SELECT distinct * FROM UP_REPORT_AWAY_HOLIDAY (:IN_ID_REPORT_SESSION) Order By Display_Order, Dep_Up_Display_Order, Dep_Up_Display_Order, ';
        DSetData.SQLs.SelectSQL.Text :=DSetData.SQLs.SelectSQL.Text+'Out_Fio_Ua Collate Win1251_Ua, Post_Display_Order, Rate_Count Desc, Id_Work_Reason Desc,Plan_Period_Beg,Plan_Period_End';
