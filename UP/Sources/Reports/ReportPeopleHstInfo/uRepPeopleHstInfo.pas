...
uses
  ...
  FIBDataSet, pFIBDataSet, frxExportHTML, frxExportRTF, ActnList, cxLabel,
  cxControls, cxContainer, cxEdit, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxCalendar, cxSpinEdit, cxTimeEdit, cxCheckBox;
...
type
  TfmMain = class(TForm)
    ...
    teTimeBeg: TcxTimeEdit;
    teTimeEnd: TcxTimeEdit;
    lblTimeBeg: TcxLabel;
    lblTimeEnd: TcxLabel;
    IsOnlyExchanged: TcxCheckBox;
    CheckByMan: TcxCheckBox;
...
begin
    ...
    begin
       ...
       CheckByMan.Checked:=False;
       CheckByMan.Enabled:=False;
    end
    else
    begin
       s:=IntToStr(id_sp_report);
       s:=Copy(s, 2, Length(s)-1);
       IdMan:=StrToInt(s);
       CheckByMan.Checked:=True;
    end;
    ...
    teTimeBeg.Time:=EncodeTime(0,0,0,1);
    teTimeEnd.Time:=EncodeTime(23,59,59,0);
    dePeriodBeg.Date:=Date;
    dePeriodEnd.Date:=Date;
end;
...
var
    ...
    m : TfrxDetailData;
begin
    if DatetoStr(dePeriodBeg.Date)='00.00.0000' then
    begin
      ShowMessage('Ви не заповнили поле "Період з"');
      Exit;
    end;

    if DatetoStr(dePeriodEnd.Date)='00.00.0000' then
    begin
      ShowMessage('Ви не заповнили поле "Період по"');
      Exit;
    end;
    ...
    begin
        ...
        DSetData.SelectSQL.Text:='select distinct * '+
                                 'from PEOPLE_REPORT_HISTORY_INFO(:IN_DATE_BEG, :IN_DATE_END, :IN_TIME_BEG, :IN_TIME_END, :ONLY_EXCHANGED, :IN_ID_MAN) '+
                                 'order by fam collate win1251_ua, imya collate win1251_ua, otch collate win1251_ua, is_suspend, date_modify';
        try
            DSetData.ParamByName('IN_DATE_BEG').AsDate:=dePeriodBeg.Date;
            DSetData.ParamByName('IN_DATE_END').AsDate:=dePeriodEnd.Date;
            DSetData.ParamByName('IN_TIME_BEG').AsTime:=teTimeBeg.Time;
            DSetData.ParamByName('IN_TIME_END').AsTime:=teTimeEnd.Time;
            if IsOnlyExchanged.Checked then DSetData.ParamByName('ONLY_EXCHANGED').AsString:='T'
            else DSetData.ParamByName('ONLY_EXCHANGED').AsString:='F';
            If CheckByMan.Checked then DSetData.ParamByName('IN_ID_MAN').AsInteger:=IdMan
            else DSetData.ParamByName('IN_ID_MAN').AsInteger:=-1;
        ...
        begin
            messageBox(0,PChar('За такими данними фізичних осіб не знайдено'),'Увага!',MB_OK);
            Exit;
        end;

        Report.Clear;
        Report.LoadFromFile(ExtractFilePath(Application.ExeName)+'Reports\Up\ReportPeopleHstInfo.fr3',True);
