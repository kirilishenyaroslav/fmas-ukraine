...
type
  TfrmUPReportAcceptedMain = class(TForm)
    DB: TpFIBDatabase;
...
var
  frmUPReportAcceptedMain: TfrmUPReportAcceptedMain;
...
var
    hnd: Integer;
    Print: TfrmUPReportAcceptedMain;
begin
  hnd := Input['DBHandle'];
  id_sp_report := Input['id_sp_report'];
  IsDesign := Input['DesignReport'];
  Print:=TfrmUPReportAcceptedMain.Create(Application.MainForm,hnd);
  Print.ShowModal;
  Print.Free;
end;

constructor TfrmUPReportAcceptedMain.Create(aOwner : TComponent;HandleDb:Integer);
begin
...
procedure TfrmUPReportAcceptedMain.cxButtonCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmUPReportAcceptedMain.FilterActionExecute(Sender: TObject);
var
    sp: TSprav;
begin
  try
...
procedure TfrmUPReportAcceptedMain.PrintActionExecute(Sender: TObject);
var
  SP_DataSet:TpFIBDataSet;
  SP:TpFIBStoredProc;
...
begin
  ...
  DSetData.SQLs.SelectSQL.Text:='select distinct * from UP_DT_REPORT_ACCEPTED(:IN_ID_REPORT_SESSION) order by display_2, display_1, fio collate win1251_ua';
...
procedure TfrmUPReportAcceptedMain.FormDestroy(Sender: TObject);
begin
  WriteTransaction.StartTransaction;
...
procedure TfrmUPReportAcceptedMain.FontActionExecute(Sender: TObject);
begin
