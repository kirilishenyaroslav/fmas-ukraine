object frmEditTypeInfo: TfrmEditTypeInfo
  Left = 381
  Top = 253
  BorderStyle = bsDialog
  Caption = #1056#1086#1073#1086#1090#1072' '#1079' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1108#1102' '#1087#1086' '#1090#1080#1087#1091' '#1085#1072#1082#1072#1079#1072
  ClientHeight = 264
  ClientWidth = 458
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 11
    Width = 168
    Height = 13
    Caption = #1053#1072#1081#1084#1077#1085#1091#1074#1072#1085#1085#1103' '#1090#1080#1087#1091' '#1085#1072#1082#1072#1079#1072' '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 48
    Top = 43
    Width = 123
    Height = 13
    Caption = #1052#1086#1076#1091#1083#1100' '#1076#1083#1103' '#1086#1073#1088#1086#1073#1082#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 69
    Top = 75
    Width = 102
    Height = 13
    Caption = #1055#1088#1086#1094#1077#1076#1091#1088#1072' '#1076#1088#1091#1082#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 37
    Top = 139
    Width = 144
    Height = 13
    Caption = #1044#1086#1089#1090#1091#1087#1085#1110#1089#1090#1100' '#1076#1083#1103' '#1074#1080#1073#1086#1088#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 87
    Top = 107
    Width = 83
    Height = 13
    Caption = #1064#1072#1073#1083#1086#1085' '#1076#1088#1091#1082#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label6: TLabel
    Left = 39
    Top = 161
    Width = 293
    Height = 13
    Caption = #1042#1080#1082#1086#1088#1080#1089#1090#1086#1074#1091#1074#1072#1090#1080' '#1087#1088#1080' '#1087#1077#1088#1077#1074#1110#1088#1094#1110' '#1083#1086#1075#1110#1095#1085#1080#1093' '#1079#1074#39#1103#1079#1082#1110#1074
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label7: TLabel
    Left = 38
    Top = 182
    Width = 323
    Height = 13
    Caption = #1058#1080#1087' '#1085#1072#1082#1072#1079#1091' - '#1089#1090#1074#1086#1088#1077#1085#1085#1103' '#1110#1085#1092'.'#1087#1088#1086#1089#1090#1086#1088#1091' '#1076#1083#1103' '#1087#1077#1088#1077#1084#1110#1097#1077#1085#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label8: TLabel
    Left = 38
    Top = 203
    Width = 131
    Height = 13
    Caption = #1052#1086#1078#1083#1080#1074#1110#1089#1090#1100' '#1074#1110#1076#1084#1110#1085#1103#1090#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object OkButton: TBitBtn
    Left = 145
    Top = 233
    Width = 97
    Height = 26
    Caption = #1043#1072#1088#1072#1079#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 8
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 247
    Top = 233
    Width = 97
    Height = 26
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 9
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object Title: TcxTextEdit
    Left = 184
    Top = 7
    Width = 265
    Height = 21
    Style.Color = clInfoBk
    TabOrder = 0
  end
  object BplName: TcxTextEdit
    Left = 184
    Top = 39
    Width = 265
    Height = 21
    Style.Color = clInfoBk
    TabOrder = 1
  end
  object PrintProcedure: TcxTextEdit
    Left = 184
    Top = 71
    Width = 265
    Height = 21
    Style.Color = clInfoBk
    TabOrder = 2
  end
  object Is_Active: TcxCheckBox
    Left = 184
    Top = 135
    Width = 25
    Height = 21
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = ''
    TabOrder = 4
  end
  object PrintShablon: TcxTextEdit
    Left = 184
    Top = 103
    Width = 265
    Height = 21
    Style.Color = clInfoBk
    TabOrder = 3
  end
  object use_logic_link: TcxCheckBox
    Left = 334
    Top = 157
    Width = 25
    Height = 21
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = ''
    TabOrder = 5
  end
  object SHOW_IN_PCARD: TcxCheckBox
    Left = 362
    Top = 178
    Width = 25
    Height = 21
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = ''
    TabOrder = 6
  end
  object is_cancel: TcxCheckBox
    Left = 171
    Top = 199
    Width = 25
    Height = 21
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = ''
    TabOrder = 7
  end
  object ActionList1: TActionList
    Left = 9
    Top = 47
    object Action1: TAction
      Caption = 'Action1'
      ShortCut = 13
      OnExecute = OkButtonClick
    end
    object Action2: TAction
      Caption = 'Action2'
      ShortCut = 27
      OnExecute = CancelButtonClick
    end
  end
end
