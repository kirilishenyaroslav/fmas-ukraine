...
begin
  try
    ...
    then szResult := szResult + ' по '+ fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString + ' р. <b> відізвати з </b>'
    else MessageDlg('Помилка під час формування тіла наказу!!', mtError, [mbOk], 0);


    fdsTemplateReceipt.First;
    if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BODY13', []) //дата начала отзыва
    then szResult := szResult + '<b>' +fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString + ' по </b>'
    else MessageDlg('Помилка під час формування тіла наказу!!', mtError, [mbOk], 0);

    fdsTemplateReceipt.First;
    if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BODY14', []) //дата окончания отзыва
    ...
    then szResult := szResult + '<b> на '+ fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString + ' день(ів).</b>'
    else MessageDlg('Помилка під час формування тіла наказу!!', mtError, [mbOk], 0);

    //Проверяем необходимо ли печатать фразу Невикористані.....
    if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BODY18', []) //количество дней
    then begin
              if (Trim(fdsTemplateReceipt.FieldByName('PARAMETR_VALUE').AsString)='T')
              then begin
                        fdsTemplateReceipt.First;
                        if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BODY12', []) //количество дней
                        then szResult := szResult + ' Невикористані '+fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString+' календарних днів відпустки надати за заявою. '
                        else MessageDlg('Помилка під час формування тіла наказу!!', mtError, [mbOk], 0);
              end
              else begin
                        szResult := szResult;
