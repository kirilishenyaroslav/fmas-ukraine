object frmGetExtInfo: TfrmGetExtInfo
  Left = 293
  Top = 203
  BorderStyle = bsDialog
  Caption = 
    #1042#1074#1077#1076#1077#1085#1085#1103' '#1076#1086#1076#1072#1090#1082#1086#1074#1086#1111' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1111' '#1087#1086' '#1085#1072#1082#1072#1079#1091' '#1085#1072' '#1085#1072#1076#1072#1085#1085#1103' '#1085#1072#1076#1073#1072#1074#1082#1080'/'#1076#1086#1087 +
    #1083#1072#1090#1080
  ClientHeight = 370
  ClientWidth = 559
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label3: TLabel
    Left = 15
    Top = 54
    Width = 298
    Height = 13
    Caption = #1057#1091#1084#1110#1097#1077#1085#1085#1103' '#1087#1086#1089#1072#1076' ('#1087#1110#1076#1088#1086#1079#1076#1110#1083' '#1090#1072' '#1087#1086#1089#1072#1076#1086#1074#1080#1081' '#1086#1082#1083#1072#1076'):'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 7
    Top = 268
    Width = 346
    Height = 13
    Caption = #1042#1080#1082#1086#1085#1072#1085#1085#1103' '#1086#1073#1086#1074#39#1103#1079#1082#1110#1074' '#1090#1080#1084#1095#1072#1089#1086#1074#1086' '#1074#1110#1076#1089#1091#1090#1085#1100#1086#1075#1086' '#1087#1088#1072#1094#1110#1074#1085#1080#1082#1072':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Department: TqFSpravControl
    Left = 10
    Top = 75
    Width = 505
    Height = 21
    FieldName = 'Sovm_Id_Department'
    DisplayName = #1055#1110#1076#1088#1086#1079#1076#1110#1083
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 2
    AutoSaveToRegistry = False
    OnOpenSprav = DepartmentOpenSprav
    DisplayText = ''
    DisplayTextField = 'Sovm_Name_Department'
  end
  object PostSalary: TqFSpravControl
    Left = 10
    Top = 154
    Width = 505
    Height = 21
    FieldName = 'Sovm_Id_Post_Salary'
    DisplayName = #1055#1086#1089#1072#1076#1086#1074#1080#1081' '#1086#1082#1083#1072#1076
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = True
    TabOrder = 4
    AutoSaveToRegistry = False
    OnOpenSprav = PostSalaryOpenSprav
    DisplayText = ''
    DisplayTextField = 'Sovm_Name_Post_Salary'
  end
  object qFSC_SovmestFIO: TqFSpravControl
    Left = 14
    Top = 293
    Width = 497
    Height = 21
    FieldName = 'ID_PCARD_abs'
    DisplayName = #1055#1030#1041
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 7
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_SovmestFIOOpenSprav
    DisplayText = ''
    DisplayTextField = 'FIO_abs'
  end
  object OkButton: TBitBtn
    Left = 175
    Top = 330
    Width = 97
    Height = 26
    Caption = #1043#1072#1088#1072#1079#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 8
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 279
    Top = 330
    Width = 97
    Height = 26
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 9
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object SovmKoeff: TqFFloatControl
    Left = 11
    Top = 199
    Width = 249
    Height = 21
    DisplayName = #1050#1086#1077#1092#1110#1094#1110#1108#1085#1090
    Interval = 120
    Value = 0.000000000000000000
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 5
    AutoSaveToRegistry = False
    MaxLength = 0
    NegativeAllowed = False
    Precision = 2
    Format = '#########0.00'
  end
  object TypePost: TqFSpravControl
    Left = 15
    Top = 235
    Width = 497
    Height = 21
    DisplayName = #1058#1080#1087' '#1087#1077#1088#1089#1086#1085#1072#1083#1091
    Interval = 115
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = False
    Asterisk = False
    Enabled = True
    Blocked = True
    TabOrder = 6
    AutoSaveToRegistry = True
    OnOpenSprav = TypePostOpenSprav
    DisplayText = ''
  end
  object PostSalaryShtat: TqFSpravControl
    Left = 9
    Top = 121
    Width = 504
    Height = 21
    FieldName = 'Sovm_Id_Post_Salary'
    DisplayName = #1055#1086#1089#1072#1076#1086#1074#1080#1081' '#1086#1082#1083#1072#1076' '#1064#1056
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = True
    TabOrder = 3
    AutoSaveToRegistry = False
    OnOpenSprav = PostSalaryShtatOpenSprav
    DisplayText = ''
    DisplayTextField = 'Sovm_Name_Post_Salary'
  end
  object ShrId: TqFIntControl
    Left = 16
    Top = 16
    Width = 249
    Height = 21
    DisplayName = #1030#1076#1077#1085#1090#1080#1092#1110#1082#1072#1090#1086#1088' '#1064#1056
    Interval = 105
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 0
    AutoSaveToRegistry = False
    MaxLength = 25
    NegativeAllowed = False
    ZeroAllowed = False
  end
  object btnSHRSearch: TcxButton
    Left = 279
    Top = 14
    Width = 75
    Height = 25
    Caption = #1064#1091#1082#1072#1090#1080
    TabOrder = 1
    OnClick = btnSHRSearchClick
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
  end
  object btnClearinfo: TcxButton
    Left = 367
    Top = 14
    Width = 114
    Height = 25
    Caption = #1054#1095#1080#1089#1090#1080#1090#1080' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1102
    TabOrder = 10
    OnClick = btnClearinfoClick
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
  end
  object SHRSet: TpFIBDataSet
    Database = fmBonusOrder.WorkDatabase
    Transaction = fmBonusOrder.ReadTransaction
    Left = 336
    Top = 40
    poSQLINT64ToBCD = True
  end
  object StorProc: TpFIBStoredProc
    Database = fmBonusOrder.WorkDatabase
    Transaction = ShrTransaction
    Left = 376
    Top = 40
  end
  object ShrTransaction: TpFIBTransaction
    DefaultDatabase = fmBonusOrder.WorkDatabase
    TimeoutAction = TARollback
    Left = 408
    Top = 40
  end
end
