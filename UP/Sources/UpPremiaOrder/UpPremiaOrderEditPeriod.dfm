object frmEditPeriod: TfrmEditPeriod
  Left = 332
  Top = 251
  BorderIcons = [biMinimize, biMaximize]
  BorderStyle = bsDialog
  Caption = #1056#1086#1073#1086#1090#1072' '#1079' '#1110#1085#1092#1086#1088#1084#1072#1094#1110#1108#1102' '#1087#1086' '#1087#1077#1088#1110#1086#1076#1091' '#1087#1088#1077#1084#1110#1111' '#1072#1073#1086' '#1084#1072#1090'.'#1076#1086#1087#1086#1084#1086#1075#1080
  ClientHeight = 247
  ClientWidth = 513
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label6: TLabel
    Left = 15
    Top = 20
    Width = 98
    Height = 13
    Caption = #1055#1086#1095#1072#1090#1086#1082' '#1087#1077#1088#1110#1086#1076#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 15
    Top = 52
    Width = 112
    Height = 13
    Caption = #1047#1072#1082#1110#1085#1095#1077#1085#1085#1103' '#1087#1077#1088#1086#1076#1091
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object OkButton: TBitBtn
    Left = 160
    Top = 213
    Width = 97
    Height = 26
    Action = OKBut
    Caption = #1043#1072#1088#1072#1079#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 5
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 280
    Top = 213
    Width = 97
    Height = 26
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 6
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object DateBeg: TcxDateEdit
    Left = 136
    Top = 16
    Width = 193
    Height = 21
    Style.Color = 16776176
    TabOrder = 0
  end
  object DateEnd: TcxDateEdit
    Left = 136
    Top = 48
    Width = 193
    Height = 21
    Style.Color = 16776176
    TabOrder = 1
  end
  object Per_Department: TqFSpravControl
    Left = 8
    Top = 107
    Width = 502
    Height = 21
    FieldName = 'Id_Department'
    DisplayName = #1055#1110#1076#1088#1086#1079#1076#1110#1083
    Interval = 120
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
    OnOpenSprav = Per_DepartmentOpenSprav
    DisplayText = ''
    DisplayTextField = 'Name_Department'
  end
  object EditForAllPeriod: TcxCheckBox
    Left = 15
    Top = 77
    Width = 289
    Height = 21
    ParentFont = False
    Properties.DisplayUnchecked = 'False'
    Properties.Caption = #1047#1084#1110#1085#1080#1090#1080' '#1087#1077#1088#1110#1086#1076' '#1076#1083#1103' '#1074#1089#1110#1093' '#1074#1110#1076#1082#1088#1080#1090#1080#1093' '#1087#1091#1085#1082#1090#1110#1074
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clGreen
    Style.Font.Height = -11
    Style.Font.Name = 'MS Sans Serif'
    Style.Font.Style = [fsBold]
    TabOrder = 2
  end
  object GroupBox1: TGroupBox
    Left = 5
    Top = 136
    Width = 503
    Height = 70
    Caption = #1047#1084#1110#1085#1072' '#1089#1091#1084#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    object EditForRound: TcxCheckBox
      Left = 10
      Top = 40
      Width = 105
      Height = 21
      ParentFont = False
      Properties.DisplayUnchecked = 'False'
      Properties.Caption = #1054#1082#1088#1091#1075#1083#1077#1085#1085#1103':'
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clGreen
      Style.Font.Height = -11
      Style.Font.Name = 'MS Sans Serif'
      Style.Font.Style = [fsBold]
      TabOrder = 2
      OnClick = EditForRoundClick
    end
    object EditForRecalc: TcxCheckBox
      Left = 10
      Top = 16
      Width = 249
      Height = 21
      ParentFont = False
      Properties.DisplayUnchecked = 'False'
      Properties.Caption = #1055#1077#1088#1077#1092#1086#1088#1084#1091#1074#1072#1085#1085#1103' ('#1074#1074#1077#1076#1110#1090#1100' '#1085#1086#1074#1091' '#1089#1091#1084#1091'):'
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clGreen
      Style.Font.Height = -11
      Style.Font.Name = 'MS Sans Serif'
      Style.Font.Style = [fsBold]
      TabOrder = 0
      OnClick = EditForRecalcClick
    end
    object RecalcEdit: TcxCurrencyEdit
      Left = 272
      Top = 16
      Width = 121
      Height = 21
      Enabled = False
      Properties.DisplayFormat = '0.00'
      Properties.NullString = '0.00'
      TabOrder = 1
    end
    object RndComboBox: TcxComboBox
      Left = 120
      Top = 40
      Width = 169
      Height = 21
      Enabled = False
      ParentFont = False
      Properties.Items.Strings = (
        #1076#1086' '#1094#1110#1083#1086#1075#1086
        #1076#1086' 10 '#1075#1088#1085'.'
        #1076#1086' 100 '#1075#1088#1085'.')
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clGreen
      Style.Font.Height = -11
      Style.Font.Name = 'MS Sans Serif'
      Style.Font.Style = [fsBold]
      TabOrder = 3
      Text = #1076#1086' '#1094#1110#1083#1086#1075#1086
    end
  end
  object ActionList1: TActionList
    Left = 448
    Top = 64
    object OKBut: TAction
      Caption = #1043#1072#1088#1072#1079#1076
      ShortCut = 121
      OnExecute = OKButExecute
    end
  end
end
