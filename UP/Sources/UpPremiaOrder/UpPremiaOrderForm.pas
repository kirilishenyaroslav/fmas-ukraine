...
uses
  ...
  AppEvnts, cxLabel, cxDBLabel, cxCurrencyEdit, UpPremiaOrderReasonForm,
...
uses BaseTypes, AccMgmt, UpPremiaOrderFormEditSm, UPremiaOrderAddNW, FIBQuery,
     UpPremiaOrderEditPeriod, UpKernelUnit, UPremiaOrderShowErrors, UpPremiaNoClone,
     UpOrderDismission;
...
var  warnForm :TfmWarnDismission;
begin
     //Проверка попали ли в приказа уволенніе люди
     WarnDataset.Close;
     WarnDataset.SelectSQL.Text:='SELECT * FROM UP_DISMISSION_FROM_ORDER_SELECT(:id_order_in, :key_session)';
     WarnDataset.ParamByName('id_order_in').AsInt64 := self.IdOrder;
     WarnDataset.ParamByName('key_session').AsInt64 := self.Key_session;
     WarnDataset.Open;

     if not WarnDataset.IsEmpty then
     begin
         warnForm := TfmWarnDismission.Create(Self);
         warnForm.WarningsView.DataController.DataSource := WarnSource;
         if(warnForm.ShowModal=mrYes)then
         begin
             result:=true;
             warnForm.Free;
         end
...
var UpdateSP:TpFibStoredProc;  i, rnd_value, id_item:integer;
begin
     ...
     begin
           ...
           if not VarIsNull(MainOrderDataSet.FieldByName('ID_ORDER_ITEM').Value) then
           Id_item:=StrToInt64(MainOrderDataSet.FieldByName('ID_ORDER_ITEM').AsString)
           else id_item:=0;
           ...
           then begin
                     ...
                     then begin
                               ReportDataView.DataController.BeginUpdate;
                               MainOrderDataSet.First;
                               ...
                               UpdateSP.Free;
                               WriteTransaction.Commit;
                               ReportDataView.DataController.EndUpdate;
                     end
                     else begin
                               UpdateSP:=TpFibStoredProc.Create(self);
                               UpdateSP.Database    :=WorkDatabase;
                     ...
                     Panel12.Visible:=False;
                     GetData;
                     MainOrderDataSet.Locate('ID_ORDER_ITEM',Id_item,[]);
           end;
          Free;
     end;
end;
...
Var id_order_old, cnt_clone, cnt_no_clone: int64;
    ...
    If id_order_old <> -1 Then
        ...
        //Создаем новый пункт приказа в буфферной таблице
          ...
          // agMessageDlg('Увага!', 'По введеним атрибутам не знайдено жодного пункту для клонування!', mtInformation, [mbOk]);
            ...
            GetData;
            cnt_clone:=0;
            cnt_clone:=MainOrderDataSet.RecordCount;
            ...
            //открываем пункты, которые не были клонированы
             with  TfrmNoCloneForm.Create(self) do
             begin
                 GetCloneErrorDS.Database:=WorkDatabase;
                 GetCloneErrorDS.Transaction := ReadTransaction;
                 GetCloneErrorDS.SelectSQL.Text:='SELECT * FROM UP_DT_MAN_PREMIA_CHECK_SEL('+IntToStr(self.Key_session)+')';
                 try
                 GetCloneErrorDS.Open;
                 except
                 on e:Exception do showmessage(e.message);
                 end;
                 GetCloneErrorDS.FetchAll;
                 cnt_no_clone:=0;
                 cnt_no_clone:=GetCloneErrorDS.RecordCount;
                 GetCloneErrorDS.First;

                 cntlbl.Caption:='Оброблено '+inttostr(cnt_clone)+' записи, необроблено '+inttostr(cnt_no_clone);
                if  ShowModal=mrOk then
                begin

                end;
                GetCloneErrorDS.Close;
             end;
...
procedure TfmPremiaOrder.FormResize(Sender: TObject);
begin
    Panel12.Top := (self.ClientHeight div 2) - (Panel12.Height div 2);
    Panel12.Left := (self.ClientWidth div 2) - (Panel12.Width div 2);
end;
