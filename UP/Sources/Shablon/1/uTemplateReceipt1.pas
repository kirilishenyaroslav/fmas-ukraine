...
{        szTemp := fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString;
    ...
    //  Дата начала сотрудничества
        ...
        begin
           ...
           { if ((IdAwayType = 1) and (IdAwayType <> 0)) then
            begin
...
{              else
                ...
                begin
                    ...
                    begin
                        ...
                        if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BONUS_END_BEG_' + IntToStr(i) + '_' + IntToStr(iTemp), []) then
                            if ((fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsDateTime < d_inf) and
                                (IdAwayType <> 0)) then
                                szResult := szResult + 'по ' + fdsTemplateReceipt.FBN('PARAMETR_VALUE').AsString + ' ';
                        szResult := szResult + '</b>';
                        ...
                        if fdsTemplateReceipt.Locate('PARAMETR_NAME', 'BONUS_PERCENT_' + IntToStr(i) + '_' + IntToStr(iTemp), [])
                            then begin
                            if not fdsTemplateReceipt.FBN('PARAMETR_VALUE').isNull
                                then begin
    ...
    //  Информация по основному месту работы
        if (id_serv = 99) then
        begin
