unit uTemplateReceipt22; {Тимчасове переведення на інше місце роботи}
...
function GetTemplateString(hConnection: TISC_DB_HANDLE; id_session, id_item: Int64): string; stdcall;
var
    szResult, stavka_str: string; //  Строка для формирования результирующей строки
    szTemp, s, s1 {, shablon, shadow}: string; //  Строка для хранения текстовой строки
    cTemp: Char; //  ¦хЁхьхээр  фы  їЁрэхэш  ёшьтюыр
    iTemp, k, i, j, m: Integer; //  Переменная для хранения значения целого типа
    iLength, mm, iT, iR: Integer; //  Переменная для хранения значения целого типа длины цикла
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        if DS.Locate('PARAMETR_NAME', 'BONUS_PERCENT_' + IntToStr(i) + '_' + IntToStr(iTemp), []) then
                        begin
                            if not DS.FBN('PARAMETR_VALUE').isNull then
                            begin
                            ...
                            begin
                                if DS.Locate('PARAMETR_NAME', 'BONUS_SUMMA_' + IntToStr(i) + '_' + IntToStr(iTemp), []) then
                                begin
