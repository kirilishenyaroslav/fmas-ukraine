unit uPas32; {Зміна педагогічного навантаження}
...
function GetTemplateString(hConnection: TISC_DB_HANDLE; id_session, id_item: Int64): string; stdcall;
var szResult, stavka_str, hours_str: string; //  Строка для формирования результирующей строки
    szTemp, s, s1, s2 {, shablon, shadow}: string; //  Строка для хранения текстовой строки
    cTemp: Char; //  ¦хЁхьхээр  фы  їЁрэхэш  ёшьтюыр
    iTemp, k, i, j, m, iT: Integer; //  Переменная для хранения значения целого типа
    iLength, mm: Integer; //  Переменная для хранения значения целого типа длины цикла
    ...
    is_b_p, UseDigit: Integer; //Печатать расшифровку по сметам в надбавках
    IsInSovm, AddInfo, BonusSumma, BonusSummaPps, KoeffPps: string;
    PayForm, iR, IdAwayType: Integer;
...
//    func : function(hConnection: TISC_DB_HANDLE; id_session, id_item : Int64): String; stdcall;
    ...
    d_inf: TDateTime;
    flag_bonus, flag_name_cond, flagok, flR, Is_Ped_Work: boolean;
...
//    rate_count := 0;
        ...
        begin
            ...
            if ((IdAwayType = 1) or (IdAwayType = -1)) then szResult := szResult + sMaxDate;
  ...
  //                      if DS.FBN('PARAMETR_VALUE').AsString = 'T' then
                    begin
                        szResult := szResult + 'на умовах ' + s + ', ';
                           { if DS.Locate('PARAMETR_NAME', 'INST_A_DATE_BEG_1_1', []) then
                            begin
                                if not DS.FBN('PARAMETR_VALUE').isNull then
                                begin
    ...
    //////
        ...
        if DS.Locate('PARAMETR_NAME', 'INST_IS_PED_WORK_1_1', []) then
        begin
            if (DS.FBN('PARAMETR_VALUE').AsInteger = 1) then
                Is_Ped_Work := True
        ...
        if Is_Ped_Work then
        begin
            if DS.Locate('PARAMETR_NAME', 'INST_NAME_POST_SALARY_1_1', []) then
//        szResult := szResult + '<b>' + DS.FBN('PARAMETR_VALUE').AsString + '</b>, ';
            ...
            if DS.Locate('PARAMETR_NAME', 'INST_NAME_POST_BASE_SALARY_1_1', []) then
//        szResult := szResult + '<b>' + DS.FBN('PARAMETR_VALUE').AsString + '</b>, ';
        ...
        begin
            ...
            begin
                ...
                for iTemp := 1 to iLength do begin
                    if Is_Ped_Work then
                    begin
                        if DS.Locate('PARAMETR_NAME', 'INST_HOURS_COUNT_' + IntToStr(j) + '_1_' + IntToStr(iTemp), []) then
                        begin
                            hours_count := hours_count + DS.FBN('PARAMETR_VALUE').AsFloat;
                        end;
                        if iLength > 1 then
                        begin

                            szTemp := szTemp + FormatFloat(hours_str, DS.FBN('PARAMETR_VALUE').AsFloat) + ' год. за рахунок ';
...
{        else
        begin
            szTemp := szTemp + '),';
                ...
                if is_ped_work then
                begin
                    szResult := szResult + ' встановити педагогічне навантаження <b>' + FormatFloat(hours_str, hours_count);
                    szResult := szResult + ' год. / ';
                    DS.Locate('PARAMETR_NAME', 'INST_MONTH_HOURS_COUNT_' + IntToStr(j) + '_1', []);
                    szResult := szResult + DS.FBN('PARAMETR_VALUE').AsString + ' міс. </b> ' + szTemp;
                end
                else
                begin
                    ...
                    if not DS.FBN('PARAMETR_VALUE').isNull then
                        ...
                        begin
                            ...
                            begin
                                if Is_Ped_Work then
                                begin
                                    if DS.Locate('PARAMETR_NAME', 'INST_SALARY_SUM_' + IntToStr(j) + '_1', []) then
                                    begin

                                        szResult := szResult + ' ставка заробітної плати ' + DS.FBN('PARAMETR_VALUE').AsString + ' грн.'
                                    end
                                end else
                                    szResult := szResult + ' з окладом ' + FormatFloat('0.00', fTemp) + ' грн. на місяць' + szTemp
                            end;
                        end
                    else
                    begin
                ...
                begin
                    ...
                    begin
                        ...
                        if DS.Locate('PARAMETR_NAME', 'BONUS_END_BEG_' + IntToStr(i) + '_' + IntToStr(iTemp), []) then
                            if ((DS.FBN('PARAMETR_VALUE').AsDateTime < d_inf)
                                and (IdAwayType <> 0)) then
                                szResult := szResult + 'по ' + DS.FBN('PARAMETR_VALUE').AsString + ' ';
                        szResult := szResult + '</b>';
