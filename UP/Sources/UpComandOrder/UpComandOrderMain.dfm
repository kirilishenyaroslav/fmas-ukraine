object FormComandMain: TFormComandMain
  Left = 189
  Top = 156
  Width = 776
  Height = 459
  BorderIcons = [biSystemMenu, biMinimize, biMaximize, biHelp]
  Caption = #1053#1072#1082#1072#1079' '#1085#1072' '#1074#1110#1076#1088#1103#1076#1078#1077#1085#1085#1103
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Visible = True
  OnClose = FormClose
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object BottomPanel: TPanel
    Left = 0
    Top = 380
    Width = 760
    Height = 41
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 0
    DesignSize = (
      760
      41)
    object Panel3: TPanel
      Left = 286
      Top = 0
      Width = 217
      Height = 41
      Anchors = [akTop]
      BevelOuter = bvNone
      TabOrder = 0
      object OkButton: TBitBtn
        Left = 8
        Top = 7
        Width = 97
        Height = 26
        Caption = #1043#1072#1088#1072#1079#1076
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        OnClick = OkButtonClick
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
      object CancelButton: TBitBtn
        Left = 112
        Top = 7
        Width = 97
        Height = 26
        Caption = #1042#1110#1076#1084#1110#1085#1072
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ModalResult = 2
        ParentFont = False
        TabOrder = 1
        OnClick = CancelButtonClick
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
      end
    end
  end
  object TopPanel: TPanel
    Left = 0
    Top = 0
    Width = 760
    Height = 380
    Align = alClient
    TabOrder = 1
    object PanelTypeComand: TPanel
      Left = 1
      Top = 1
      Width = 758
      Height = 41
      Align = alTop
      TabOrder = 0
    end
    object PanelReason: TPanel
      Left = 1
      Top = 304
      Width = 758
      Height = 75
      Align = alBottom
      TabOrder = 1
      object Label1: TLabel
        Left = 5
        Top = 30
        Width = 60
        Height = 13
        Caption = #1055#1088#1080#1084#1110#1090#1082#1072':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clGreen
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 5
        Top = 54
        Width = 58
        Height = 13
        Caption = #1055#1110#1076#1089#1090#1072#1074#1072':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clGreen
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 5
        Top = 6
        Width = 35
        Height = 13
        Caption = #1052#1077#1090#1072':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clGreen
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object cxCPLblNote: TcxDBLabel
        Left = 75
        Top = 28
        Width = 590
        Height = 17
        DataBinding.DataField = 'Note'
        DataBinding.DataSource = MainDS
        AutoSize = False
        ParentFont = False
        TabOrder = 0
      end
      object cxCPLblReason: TcxDBLabel
        Left = 75
        Top = 52
        Width = 590
        Height = 17
        DataBinding.DataField = 'Reason'
        DataBinding.DataSource = MainDS
        AutoSize = False
        ParentFont = False
        TabOrder = 1
      end
      object cxCPLblTarget: TcxDBLabel
        Left = 75
        Top = 5
        Width = 590
        Height = 17
        DataBinding.DataField = 'Target'
        DataBinding.DataSource = MainDS
        AutoSize = False
        ParentFont = False
        TabOrder = 2
      end
    end
    object PanelBar: TPanel
      Left = 1
      Top = 42
      Width = 758
      Height = 47
      Align = alTop
      TabOrder = 2
      object dxBarDockControl1: TdxBarDockControl
        Left = 0
        Top = 0
        Width = 705
        Height = 46
        Align = dalNone
        BarManager = dxCPBarManager
      end
    end
    object cxCPGridMain: TcxGrid
      Left = 1
      Top = 89
      Width = 758
      Height = 215
      Align = alClient
      TabOrder = 3
      object cxCPGridMainTableView: TcxGridDBTableView
        OnKeyDown = cxCPGridMainTableViewKeyDown
        DataController.DataSource = MainDS
        DataController.Filter.Criteria = {FFFFFFFF0000000000}
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        NavigatorButtons.ConfirmDelete = False
        OptionsBehavior.CellHints = True
        OptionsCustomize.ColumnFiltering = False
        OptionsSelection.CellSelect = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.GroupByBox = False
        OptionsView.HeaderAutoHeight = True
        Styles.StyleSheet = qizzStyle
        object colNum_Item: TcxGridDBColumn
          Caption = #8470' '#1087#1091#1085#1082#1090#1091
          Visible = False
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          DataBinding.FieldName = 'Num_Item'
        end
        object colNum_Sub_Item: TcxGridDBColumn
          Caption = #8470' '#1087#1110#1076#1087#1091#1085#1082#1090#1091
          Visible = False
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          DataBinding.FieldName = 'Num_Sub_Item'
        end
        object cxNum_Cert: TcxGridDBColumn
          Caption = #1056#1077#1108#1089#1090#1088'. '#1085#1086#1084#1077#1088
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 46
          DataBinding.FieldName = 'NUM_CERT'
        end
        object cxDate_Cert: TcxGridDBColumn
          Caption = #1056#1077#1108#1089#1090#1088'. '#1076#1072#1090#1072
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 49
          DataBinding.FieldName = 'DATE_CERT'
        end
        object colTN: TcxGridDBColumn
          Caption = #1058#1053
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 30
          DataBinding.FieldName = 'TN'
        end
        object colFIO: TcxGridDBColumn
          Caption = #1055'.'#1030'.'#1041'.'
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 143
          DataBinding.FieldName = 'FIO'
        end
        object colName_Post: TcxGridDBColumn
          Caption = #1055#1086#1089#1072#1076#1072
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 110
          DataBinding.FieldName = 'NAME_POST_FULL'
        end
        object cxName_Org_Full: TcxGridDBColumn
          Caption = #1052#1110#1089#1090#1086' '#1074#1110#1076#1088#1103#1076#1078#1077#1085#1085#1103
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 97
          DataBinding.FieldName = 'NAME_ORG_FULL'
        end
        object cxName_Smet_Full: TcxGridDBColumn
          Caption = #1060#1110#1085#1072#1085#1089#1091#1074#1072#1085#1085#1103
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 134
          DataBinding.FieldName = 'SM_TITLE_FULL'
        end
        object colDate_Beg: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1087#1086#1095#1072#1090#1082#1091
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 52
          DataBinding.FieldName = 'DATE_BEG'
        end
        object colDate_End: TcxGridDBColumn
          Caption = #1044#1072#1090#1072' '#1082#1110#1085#1094#1103
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 53
          DataBinding.FieldName = 'DATE_END'
        end
        object colDays_Count: TcxGridDBColumn
          Caption = #1050'-'#1089#1090#1100' '#1076#1085#1110#1074
          HeaderAlignmentHorz = taCenter
          HeaderAlignmentVert = vaCenter
          Width = 42
          DataBinding.FieldName = 'DAYS_COUNT'
        end
      end
      object cxCPGridMainLevel: TcxGridLevel
        GridView = cxCPGridMainTableView
      end
    end
    object ComandEdit: TqFSpravControl
      Left = 2
      Top = 8
      Width = 679
      Height = 21
      FieldName = 'ID_COMAND_TYPE'
      DisplayName = #1058#1080#1087' '#1074#1110#1076#1088#1103#1076#1078#1077#1085#1085#1103
      Interval = 120
      Value = Null
      LabelColor = clGreen
      Required = True
      Semicolon = True
      Asterisk = True
      Enabled = True
      OnChange = ComandEditChange
      Blocked = False
      TabOrder = 4
      AutoSaveToRegistry = False
      OnOpenSprav = ComandEditOpenSprav
      DisplayText = ''
      DisplayTextField = 'TYPE_NAME'
    end
  end
  object dxCPBarManager: TdxBarManager
    AlwaysMerge = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Bars = <
      item
        AllowCustomizing = False
        AllowQuickCustomizing = False
        BorderStyle = bbsNone
        Caption = 'main'
        DockControl = dxBarDockControl1
        DockedDockControl = dxBarDockControl1
        DockedLeft = 0
        DockedTop = 0
        FloatLeft = 334
        FloatTop = 473
        FloatClientWidth = 23
        FloatClientHeight = 22
        ItemLinks = <
          item
            Item = dxAddButton
            Visible = True
          end
          item
            Item = dxModifyButton
            Visible = True
          end
          item
            Item = dxDeleteButton
            Visible = True
          end
          item
            Item = dxViewButton
            Visible = True
          end>
        Name = 'main'
        OneOnRow = True
        Row = 0
        ShowMark = False
        SizeGrip = False
        UseOwnFont = False
        Visible = True
        WholeRow = True
      end>
    CanCustomize = False
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    HotImages = IL_OrdAcc
    Images = IL_OrdAcc
    LargeImages = IL_OrdAcc
    NotDocking = [dsNone, dsLeft, dsTop, dsRight, dsBottom]
    PopupMenuLinks = <>
    ShowFullMenusAfterDelay = False
    UseSystemFont = True
    Left = 576
    Top = 168
    DockControlHeights = (
      0
      0
      0
      0)
    object dxAddButton: TdxBarLargeButton
      Caption = #1044#1086#1076#1072#1090#1080
      Category = 0
      Hint = #1044#1086#1076#1072#1090#1080' (Ins)'
      Visible = ivAlways
      ImageIndex = 0
      ShortCut = 45
      OnClick = dxAddButtonClick
      AutoGrayScale = False
      HotImageIndex = 0
    end
    object dxModifyButton: TdxBarLargeButton
      Caption = #1047#1084#1110#1085#1080#1090#1080
      Category = 0
      Hint = #1047#1084#1110#1085#1080#1090#1080' (F2)'
      Visible = ivAlways
      ImageIndex = 1
      ShortCut = 113
      OnClick = dxModifyButtonClick
      AutoGrayScale = False
      HotImageIndex = 1
    end
    object dxDeleteButton: TdxBarLargeButton
      Caption = #1042#1080#1076#1072#1083#1080#1090#1080
      Category = 0
      Hint = #1042#1080#1076#1072#1083#1080#1090#1080' (Del)'
      Visible = ivAlways
      DropDownMenu = dxBarPopupMenu1
      ImageIndex = 2
      ShortCut = 46
      OnClick = dxDeleteButtonClick
      AutoGrayScale = False
    end
    object dxViewButton: TdxBarLargeButton
      Caption = #1055#1077#1088#1077#1075#1083#1103#1076
      Category = 0
      Hint = #1055#1077#1088#1077#1075#1083#1103#1076
      Visible = ivAlways
      ImageIndex = 5
      OnClick = dxViewButtonClick
      AutoGrayScale = False
    end
  end
  object dxBarPopupMenu1: TdxBarPopupMenu
    BarManager = dxCPBarManager
    ItemLinks = <>
    UseOwnFont = False
    Left = 608
    Top = 168
  end
  object IL_OrdAcc: TImageList
    Height = 24
    ShareImages = True
    Width = 24
    Left = 576
    Top = 128
    Bitmap = {
      
      000000000000}
  end
  object ActionList1: TActionList
    Left = 640
    Top = 168
    object Add: TAction
      Caption = #1044#1086#1076#1072#1090#1080
      Hint = #1044#1086#1076#1072#1090#1080' (Ins)'
      ImageIndex = 0
      ShortCut = 45
    end
    object Modify: TAction
      Caption = #1047#1084#1110#1085#1080#1090#1080
      Hint = #1047#1084#1110#1085#1080#1090#1080' (F2)'
      ImageIndex = 1
      ShortCut = 113
    end
  end
  object Database: TpFIBDatabase
    DBParams.Strings = (
      ''
      'user_name='
      'password='
      'sql_role_name=')
    DefaultTransaction = ReadTransaction
    SQLDialect = 3
    Timeout = 0
    DesignDBOptions = []
    WaitForRestoreConnect = 0
    Left = 25
    Top = 213
  end
  object ReadTransaction: TpFIBTransaction
    DefaultDatabase = Database
    TimeoutAction = TARollback
    Left = 57
    Top = 213
  end
  object WriteTransaction: TpFIBTransaction
    DefaultDatabase = Database
    TimeoutAction = TARollback
    TPBMode = tpbDefault
    Left = 89
    Top = 213
  end
  object MainCPOrderDataSet: TpFIBDataSet
    Database = Database
    Transaction = ReadTransaction
    Left = 24
    Top = 112
    poSQLINT64ToBCD = True
  end
  object MainDS: TDataSource
    DataSet = MainCPOrderDataSet
    Left = 56
    Top = 112
  end
  object StyleRepository: TcxStyleRepository
    Left = 488
    Top = 120
    object stBackground: TcxStyle
      AssignedValues = [svColor]
      Color = 14745599
    end
    object stContent: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14745599
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object stContentEven: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14745599
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object stContentOdd: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object stFilterBox: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 14811135
      TextColor = clBlack
    end
    object stFooter: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clNavy
    end
    object stGroup: TcxStyle
      AssignedValues = [svColor]
      Color = 14872561
    end
    object stGroupByBox: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = clBtnFace
      TextColor = clBlack
    end
    object stHeader: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16776176
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object stInactive: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object stIncSearch: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = clBtnFace
      TextColor = clBlack
    end
    object stIndicator: TcxStyle
      AssignedValues = [svColor, svFont]
      Color = 15451300
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
    end
    object stPreview: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 14811135
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clNavy
    end
    object stSelection: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clMoneyGreen
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clBlack
    end
    object stHotTrack: TcxStyle
      AssignedValues = [svTextColor]
      TextColor = clMaroon
    end
    object qizzStyle: TcxGridTableViewStyleSheet
      Styles.Background = stBackground
      Styles.Content = stContent
      Styles.ContentEven = stContentEven
      Styles.ContentOdd = stContentOdd
      Styles.Inactive = stInactive
      Styles.IncSearch = stIncSearch
      Styles.Selection = stSelection
      Styles.FilterBox = stFilterBox
      Styles.Footer = stFooter
      Styles.Group = stGroup
      Styles.GroupByBox = stGroupByBox
      Styles.Header = stHeader
      Styles.Indicator = stIndicator
      Styles.Preview = stPreview
      BuiltIn = True
    end
  end
  object StorProc: TpFIBStoredProc
    Database = Database
    Transaction = WriteTransaction
    Left = 528
    Top = 168
  end
  object DSetSelect: TpFIBDataSet
    Database = Database
    Transaction = ReadTransaction
    Left = 216
    Top = 216
    poSQLINT64ToBCD = True
  end
end
