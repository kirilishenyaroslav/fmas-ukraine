...
var
    ...
    NameReport, OrderBy:TpFIBDataSet;
    m : TfrxMasterData;
    SORT_FIELD : STring[50];
begin
    ...
    OrderBy:=TpFIBDataSet.Create(Self);
    OrderBy.Database:=Dm.DB;
    OrderBy.Transaction:=Dm.ReadTransaction;
    OrderBy.SelectSQL.Text:='select sort_field  from UP_DT_REPORT_SORT_PARAM(:in_id_report_session)';
    OrderBy.ParamByName('in_id_report_session').AsInt64:=id_session;
    OrderBy.Active:=true;
    SORT_FIELD := OrderBy.FieldValues['sort_field'];
    with Dm do
    begin
        DSetData.Active:=False;
        DSetData.SelectSQL.Text:='select distinct * from UP_DT_SHTAT_FORMULAR_PPS(:IN_ID_REPORT_SESSION) Order By display_order_tp,NAME_FACULTY,NAME_CAFEDRA, id_work_reas asc,'+SORT_FIELD;
