object Add_Place_Form: TAdd_Place_Form
  Left = 299
  Top = 181
  BorderStyle = bsDialog
  ClientHeight = 196
  ClientWidth = 392
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object qFFC_Place: TqFFormControl
    Left = 280
    Top = 112
    Width = 60
    Height = 21
    ControlsOwner = Owner
    InsertSQL.Strings = (
      
        'select id_place from ADR_PLACE_IU(:where,:ID_REGION,:ID_DISTRICT' +
        ',:ID_PLACE_TYPE,:NAME_PLACE,:ZIPCODE,:IS_DISTRICT_CENTRE,:IS_REG' +
        'IONAL_CENTRE,:IS_CAPITAL);')
    UpdateSQL.Strings = (
      
        'select id_place from ADR_PLACE_IU(:where,:ID_REGION,:ID_DISTRICT' +
        ',:ID_PLACE_TYPE,:NAME_PLACE,:ZIPCODE,:IS_DISTRICT_CENTRE,:IS_REG' +
        'IONAL_CENTRE,:IS_CAPITAL);')
    SelectSQL.Strings = (
      'select * from ADR_PLACE_S(:where);')
    AddCaption = #1044#1086#1076#1072#1090#1080' '#1084#1110#1089#1090#1086
    ModifyCaption = #1047#1084#1110#1085#1080#1090#1080' '#1084#1110#1089#1090#1086
    InfoCaption = #1052#1110#1089#1090#1086
    Mode = fmAdd
    CheckWhere = True
    CloseForm = True
  end
  object qFCC_Name: TqFCharControl
    Left = 8
    Top = 8
    Width = 377
    Height = 21
    FieldName = 'NAME_PLACE'
    DisplayName = #1053#1072#1079#1074#1072
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 0
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFSC_Type: TqFSpravControl
    Left = 8
    Top = 32
    Width = 377
    Height = 21
    FieldName = 'ID_PLACE_TYPE'
    DisplayName = #1058#1080#1087
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 1
    AutoSaveToRegistry = True
    OnOpenSprav = qFSC_TypeOpenSprav
    DisplayText = ''
    DisplayTextField = 'TYPE_NAME'
  end
  object qFSC_Region: TqFSpravControl
    Left = 8
    Top = 56
    Width = 377
    Height = 21
    FieldName = 'ID_REGION'
    DisplayName = #1056#1077#1075#1110#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 2
    AutoSaveToRegistry = True
    OnOpenSprav = qFSC_RegionOpenSprav
    DisplayText = ''
    DisplayTextField = 'NAME_REGION'
  end
  object qFSC_District: TqFSpravControl
    Left = 8
    Top = 80
    Width = 377
    Height = 21
    FieldName = 'ID_DISTRICT'
    DisplayName = #1056#1072#1081#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_DistrictOpenSprav
    DisplayText = ''
    DisplayTextField = 'NAME_DISTRICT'
  end
  object qFCC_Zip: TqFCharControl
    Left = 8
    Top = 112
    Width = 153
    Height = 21
    FieldName = 'ZIPCODE'
    DisplayName = #1030#1085#1076#1077#1082#1089
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 4
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFBC_IsCap: TqFBoolControl
    Left = 8
    Top = 136
    Width = 65
    Height = 21
    FieldName = 'IS_CAPITAL'
    DisplayName = #1057#1090#1086#1083#1080#1094#1103
    Interval = 120
    Value = False
    LabelColor = clGreen
    Required = True
    Semicolon = False
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 5
    AutoSaveToRegistry = False
    StoreAs = StoreAsTF
  end
  object qFBC_IsRC: TqFBoolControl
    Left = 80
    Top = 136
    Width = 121
    Height = 21
    FieldName = 'IS_REGIONAL_CENTRE'
    DisplayName = #1056#1077#1075#1110#1086#1085#1072#1083#1100#1085#1080#1081' '#1094#1077#1085#1090#1088
    Interval = 120
    Value = False
    LabelColor = clGreen
    Required = True
    Semicolon = False
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 6
    AutoSaveToRegistry = False
    StoreAs = StoreAsTF
  end
  object qFBC_IsDC: TqFBoolControl
    Left = 208
    Top = 136
    Width = 97
    Height = 21
    FieldName = 'IS_DISTRICT_CENTRE'
    DisplayName = #1056#1072#1081#1086#1085#1080#1081' '#1094#1077#1085#1090#1088
    Interval = 120
    Value = False
    LabelColor = clGreen
    Required = True
    Semicolon = False
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 7
    AutoSaveToRegistry = False
    StoreAs = StoreAsTF
  end
  object OkButton: TBitBtn
    Left = 37
    Top = 166
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 8
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 258
    Top = 166
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 9
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
end
