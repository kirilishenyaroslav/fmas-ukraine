...
type
  FormMode = (fsmShow, fsmSelect, fsmMSelect);
...
uses {uAddModifForm,} StrUtils, Math;
...
begin
   ...
   DSetMain.SQLs.SelectSQL.Text:='SELECT * FROM '+pUnNormalProcedure+'('''+StringForSearch(SearchEdit.Text)+''')';
   DSetMain.Open;
end;
...
{ Теперь из кода запроса находим данные по умолчанию. Следуют они, если их несколько,
  ...
  if VarIsNull(KeyValue) then
    ViewForm.SetParams(DB.Handle,-1,SpravParams.Additional)
  else
    ViewForm.SetParams(DB.Handle,KeyValue,Null);
