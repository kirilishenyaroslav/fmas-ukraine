object AddModifAdrForm3: TAddModifAdrForm3
  Left = 484
  Top = 172
  BorderStyle = bsDialog
  ClientHeight = 241
  ClientWidth = 492
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object qFFC_Adress: TqFFormControl
    Left = 208
    Top = 216
    Width = 73
    Height = 17
    ControlsOwner = Owner
    InsertSQL.Strings = (
      
        'select * from ADR_ADRESS_MAIN_IUM(:where,:zipcode,:id_city_area,' +
        ':id_street,:korpus,:house,:flat,'#39'01.01.1900'#39','#39'31.12.2500'#39',:id_ad' +
        'r_pk,0);')
    UpdateSQL.Strings = (
      
        'select * from ADR_ADRESS_MAIN_IUM(:where,:zipcode,:id_city_area,' +
        ':id_street,:korpus,:house,:flat,'#39'01.01.1900'#39','#39'31.12.2500'#39',:id_ad' +
        'r_pk,0);'
      '')
    SelectSQL.Strings = (
      'select * from ADR_ADRESS_SEL2(:where);')
    AddCaption = #1044#1086#1076#1072#1090#1080' '#1072#1076#1088#1077#1089#1091
    ModifyCaption = #1047#1084#1110#1085#1080#1090#1080' '#1072#1076#1088#1077#1089#1091
    Mode = fmAdd
    CheckWhere = True
    CloseForm = True
  end
  object qFCC_Korpus: TqFCharControl
    Left = 8
    Top = 152
    Width = 137
    Height = 21
    FieldName = 'korpus'
    DisplayName = #1050#1086#1088#1087#1091#1089
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 6
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFCC_House: TqFCharControl
    Left = 178
    Top = 152
    Width = 137
    Height = 21
    FieldName = 'house'
    DisplayName = #1041#1091#1076#1080#1085#1086#1082
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 7
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFCC_Flat: TqFCharControl
    Left = 344
    Top = 152
    Width = 137
    Height = 21
    FieldName = 'flat'
    DisplayName = #1050#1074#1072#1088#1090#1080#1088#1072
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 8
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFCC_Zip: TqFCharControl
    Left = 146
    Top = 176
    Width = 200
    Height = 21
    FieldName = 'zipcode'
    DisplayName = #1030#1085#1076#1077#1082#1089
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 9
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object OkButton: TBitBtn
    Left = 85
    Top = 211
    Width = 97
    Height = 26
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 10
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 309
    Top = 211
    Width = 97
    Height = 26
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 11
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object qFSC_CArea: TqFSpravControl
    Left = 8
    Top = 128
    Width = 473
    Height = 21
    FieldName = 'id_city_area'
    DisplayName = #1056#1072#1081#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 5
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_CAreaOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_city_area'
  end
  object qFSC_Street: TqFSpravControl
    Left = 8
    Top = 104
    Width = 473
    Height = 21
    FieldName = 'id_street'
    DisplayName = #1042#1091#1083#1080#1094#1103
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 4
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_StreetOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_street'
  end
  object qFIC_AdrPK: TqFInvisControl
    Left = 416
    Top = 200
    Width = 57
    Height = 21
    FieldName = 'id_adr_pk'
    DisplayName = #1053#1072#1079#1074#1072' '#1087#1086#1083#1103
    Interval = 120
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 12
    AutoSaveToRegistry = False
  end
  object qFSC_Country: TqFSpravControl
    Left = 8
    Top = 8
    Width = 473
    Height = 21
    FieldName = 'id_country'
    DisplayName = #1050#1088#1072#1111#1085#1072
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    OnChange = qFSC_CountryChange
    Blocked = False
    TabOrder = 0
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_CountryOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_country'
  end
  object qFSC_Region: TqFSpravControl
    Left = 8
    Top = 32
    Width = 473
    Height = 21
    FieldName = 'id_district'
    DisplayName = #1056#1077#1075#1110#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    OnChange = qFSC_RegionChange
    Blocked = False
    TabOrder = 1
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_RegionOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_district'
  end
  object qFSC_District: TqFSpravControl
    Left = 8
    Top = 56
    Width = 473
    Height = 21
    FieldName = 'id_region'
    DisplayName = #1056#1072#1081#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    OnChange = qFSC_DistrictChange
    Blocked = False
    TabOrder = 2
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_DistrictOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_region'
  end
  object qFSC_Place: TqFSpravControl
    Left = 8
    Top = 80
    Width = 473
    Height = 21
    FieldName = 'id_place'
    DisplayName = #1052#1110#1089#1090#1086
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    OnChange = qFSC_PlaceChange
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_PlaceOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_place'
  end
end
