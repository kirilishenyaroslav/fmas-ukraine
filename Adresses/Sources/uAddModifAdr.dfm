object AddModifAdrForm: TAddModifAdrForm
  Left = 352
  Top = 311
  BorderStyle = bsDialog
  ClientHeight = 185
  ClientWidth = 492
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object qFFC_Adress: TqFFormControl
    Left = 216
    Top = 160
    Width = 73
    Height = 17
    ControlsOwner = Owner
    InsertSQL.Strings = (
      
        'select * from ADR_ADRESS_MAIN_IUM(:where,:zipcode,:id_city_area,' +
        ':id_street,:korpus,:house,:flat,:date_beg,:date_end,:id_adr_pk,:' +
        'is_modif);')
    UpdateSQL.Strings = (
      
        'select * from ADR_ADRESS_MAIN_IUM(:where,:zipcode,:id_city_area,' +
        ':id_street,:korpus,:house,:flat,:date_beg,:date_end,:id_adr_pk,:' +
        'is_modif);'
      '')
    SelectSQL.Strings = (
      'select * from ADR_ADRESS_SEL(:where);')
    AddCaption = #1044#1086#1076#1072#1090#1080' '#1072#1076#1088#1077#1089#1091
    ModifyCaption = #1047#1084#1110#1085#1080#1090#1080' '#1072#1076#1088#1077#1089#1091
    Mode = fmAdd
    CheckWhere = True
    CloseForm = True
  end
  object qFSimpleCheck1: TqFSimpleCheck
    Left = qFDC_DateBeg
    Top = 24
    Width = 36
    Height = 21
    Right = qFDC_DateEnd
  end
  object qFDC_DateBeg: TqFDateControl
    Left = 95
    Top = 24
    Width = 129
    Height = 21
    FieldName = 'Date_beg'
    DisplayName = #1047
    Interval = 40
    Value = 38651
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 2
    AutoSaveToRegistry = False
  end
  object qFDC_DateEnd: TqFDateControl
    Left = 279
    Top = 24
    Width = 129
    Height = 21
    FieldName = 'Date_end'
    DisplayName = #1044#1086
    Interval = 40
    Value = 38651
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 3
    AutoSaveToRegistry = False
  end
  object qFSC_Adress: TqFSpravControl
    Left = 8
    Top = 56
    Width = 473
    Height = 21
    FieldName = 'id_street'
    DisplayName = #1040#1076#1088#1077#1089#1072
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    OnChange = qFSC_AdressChange
    Blocked = False
    TabOrder = 4
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_AdressOpenSprav
    DisplayText = ''
    DisplayTextField = 'adress_name'
  end
  object qFCC_Korpus: TqFCharControl
    Left = 8
    Top = 104
    Width = 137
    Height = 21
    FieldName = 'korpus'
    DisplayName = #1050#1086#1088#1087#1091#1089
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 6
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFCC_House: TqFCharControl
    Left = 178
    Top = 104
    Width = 137
    Height = 21
    FieldName = 'house'
    DisplayName = #1041#1091#1076#1080#1085#1086#1082
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 7
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object qFCC_Flat: TqFCharControl
    Left = 344
    Top = 104
    Width = 137
    Height = 21
    FieldName = 'flat'
    DisplayName = #1050#1074#1072#1088#1090#1080#1088#1072
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 8
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object OkButton: TBitBtn
    Left = 86
    Top = 155
    Width = 97
    Height = 25
    Caption = #1043#1072#1088#1072#1079#1076
    Default = True
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 10
    OnClick = OkButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 310
    Top = 155
    Width = 97
    Height = 25
    Cancel = True
    Caption = #1042#1110#1076#1084#1110#1085#1072
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ModalResult = 2
    ParentFont = False
    TabOrder = 11
    OnClick = CancelButtonClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object qFSC_CArea: TqFSpravControl
    Left = 8
    Top = 80
    Width = 473
    Height = 21
    FieldName = 'id_city_area'
    DisplayName = #1056#1072#1081#1086#1085
    Interval = 60
    Value = Null
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 5
    AutoSaveToRegistry = False
    OnOpenSprav = qFSC_CAreaOpenSprav
    DisplayText = ''
    DisplayTextField = 'name_city_area'
  end
  object qFIC_IsModif: TqFInvisControl
    Left = 8
    Top = 8
    Width = 57
    Height = 21
    FieldName = 'is_modif'
    DisplayName = #1053#1072#1079#1074#1072' '#1087#1086#1083#1103
    Interval = 120
    Value = 1
    LabelColor = clGreen
    Required = True
    Semicolon = True
    Asterisk = True
    Enabled = True
    Default = 1
    Blocked = False
    TabOrder = 12
    AutoSaveToRegistry = False
  end
  object qFIC_AdrPK: TqFInvisControl
    Left = 432
    Top = 8
    Width = 57
    Height = 21
    FieldName = 'id_adr_pk'
    DisplayName = #1053#1072#1079#1074#1072' '#1087#1086#1083#1103
    Interval = 120
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = True
    Enabled = True
    Blocked = False
    TabOrder = 13
    AutoSaveToRegistry = False
  end
  object cxRB_NewModif: TcxRadioButton
    Left = 107
    Top = 0
    Width = 113
    Height = 17
    Hint = #1047#1084#1110#1085#1080#1090#1080' '#1072#1076#1088#1077#1089#1091' '#1085#1072' '#1085#1086#1074#1080#1081' '#1087#1077#1088#1110#1086#1076
    Caption = #1047#1084#1110#1085#1080#1090#1080
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    TabStop = True
    OnClick = cxRB_NewModifClick
  end
  object cxRB_Modif: TcxRadioButton
    Left = 283
    Top = 0
    Width = 113
    Height = 17
    Hint = #1042#1110#1076#1088#1077#1076#1072#1075#1091#1074#1072#1090#1080' '#1072#1076#1088#1077#1089#1091
    Caption = #1056#1077#1076#1072#1075#1091#1074#1072#1090#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    OnClick = cxRB_NewModifClick
  end
  object qFCC_Zip: TqFCharControl
    Left = 146
    Top = 128
    Width = 200
    Height = 21
    FieldName = 'zipcode'
    DisplayName = #1030#1085#1076#1077#1082#1089
    Interval = 60
    Value = ''
    LabelColor = clGreen
    Required = False
    Semicolon = True
    Asterisk = False
    Enabled = True
    Blocked = False
    TabOrder = 9
    AutoSaveToRegistry = False
    MaxLength = 0
  end
  object pFIBDS_SelPlace: TpFIBDataSet
    SelectSQL.Strings = (
      'select id_place from adr_street where id_street=:id_street;')
    Left = 24
    Top = 144
    poSQLINT64ToBCD = True
  end
end
