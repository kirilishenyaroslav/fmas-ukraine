...
uses
  ...
  Dialogs, uSpecKlassSpravEdit, cxLookAndFeelPainters, ActnList, StdCtrls,
  cxButtons, cxTextEdit, cxControls, cxContainer, cxEdit, cxLabel, ExtCtrls,
  cxMaskEdit, cxButtonEdit,uPUB_SP_SPEC, uPrK_Resources;

type
  TFormPUB_SP_SPEC_Edit = class(TFormSpecKlassSpravEdit)
...
implementation
uses  uConstants,uPrK_Loader,AArray, uSpecKlassSprav, FIBDatabase;
{$R *.dfm}
...
begin
  inherited;
  cxLabelUSpec.Caption :=ncolUSPEC_NAME[IndLangEdit];
  cxLabelMova.Caption  :=ncolMOVA[IndLangEdit];
  cxLabelFullName.Caption:=ncolFullName[IndLangEdit];
end;
...
var
  Res:Variant;
begin
  Res:=ShowSprav(Self,TFormPUB_SP_SPEC(self.Owner).DataBasePrKSprav.Handle,PUB_SP_USPEC,fsNormal);
...
var
  Res:Variant;
begin
  Res:=ShowSprav(Self,TFormPUB_SP_SPEC(self.Owner).DataBasePrKSprav.Handle,PUB_SP_MOVA,fsNormal);
  if VarArrayDimCount(Res)>0 then
    begin
