...
type
  TFormPrK_PZ_Abit_Show = class(TForm)
...
var
  FormPrK_PZ_Abit_Show: TFormPrK_PZ_Abit_Show;

implementation
uses
 uPrK_PROT_ZACH,uPrK_Resources,
 uConstants,uPrK_PZ_Abit_Show_Add;
{$R *.dfm}

{ TFormPrK_PZ_Abit_Show }

constructor TFormPrK_PZ_Abit_Show.Create(aOwner: TComponent;
  aParam: TAArray);
begin
  ...
  if DataPZAS['Prot_Prikaz'].AsInteger=1
   then begin
       ...
       cxTextEditNomer.Text   := TFormPrK_PROT_ZACH(Self.Owner).DataSetPrKPZ.FieldValues['NOMER'];
       cxDateEditDate.Date    := TFormPrK_PROT_ZACH(Self.Owner).DataSetPrKPZ.FieldValues['DATE_FORM'];
   end
   else begin
       // Приказ
       cxGroupBoxPrik.Visible    := True;
       cxDateEditDateZarah.Date  := TFormPrK_PROT_ZACH(Self.Owner).DataSetPZ_Right.FieldValues['DATE_ZARAX'];
       cxDateEditDatePrikaz.Date := TFormPrK_PROT_ZACH(Self.Owner).DataSetPZ_Right.FieldValues['DATE_PRIKAZ'];
       cxTextEditNomerPr.Text    := TFormPrK_PROT_ZACH(Self.Owner).DataSetPZ_Right.FieldValues['NOMER'];
   end;
  SelectSQLTextPZAS := DataPZAS['SelectSQL_Text'].AsString;
end;

procedure TFormPrK_PZ_Abit_Show.InicCaption;
begin
 LangEdit :=SelectLanguage;
    ...
    cxGridPrKPZ_AbitShowDBTableView1.Styles.Header     :=TFormPrK_PROT_ZACH(Self.Owner).cxStyleHeader;
    cxGridPrKPZ_AbitShowDBTableView1.Styles.Background :=TFormPrK_PROT_ZACH(Self.Owner).cxStyleBackGround_Content;
    cxGridPrKPZ_AbitShowDBTableView1.Styles.Content    :=TFormPrK_PROT_ZACH(Self.Owner).cxStyleBackGround_Content;
    cxGridPrKPZ_AbitShowDBTableView1.Styles.Selection  :=TFormPrK_PROT_ZACH(Self.Owner).cxStyleSelection;
    cxGridPrKPZ_AbitShowDBTableView1.Styles.Inactive   :=TFormPrK_PROT_ZACH(Self.Owner).cxStyleInactive;
...
procedure TFormPrK_PZ_Abit_Show.Obnovit(Id_Obnov: int64);
begin
  DataSetPrKPZ_AbitShow.CloseOpen(false);
  DataSetPrKPZ_AbitShow.Locate('ID_DT_ABIT',Id_Obnov,[]);
end;

procedure TFormPrK_PZ_Abit_Show.SetSelectSQLTextPZAS(const Value: String);
begin
  FSelectSQLTextPZAS := Value;
    DataSetPrKPZ_AbitShow.Active                      :=false;
    DataSetPrKPZ_AbitShow.Database                    :=TFormPrK_PROT_ZACH(self.Owner).DataBasePrk;
    DataSetPrKPZ_AbitShow.Transaction                 :=TFormPrK_PROT_ZACH(self.Owner).TransactionReadPrK;
    ...
    DataSetPrKPZ_AbitShow.ParamByName('ID_SP_GOD_NABORA').AsInt64               :=TFormPrK_PROT_ZACH(self.Owner).ID_GOD_NABORA_GLOBAL;
    DataSetPrKPZ_AbitShow.CloseOpen(true);
end;

procedure TFormPrK_PZ_Abit_Show.ActionOtmenaExecute(Sender: TObject);
begin
   close;
end;

procedure TFormPrK_PZ_Abit_Show.ActionADDExecute(Sender: TObject);
var
   DataPrAdd :TAArray;
   T:TFormPrK_PZ_Abit_Show_Add;
   TryAgain :boolean;
begin
   TryAgain:=false;
   ...
   T := TFormPrK_PZ_Abit_Show_Add.Create(self,DataPrAdd);
   ...
   if T.ShowModal=MrOk then
    begin
       ...
       TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.StoredProcName:=DataPZAS['StoredProcAddName'].AsString;
       TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.Prepare;
       TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.ParamByName(DataPZAS['ParamName'].AsString).AsInt64 :=DataPZAS['ParamValue'].AsInt64;
       TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.ParamByName('ID_DT_ABIT').AsInt64                   :=T.DataSetPrKPZ_AbitShow_Add.FieldValues['ID_DT_ABIT'];
...
procedure TFormPrK_PZ_Abit_Show.ActionObnovExecute(Sender: TObject);
begin
   if DataSetPrKPZ_AbitShow.FieldValues['ID_DT_ABIT']<>Null
...
procedure TFormPrK_PZ_Abit_Show.ActionDELExecute(Sender: TObject);
begin
    ...
    if DataSetPrKPZ_AbitShow.FieldValues['ID_DT_ABIT']<>Null then
     begin
       if prkMessageDlg(nMsgBoxTitle[LangEdit],nMsgDel[LangEdit],
         ...
         begin
            TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.Transaction.StartTransaction;
            TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.StoredProcName:=DataPZAS['StoredProcDelName'].AsString;
            TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.Prepare;
            TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.ParamByName('ID_DT_ABIT').AsInt64     := DataSetPrKPZ_AbitShow.FieldValues['ID_DT_ABIT'];
            TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.ParamByName('ID_USER').AsInt64        := TFormPrK_PROT_ZACH(self.Owner).ID_USER_GLOBAL;
            try
              TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.ExecProc;
              TFormPrK_PROT_ZACH(self.Owner).StoredProcPrK.Transaction.Commit;
