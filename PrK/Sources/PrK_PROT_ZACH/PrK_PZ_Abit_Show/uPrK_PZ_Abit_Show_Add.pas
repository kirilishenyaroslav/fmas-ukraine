...
type
  TFormPrK_PZ_Abit_Show_Add = class(TForm)
...
var
  FormPrK_PZ_Abit_Show_Add: TFormPrK_PZ_Abit_Show_Add;
...
procedure TFormPrK_PZ_Abit_Show_Add.InicCaption;
begin
    ...
    cxGridPrKPZ_AbitShow_AddDBTableView1.Styles.Header     :=TFormPrK_PROT_ZACH(Self.Owner.Owner).cxStyleHeader;
    cxGridPrKPZ_AbitShow_AddDBTableView1.Styles.Background :=TFormPrK_PROT_ZACH(Self.Owner.Owner).cxStyleBackGround_Content;
    cxGridPrKPZ_AbitShow_AddDBTableView1.Styles.Content    :=TFormPrK_PROT_ZACH(Self.Owner.Owner).cxStyleBackGround_Content;
    cxGridPrKPZ_AbitShow_AddDBTableView1.Styles.Selection  :=TFormPrK_PROT_ZACH(Self.Owner.Owner).cxStyleSelection;
    cxGridPrKPZ_AbitShow_AddDBTableView1.Styles.Inactive   :=TFormPrK_PROT_ZACH(Self.Owner.Owner).cxStyleInactive;
...
procedure TFormPrK_PZ_Abit_Show_Add.ActionVibratExecute(Sender: TObject);
begin
   if DataSetPrKPZ_AbitShow_Add.FieldValues['ID_DT_ABIT']<>null
    then  ModalResult:=MrOK;
end;

procedure TFormPrK_PZ_Abit_Show_Add.ActionObnovExecute(Sender: TObject);
Var
  id_Obnov: Int64;
begin
  if DataSetPrKPZ_AbitShow_Add.FieldValues['ID_DT_ABIT']<>Null
...
procedure TFormPrK_PZ_Abit_Show_Add.ActionOtmenaExecute(Sender: TObject);
begin
   close;
end;

constructor TFormPrK_PZ_Abit_Show_Add.Create(aOwner: TComponent;
  aParam: TAArray);
begin
...
procedure TFormPrK_PZ_Abit_Show_Add.SetSelectSQLTextPZASAdd(
  const Value: String);
begin
  FSelectSQLTextPZASAdd := Value;
    DataSetPrKPZ_AbitShow_Add.Active                      :=false;
    DataSetPrKPZ_AbitShow_Add.Database                    :=TFormPrK_PROT_ZACH(self.Owner.Owner).DataBasePrk;
    DataSetPrKPZ_AbitShow_Add.Transaction                 :=TFormPrK_PROT_ZACH(self.Owner.Owner).TransactionReadPrK;
    DataSetPrKPZ_AbitShow_Add.SelectSQL.Clear;
    DataSetPrKPZ_AbitShow_Add.SQLs.SelectSQL.Text         :=FSelectSQLTextPZASAdd;
    DataSetPrKPZ_AbitShow_Add.ParamByName('ID_SP_GOD_NABORA').AsInt64               :=TFormPrK_PROT_ZACH(self.Owner.Owner).ID_GOD_NABORA_GLOBAL;
    DataSetPrKPZ_AbitShow_Add.CloseOpen(false);
end;
