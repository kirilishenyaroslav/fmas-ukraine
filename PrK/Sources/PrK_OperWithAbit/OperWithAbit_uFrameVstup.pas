...
uses
  ...
  cxListBox,uPrK_Resources, cxButtonEdit, cxCalendar;
...
type
  TFrameVstup = class(TFrame)
    ...
    cxButtonEditTypePerekl: TcxButtonEdit;
    cxLabeTypePerekl: TcxLabel;
    cxDateEditDatePerekl: TcxDateEdit;
    cxLabelDatePerekl: TcxLabel;
    ...
    procedure cxButtonEditTypePereklKeyPress(Sender: TObject;
      var Key: Char);
    procedure cxButtonEditTypePereklPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
  private
...
implementation
uses
    OperWithAbit_Perekl,uConstants,uPrK_Loader, DateUtils;
{$R *.dfm}
...
var
   i: integer;
begin
 i:=TFPrKPereklad(self.Owner).IndLangAbit;
 ...
 cxLabeTypePerekl.Caption  :=nLabelTypePerekl[i];

 cxLabelNomerDela.Caption  :=nLabelNomerDela[i];
 cxLabelNomerDela_Pristavka.Caption:='';

 cxButtonEditPotok.Text      := '';
 cxButtonEditTypePerekl.Text := '';
 cxTextEditNomerDela.Text    := '0';

 cxDateEditDatePerekl.Date:=Today;
 cxLabelDatePerekl.Caption:=nLabelDatePerekl[i];
end;
...
begin
  ...
  i:=TFPrKPereklad(self.Owner).ID_GOD_NABORA_GLOBAL;
...
begin
  FSelectSQLTextDergZakaz := Value;
    ...
    DataSetDergZakaz.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetDergZakaz.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextFacul := Value;
    ...
    DataSetFacul.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetFacul.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextFormStudy := Value;
    ...
    DataSetFormStudy.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetFormStudy.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextKatStud := Value;
    ...
    DataSetKatStud.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetKatStud.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextKurs := Value;
    ...
    DataSetKurs.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetKurs.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextSpec := Value;
    ...
    DataSetSpec.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetSpec.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextSrokOb := Value;
    ...
    DataSetSrokOb.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetSrokOb.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextTypeDepart := Value;
    ...
    DataSetTypeDepart.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetTypeDepart.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextVid_Dii := Value;
    ...
    DataSetVid_Dii.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetVid_Dii.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  FSelectSQLTextNpk := Value;
    ...
    DataSetNpk.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetNpk.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
...
begin
  ...
  if trim(cxLookupComboBoxNpk.Text)='' then
   ...
   if (trim(cxButtonEditPotok.Text)='') or
      (TFPrKPereklad(self.Owner).ResultArray['AbitData']['ID_SP_POTOK'].AsInt64=-1) then
   begin
   ...
   if (trim(cxButtonEditTypePerekl.Text)='') or
      (TFPrKPereklad(self.Owner).ResultArray['AbitData']['ID_SP_TYPE_PEREKL'].AsInt64=-1) then
   begin
     ShowMessage(nMsgEmptyTypePerekl[i]);
     cxButtonEditTypePerekl.SetFocus;
     result:=1;
     exit;
   end;
...
var
  res: Variant;
begin
  res :=  uPrK_Loader.ShowPrkSprav(self,TFPrKPereklad(self.Owner).DataBasePrK_ABIT.Handle,PrK_SP_POTOK,fsNormal);
  if VarArrayDimCount(res) > 0 then
  begin
    if res[0]<>NULL THEN
    begin
        TFPrKPereklad(self.Owner).ResultArray['AbitData']['ID_SP_POTOK'].AsInt64      := Res[0];
        TFPrKPereklad(self.Owner).ResultArray['AbitData']['Name_SP_POTOK'].AsString   := Res[2];
        cxButtonEditPotok.Text      :=TFPrKPereklad(self.Owner).ResultArray['AbitData']['Name_SP_POTOK'].AsString;
    end;
  end;
end;
...
begin
    ...
    DataSetNomerDela.Database                       := TFPrKPereklad(self.Owner).DataBasePrK_ABIT;
    DataSetNomerDela.Transaction                    := TFPrKPereklad(self.Owner).TransactionReadPrK_ABIT;
    ...
    DataSetNomerDela.ParamByName('ID_SP_GOD_NABORA').AsInt64      :=TFPrKPereklad(self.Owner).ID_GOD_NABORA_GLOBAL;
    DataSetNomerDela.ParamByName('ID_CN_SP_FAK').AsInt64          :=DataSetFacul.FieldValues['ID_CN_SP_FAK'];
    DataSetNomerDela.ParamByName('ID_CN_JN_FACUL_SPEC').AsInt64   :=DataSetSpec.FieldValues['ID_CN_JN_FACUL_SPEC'];
    DataSetNomerDela.ParamByName('ID_CN_SP_FORM_STUD').AsInt64    :=DataSetFormStudy.FieldValues['ID_CN_SP_FORM_STUD'];
    DataSetNomerDela.ParamByName('ID_SP_DERG_ZAKAZ').AsInt64      :=DataSetDergZakaz.FieldValues['ID_SP_DERG_ZAKAZ'];
    DataSetNomerDela.ParamByName('ID_CN_SP_KAT_STUD').AsInt64     :=DataSetKatStud.FieldValues['ID_CN_SP_KAT_STUD'];
    DataSetNomerDela.ParamByName('ID_SP_VID_DII').AsInt64         :=DataSetVid_Dii.FieldValues['ID_SP_VID_DII'];
...
begin
   IdGodNabLocal   :=IntToStr(TFPrKPereklad(self.Owner).ID_GOD_NABORA_GLOBAL);
...
procedure TFrameVstup.cxButtonEditTypePereklKeyPress(Sender: TObject;
  var Key: Char);
begin
  Key:=CHR(0);
end;

procedure TFrameVstup.cxButtonEditTypePereklPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
var
  res: Variant;
begin
  res :=  uPrK_Loader.ShowPrkSprav(self,TFPrKPereklad(self.Owner).DataBasePrK_ABIT.Handle,PrK_SP_TYPE_PEREKL,fsNormal);
  if VarArrayDimCount(res) > 0 then
  begin
    if res[0]<>NULL THEN
    begin
        TFPrKPereklad(self.Owner).ResultArray['AbitData']['ID_SP_TYPE_PEREKL'].AsInt64      := Res[0];
        TFPrKPereklad(self.Owner).ResultArray['AbitData']['Name_SP_TYPE_PEREKL'].AsString   := Res[2];
        cxButtonEditTypePerekl.Text      :=TFPrKPereklad(self.Owner).ResultArray['AbitData']['Name_SP_TYPE_PEREKL'].AsString;
    end;
  end;
end;
