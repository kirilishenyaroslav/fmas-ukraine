...
type
  TFormPrKSpravEditOCENKA = class(TFormPrKKlassSpravEdit)
    ...
    cxCheckBoxIsNegative: TcxCheckBox;
    cxLabelIsNegative: TcxLabel;
...
begin
  ...
  if DataPrKSpravEdit.IsNegative=0
   then cxCheckBoxIsNegative.Checked    := false
   else cxCheckBoxIsNegative.Checked    := true;
...
begin
  ...
  cxLabelOcenka.Caption      :=nLabelKOLVO_BALLOV[IndLangEdit];
  cxLabelIsNegative.Caption  :=nLabelIS_NEGATIVE[IndLangEdit];
end;
...
begin
  ...
  if cxCheckBoxIsNegative.Checked=false
   then DataPrKSpravEdit.IsNegative:=0
   else DataPrKSpravEdit.IsNegative:=1;
