...
type
  TFormPrKSpravEditMIN_PERELIK_SPEC = class(TFormPrKKlassSpravEdit)
    ...
    ClearMinPereilkPrimBtn: TcxButton;
    ...
    procedure cxButtonEditMinPerelikPrimKeyPress(Sender: TObject;
      var Key: Char);
    procedure ClearMinPereilkPrimBtnClick(Sender: TObject);
  private
...
begin
  ...
  if trim(cxButtonEditMinPerelikPrim.Text)='' then
    DataPrKSpravEdit.IdMinPerelikPRIM :=-1;
end;
...
procedure TFormPrKSpravEditMIN_PERELIK_SPEC.ClearMinPereilkPrimBtnClick(
  Sender: TObject);
begin
  inherited;
  DataPrKSpravEdit.IdMinPerelikPRIM :=-1;
  DataPrKSpravEdit.ShortNamePRIM    :='';
  cxButtonEditMinPerelikPrim.Text   :='';
end;
