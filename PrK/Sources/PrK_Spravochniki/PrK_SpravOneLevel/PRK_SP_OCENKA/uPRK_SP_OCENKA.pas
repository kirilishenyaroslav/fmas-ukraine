...
type
  TFormPRK_SP_OCENKA = class(TFormPrK_SpravOneLevel)
    ...
    colIS_NEGATIVE: TcxGridDBColumn;
  ...
  private
    ...
    constructor Create(aKodMax: Integer;aNppMax: Integer);overload;override;
     constructor Create(aId:int64; aName:String; aShortName:String;
                       ...
                       aOcenka_Num :Smallint; aIsNegative :Smallint);overload;
    ...
    procedure SetIsNegative(const Value: Smallint);
  public
    ...
    property IsNegative :Smallint read FIsNegative write SetIsNegative;
...
begin
  ...
  colOCENKA_NUM.Caption      :=ncolKOLVO_BALLOV[IndexLanguage];
  colIS_NEGATIVE.Caption     :=ncolIS_NEGATIVE[IndexLanguage];
end;
...
constructor TDataPrKSpravOcenka.Create(aId: int64; aName,
  ...
  aOcenka_Num: Smallint; aIsNegative: Smallint);
begin
...
begin
 if Value >= 0
    then  FOcenka_Num := Value
    else   FOcenka_Num :=0;
end;

procedure TDataPrKSpravOcenka.SetIsNegative(const Value: Smallint);
begin
  FIsNegative := Value;
end;
...
begin
   ...
   if T.ShowModal=MrOk then
    begin
       ...
       StoredProcPrKSprav.ParamByName('IS_NEGATIVE').AsShort    :=DataPrKSpravAdd.IsNegative;
       try
   ...
   if DataSetPrKSprav.FieldValues[ID_NAME]<>Null then
    begin
       DataPrKSpravChange:=TDataPrKSpravOcenka.Create(StrToInt64(DataSetPrKSprav.FieldValues[ID_NAME]),
                                              ...
                                              DataSetPrKSprav.FieldValues['IS_NEGATIVE']);
       ...
       if T.ShowModal=MrOk then
        begin
           ...
           StoredProcPrKSprav.ParamByName('IS_NEGATIVE').AsShort    :=DataPrKSpravChange.IsNegative;
           try
              StoredProcPrKSprav.ExecProc;
              StoredProcPrKSprav.Transaction.Commit;
           except on e: Exception do
...
begin
   if DataSetPrKSprav.FieldValues[ID_NAME]<>Null then
    begin
       ...
       T.cxCheckBoxIsNegative.Properties.ReadOnly    :=true;
       ...
       T.cxCheckBoxIsNegative.Style.Color        :=TextViewColor;
...
begin
  if DataSetPrKSprav.FieldValues[ID_NAME]<>NULL then
  begin
    ResultArray    :=VarArrayCreate([0,8], varVariant);
    ...
    ResultArray[8] :=DataSetPrKSprav.FieldValues['IS_NEGATIVE'];
    close;
  end;
end;
