...
type
  TFormPRK_SP_TYPE_PEREKL = class(TFormPrK_SpravOneLevel)
...
var
  FormPRK_SP_TYPE_PEREKL: TFormPRK_SP_TYPE_PEREKL;

implementation
uses uPrK_Resources,uConstants;
{$R *.dfm}

procedure TFormPRK_SP_TYPE_PEREKL.FormCreate(Sender: TObject);
begin
  inherited;
{ID_NAME должен стоять первым так как в SelectSQLText может делаться CloseOpen}
  ID_NAME              :='ID_SP_TYPE_PEREKL';
  SelectSQLText        :='Select * from PRK_SP_TYPE_PEREKL_SELECT';
  ShowNpp              := false;
  StoredProcAddName    :='PRK_SP_TYPE_PEREKL_ADD';
  StoredProcChangeName :='PRK_SP_TYPE_PEREKL_CHANGE';
  StoredProcDelName    :='PRK_SP_TYPE_PEREKL_DEL';
  NamePrKSpravEdit     := PrKSpravEditStandart;// возможно это не надо будет

  InicFormCaption      :=nFormPRK_SP_TYPE_PEREKL_Caption[IndexLanguage];
