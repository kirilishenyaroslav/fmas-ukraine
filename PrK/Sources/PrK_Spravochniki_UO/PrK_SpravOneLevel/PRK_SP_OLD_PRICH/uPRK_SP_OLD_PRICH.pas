...
type
  TFormPRK_SP_OLD_PRICH = class(TFormPrK_SpravOneLevel)
...
var
  FormPRK_SP_OLD_PRICH: TFormPRK_SP_OLD_PRICH;

implementation
uses uPrK_Resources,uConstants;
{$R *.dfm}

procedure TFormPRK_SP_OLD_PRICH.FormCreate(Sender: TObject);
begin
  inherited;
{ID_NAME должен стоять первым так как в SelectSQLText может делаться CloseOpen}
  ID_NAME              :='ID_SP_OLD_PRICH';
  SelectSQLText        :='Select * from PRK_SP_OLD_PRICH_SELECT';
  ShowNpp              := false;
  StoredProcAddName    :='PRK_SP_OLD_PRICH_ADD';
  StoredProcChangeName :='PRK_SP_OLD_PRICH_CHANGE';
  StoredProcDelName    :='PRK_SP_OLD_PRICH_DEL';
  NamePrKSpravEdit     := PrKSpravEditStandart;// возможно это не надо будет

  InicFormCaption      :=nFormPRK_SP_OLD_PRICH_Caption[IndexLanguage];
