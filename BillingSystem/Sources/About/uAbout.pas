...
procedure TfrmAboutForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TfrmAboutForm.Timer2Timer(Sender: TObject);
begin
...
procedure TfrmAboutForm.HyperLinkMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmAboutForm.HyperLinkMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  HyperLink.Properties.LinkColor:= clBlue;
end;

procedure TfrmAboutForm.FormCreate(Sender: TObject);
var
  ButtonRGN : HRGN;
  FormRGN   : HRGN;
begin
...
procedure TfrmAboutForm.cxButton1Click(Sender: TObject);
begin
  Timer2.Enabled:=true;
end;

procedure TfrmAboutForm.CloseSpeedButtonClick(Sender: TObject);
begin
  close;
end;

procedure TfrmAboutForm.WMNCHitTest(var Message : TWMNCHitTest);
begin
  if (Message.XPos-Left < 90) and  (Message.YPos-Top < 100)
   then Message.Result  := HTCAPTION {как бы на заголовке}
   else Message.Result := HTCLIENT;

end;

procedure TfrmAboutForm.Label9Click(Sender: TObject);
begin
  ShellExecute(Application.Handle,'open','mailto:harry@buhg.dongu.donetsk.ua',nil,nil,SW_SHOWNORMAL);
end;

procedure TfrmAboutForm.Label10Click(Sender: TObject);
begin
  ShellExecute(Application.Handle,'open','mailto:lexus@buhg.dongu.donetsk.ua',nil,nil,SW_SHOWNORMAL);
end;

procedure TfrmAboutForm.Label8Click(Sender: TObject);
begin
  ShellExecute(Application.Handle,'open','mailto:hunter@buhg.dongu.donetsk.ua',nil,nil,SW_SHOWNORMAL);
end;

procedure TfrmAboutForm.Label11Click(Sender: TObject);
begin
