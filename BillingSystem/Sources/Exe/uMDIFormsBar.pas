...
type
  ...
  public
   ...
   constructor Create(aOwnerSubItem:TdxBarSubItem; aOwnerBarMan:TdxBarManager);overload;
...
constructor TMDIToolBar.Create(aOwnerSubItem: TdxBarSubItem;
  aOwnerBarMan: TdxBarManager);
begin
  SubItem:=aOwnerSubItem;
  BarManager:=aOwnerBarMan;
  Screen.OnActiveFormChange := MDIActiveFormChange;
end;
...
begin
  ...
  for i := Num to Length(MDIBtnArray) - 2 do MDIBtnArray[i] := MDIBtnArray[i + 1];

  SetLength(MDIBtnArray, Length(MDIBtnArray) - 1);

  if (SubItem<>nil) and (Length(MDIBtnArray)>0)
   then SubItem.ItemLinks.Items[PItemLinkBeginGroup].BeginGroup:=true;
end;
...
begin
  ...
  if ((Screen.ActiveForm = nil) and
     ((BarManager.Owner as TForm).MDIChildCount = 0)) or
     (csDestroying in (BarManager.Owner as TForm).ComponentState) then
   begin
  ...
  for i := 0 to Length(MDIBtnArray) - 1 do
   if Screen.ActiveForm.Name = MDIBtnArray[i].FName then
    begin
    ...
    begin
     Deleted := True;
     for j := 0 to (BarManager.Owner as TForm).MDIChildCount - 1 do
      begin
      if (BarManager.Owner as TForm).MDIChildren[j].Name = MDIBtnArray[i].FName then
       begin
     ...
     if Deleted or (MDIBtnArray[i].FName = '') then
      begin
       MDIBtnArray[i].Btn.Destroy;
       SetLength(DelArray, Length(DelArray) + 1);
       DelArray[Length(DelArray) - 1] := i;
   ...
   if not Flag then
    begin
     ...
     with MDIBtnArray[Length(MDIBtnArray) - 1] do
      begin
       ...
       with Btn do
        begin
         ...
         if SubItem<>nil
          then Caption := strCaption
          else
           if Length(strCaption) < 35
            then Caption     := strCaption
            else
     ...
     if SubItem<> nil then
      with BarManager.BarByName('Main Menu'), SubItem.ItemLinks do
       begin
        LockUpdate := True;
      ...
      with BarManager.BarByName('MDIBar'),  ItemLinks do
       begin
        LockUpdate := True;
...
begin
  ...
  b    := False;
  for i := 0 to Length(MDIBtnArray) - 1 do
   if MDIBtnArray[i].Btn.Name = (Sender as TdxBarButton).Name then
    begin
     b := True;

     for j := 0 to ((Sender as TdxBarButton).BarManager.Owner as TForm).MDIChildCount - 1 do
      if ((Sender as TdxBarButton).BarManager.Owner as TForm).MDIChildren[j].Name = MDIBtnArray[i].FName then
       begin
        with ((Sender as TdxBarButton).BarManager.Owner as TForm).MDIChildren[j] do
         begin
...
procedure TMDIToolBar.SetVisible(const Value: boolean);
begin
  FVisible:=Value;
  Bar.Visible:=Value;
end;
